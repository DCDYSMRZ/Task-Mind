...
┌─────────────────────────────────────┐
│  Project (.task-mind/recipes/)         │  ← 优先级最高
│  - 项目特定的Recipe                 │
│  - 团队共享                         │
└─────────────────────────────────────┘
                │
...
场景1：/task-mind.run "从Upwork提取Python职位"
─────────────────────────────────────────
用户 → /task-mind.run → AI分析 → 发现Recipe: upwork_extract_job_details
     → 创建Run: cong-upwork-ti-qu-python-zhi-wei
     → 调用Recipe(chrome-js) → CDP执行 → 输出markdown文件
     → 记录JSONL日志 → 持久化Run上下文


场景2：/task-mind.recipe "提取YouTube字幕"
────────────────────────────────────
用户 → /task-mind.recipe → AI生成Recipe → 保存到.task-mind/recipes/
     → 测试Recipe → CDP执行 → 验证输出
     → 添加到Recipe注册表
...
场景3：直接CLI命令
──────────────
开发者 → task-mind chrome navigate https://...
...
| 维度 | **Playwright / Selenium** | **task-mind** |
|------|--------------------------|-----------|
...
**关键差异**：
- ✅ **持久化浏览器会话** - Playwright每次测试启动新浏览器，task-mind连接已运行的Chrome实例
...
高频操作路径：
  ...
  例如：YouTube字幕提取
  1. 用户：/task-mind.recipe "提取YouTube字幕"
  2. AI：交互式定位按钮、提取文本
  3. 固化：youtube_extract_video_transcript.js + 元数据文档
  4. 复用：uv run task-mind recipe run youtube_extract_video_transcript
...
- task-mind: AI决策（分镜设计）+ Recipe加速（重复操作）

### Recipe元数据驱动架构（004迭代）

**设计理念：代码与资源分离**
- `src/task-mind/recipes/` - Python引擎代码（元数据解析、注册表、执行器）
...
2. 用户级：`~/.task-mind/recipes/`（用户主目录）
3. 示例级：`examples/`（仓库根目录）

**三种运行时支持**：
- `chrome-js`：通过 `task-mind chrome exec-js` 执行JavaScript
...
**与人类用户的关系**：
- 人类用户：创建和维护Recipe（通过 `/task-mind.recipe` 命令）
...
```python
# 当 task-mind agent 启动时，记录 start_time
...
```bash
task-mind server start      # 在端口 8093 启动后台守护进程
task-mind server stop       # 停止后台守护进程
task-mind server status     # 检查服务状态
task-mind server --debug    # 在前台运行并显示日志
```
...
│  └─ 持久化: ~/.task-mind/sessions/claude/{session}/
│
├─ Run 系统 (工作记忆)
...
3. **与 Browser Use 的本质不同**
   ...
   - task-mind: AI 编排 + Recipe 加速（控制力强，token 效率高）
