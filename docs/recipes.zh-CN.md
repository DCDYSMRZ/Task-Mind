...
```bash
# AI 调用 Bash 工具
uv run task-mind recipe list --format json
```
...
#### Step 1: AI 调用 `/task-mind.recipe` 命令

```
/task-mind.recipe create workflow "批量提取 10 个 Upwork 职位并保存为 JSON"
```

#### Step 2: AI 自动生成 Workflow 脚本

AI 生成的文件: `~/.task-mind/recipes/workflows/upwork_batch_extract.py`
...
```bash
# 创建 Python 脚本
cat > ~/.task-mind/recipes/atomic/system/clipboard_read.py <<'EOF'
#!/usr/bin/env python3
import sys
import json
import subprocess
...
chmod +x ~/.task-mind/recipes/atomic/system/clipboard_read.py
```

#### 2. 创建元数据文件

```bash
cat > ~/.task-mind/recipes/atomic/system/clipboard_read.md <<'EOF'
---
name: clipboard_read
type: atomic
runtime: python
...
```bash
uv run task-mind recipe run clipboard_read
```
...
```bash
# 查看 Recipe 信息
uv run task-mind recipe info clipboard_read
...
```bash
cat > ~/.task-mind/recipes/workflows/youtube_batch_extract.py <<'EOF'
#!/usr/bin/env python3
import sys
import json
...
chmod +x ~/.task-mind/recipes/workflows/youtube_batch_extract.py
```

#### 2. 创建元数据文件

```bash
cat > ~/.task-mind/recipes/workflows/youtube_batch_extract.md <<'EOF'
---
...
```bash
uv run task-mind recipe run youtube_batch_extract \
...
# 创建项目级 Recipe 目录
mkdir -p .task-mind/recipes/workflows

# 创建项目专属 Workflow
cat > .task-mind/recipes/workflows/project_specific_task.py <<'EOF'
#!/usr/bin/env python3
import sys, json
...
chmod +x .task-mind/recipes/workflows/project_specific_task.py
```

#### 2. 创建元数据文件

```bash
cat > .task-mind/recipes/workflows/project_specific_task.md <<'EOF'
---
...
```bash
# 在项目目录执行
uv run task-mind recipe run project_specific_task
...
```bash
uv run task-mind recipe info <recipe_name>
```
...
```bash
# 启用调试模式
uv run task-mind --debug recipe run <recipe_name> --params '{...}'
```

**输出**:
```text
[DEBUG] RecipeRegistry: 扫描路径 /home/user/.task-mind/recipes
[DEBUG] RecipeRegistry: 找到 5 个 Recipe
[DEBUG] RecipeRunner: 选择执行器 ChromeJSExecutor
[DEBUG] ChromeJSExecutor: 执行命令: task-mind chrome exec-js ...
...
```bash
# 单个环境变量
uv run task-mind recipe run openai_chat \
  --params '{"prompt": "Hello"}' \
...
uv run task-mind recipe run openai_chat --params '{"prompt": "Hello"}'
```
...
├─────────────────────────────────────────┤
│ 3. 项目级 .task-mind/.env (当前项目)        │
├─────────────────────────────────────────┤
│ 4. 用户级 ~/.task-mind/.env (所有项目)      │
├─────────────────────────────────────────┤
│ 5. 系统环境变量       (os.environ)      │
├─────────────────────────────────────────┤
...
import sys
import json
from task-mind.recipes import RecipeRunner, WorkflowContext
...
uv run task-mind recipe info openai_chat

# JSON 格式输出（包含 env 字段）
uv run task-mind recipe info openai_chat --format json
```
...
```bash
task-mind chrome exec-js src/task-mind/recipes/upwork_extract_job.js
```
...
1. **迁移脚本到新位置**:
   ...
   uv run task-mind recipe copy upwork_extract_job_details_as_markdown
   ```
...
**解决**:
1. 检查 Recipe 名称拼写
2. 运行 `uv run task-mind recipe list` 查看可用 Recipe
3. 确认元数据文件 `.md` 存在
...
```bash
# 复制依赖的示例 Recipe
uv run task-mind recipe copy <dependency_name>
...
1. 用户提出任务
   ↓
2. AI 调用: uv run task-mind recipe list --format json
   ↓
3. AI 分析元数据:
   - description: 快速理解功能
...
4. AI 决策:
   - 使用现有 Recipe → 执行
   - 无合适 Recipe → 调用 /task-mind.recipe 生成新 Recipe
   ↓
5. AI 执行: uv run task-mind recipe run <name> --params '{...}' [--output-file/--output-clipboard]
   ↓
6. AI 处理结果:
   - success: true → 向用户报告
...
4. **AI 生成 Workflow**: `/task-mind.recipe` 命令让 AI 自动创建编排 Recipe
...
| **创建 Recipe** | `/task-mind.recipe create` 自动生成 | 手动编写脚本和元数据 |
