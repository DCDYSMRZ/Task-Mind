...
>
> **task-mind needs Python + Node.js + Chrome.** Each OS has different setup:
>
...
>
> On macOS/Linux, `task-mind init` can auto-install Node.js for you. **On Windows, it can't.**
>
> If you run `task-mind init` without Node.js installed, you'll get:
> ```
> Error: Windows 不支持自动安装 Node.js
> ```
>
> → [Install Node.js first](#install-nodejs-required-before-task-mind-init) (takes 2 minutes)
...
> ```powershell
> uv tool install task-mind-cli --python 3.13
> ```
...
| File | Purpose |
|------|---------|
| `~/.task-mind/config.json` | task-mind configuration (auth method, resource status) |
| `~/.claude/settings.json` | Claude Code settings (custom API endpoint config) |
| `~/.claude/commands/task-mind.*.md` | Installed slash commands |
| `~/.task-mind/recipes/` | User-level recipes |
...
```bash
git clone https://github.com/DCDYSMRZ/Task-Mind.git
cd task-mind uv sync --all-extras --dev
```
...
Before installing task-mind on Linux, ensure your system meets the following requirements.

### What You Need to Prepare

| Dependency | Purpose | Required |
|------------|---------|----------|
| **Python 3.9+** | task-mind runtime | Yes |
...
### What task-mind Does NOT Do For You

1. **Install system packages** - task-mind cannot run `sudo apt/dnf/pacman` commands
...
Before installing task-mind, verify:
...
Before installing task-mind on macOS, ensure your system meets the following requirements.
...
Then install task-mind as described in [Step 2](#step-2-install-task-mind).
...
Before installing task-mind on Windows, ensure your system meets the following requirements.

> **Important**: Unlike macOS/Linux, Windows does NOT support automatic Node.js installation via nvm. You must install Node.js manually before running `task-mind init`.
...
### Install Node.js (Required Before task-mind init)
...
Before installing task-mind, verify in PowerShell:

```powershell
# Python 3.9+ installed
python --version

# Node.js 20+ installed (REQUIRED before task-mind init)
node --version
...
```bash
uv tool uninstall task-mind-cli
```
