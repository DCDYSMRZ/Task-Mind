...
# task-mind Technical Architecture

## System Architecture

```
task-mind Usage Flow Architecture
==============================
...
┌─────────────────────────────────────┐
│  Project (.task-mind/recipes/)         │  ← Highest priority
...
└─────────────────────────────────────┘
                ...
                ▼
┌─────────────────────────────────────┐
│  User (~/.task-mind/recipes/)          │  ← Medium priority
...
Scenario 1: /task-mind.run "Extract Python jobs from Upwork"
─────────────────────────────────────────────────────────
User → /task-mind.run → AI analysis → Discover Recipe: upwork_extract_job_details
...
Scenario 2: /task-mind.recipe "Extract YouTube subtitles"
──────────────────────────────────────────────────────
User → /task-mind.recipe → AI generates Recipe → Save to .task-mind/recipes/
...
────────────────────────────────
Developer → task-mind chrome navigate https://...
...
### task-mind vs Playwright / Selenium

| Dimension | **Playwright / Selenium** | **task-mind** |
|-----------|---------------------------|-----------|
...
**Key Differences**:
- ✅ **Persistent browser sessions** - Playwright launches new browser per test, task-mind connects to running Chrome instance
...
### task-mind vs Browser Use

| Dimension | **Browser Use** | **task-mind** |
|-----------|----------------|-----------|
...
3. **Dependency Lightweighting**
   ...
   - task-mind: ~2MB (websocket-client)
...
High-frequency operation path:
  ...
  1. User: /task-mind.recipe "Extract YouTube subtitles"
  2. AI: Interactively locate button, extract text
  3. Solidify: youtube_extract_video_transcript.js + metadata documentation
  4. Reuse: uv run task-mind recipe run youtube_extract_video_transcript
...
task-mind chrome exec-js examples/atomic/chrome/youtube_extract_video_transcript.js
```

**Difference from Browser Use**:
- Browser Use: Every task needs LLM reasoning ($$$)
- task-mind: AI decision-making (storyboard design) + Recipe acceleration (repeated operations)

### Recipe Metadata-Driven Architecture (Iteration 004)

**Design Philosophy: Code-Resource Separation**
- `src/task-mind/recipes/` - Python engine code (metadata parsing, registry, executor)
- `examples/atomic/chrome/` - Example Recipe scripts + metadata documentation
- `~/.task-mind/recipes/` - User-level Recipes (to be implemented)
- `.task-mind/recipes/` - Project-level Recipes (to be implemented)
...
1. Project-level: `.task-mind/recipes/` (current working directory)
2. User-level: `~/.task-mind/recipes/` (user home directory)
3. Example-level: `examples/` (repository root)

**Three Runtime Support**:
- `chrome-js`: Execute JavaScript via `task-mind chrome exec-js`
...
- Human users: Create and maintain Recipes (via `/task-mind.recipe` command)
...
**1. Initialization**
```bash
uv run task-mind run init "Research YouTube subtitle extraction"
...
└──────────────────┬──────────────────────────────────────┘
                   ...
                   ▼
┌─────────────────────────────────────────────────────────┐
│              task-mind Session Storage                       │
│  ~/.task-mind/sessions/{agent_type}/{session_id}/           │
...
```python
# When task-mind agent starts, it records start_time
...
```bash
task-mind server start      # Start background daemon on port 8093
task-mind server stop       # Stop background daemon
task-mind server status     # Check server status
task-mind server --debug    # Run in foreground with logs
```
...
│  └─ Persists: ~/.task-mind/sessions/claude/{session}/
│
