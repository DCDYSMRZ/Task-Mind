...
本文档展示 task-mind 在实际场景中的应用方式，涵盖从 Recipe 创建到复杂 Workflow 编排的完整流程。

---

## 为什么用 task-mind 驱动 AI 自动化

AI agent 执行浏览器自动化时面临一个根本问题：每次对话都从零开始，没有过去工作的记忆。task-mind 通过三个机制解决这个问题：
...
| 模式 | 命令 | 目标 | 输出 |
|-----|------|------|------|
| **探索调研** | `/task-mind.run` | 收集信息以创建 Recipe | JSONL 日志 + 验证脚本 + Recipe 草稿 |
| **沉淀 Recipe** | `/task-mind.recipe` | 将探索结果转化为可复用脚本 | Recipe 文件（.js/.py + .md） |
| **任务执行** | `/task-mind.exec` | 完成具体业务目标 | 任务结果 + 执行日志 |

选择建议：
- **未知页面/流程**：使用 `/task-mind.run` 探索，积累上下文
- **重复性任务**：创建 Recipe 后使用 `/task-mind.exec` 或直接执行 Recipe
...
```
/task-mind.recipe 为这个页面生成一个提取脚本：https://news.ycombinator.com/，提取前10条新闻的标题和链接
```
...
3. **验证与保存**：在临时环境中测试运行，成功后保存至 `.task-mind/recipes/project/`
4. **即时可用**：后续可通过 `/task-mind.run` 直接调用该 Recipe

### CLI 等效操作

```bash
# 手动创建 Recipe（需要人工编写代码）
cat > .task-mind/recipes/project/hn_extract.js <<'EOF'
(async () => {
...
# 创建元数据文件
cat > .task-mind/recipes/project/hn_extract.md <<'EOF'
---
name: hn_extract
type: atomic
runtime: chrome-js
...
# 执行 Recipe
uv run task-mind recipe run hn_extract --output-file news.json
```
...
```
/task-mind.run 提取这个视频的字幕：https://www.youtube.com/watch?v=dQw4w9WgXcQ
```
...
```bash
# 1. 初始化调试会话
uv run task-mind run init "排查登录页面布局偏移问题"

# 2. 执行一系列操作（上下文自动关联到当前 Run）
task-mind chrome navigate https://staging.example.com/login
task-mind chrome exec-js "window.innerWidth"
task-mind chrome screenshot before_click.png
task-mind chrome click "#login-btn"
task-mind chrome screenshot after_click.png
...
# 4. 归档会话
uv run task-mind run archive "排查登录页面布局偏移问题"
```

### Claude Code 方式

```
/task-mind.run 排查 staging.example.com/login 的布局偏移问题，先截图，点击登录按钮后再截图，对比差异
```
...
```
/task-mind.run 监控 Amazon 和 eBay 上 iPhone 15 的价格，生成对比报告并保存为 markdown
```
