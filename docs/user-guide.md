...
# task-mind User Guide

## Overview

task-mind is a multi-runtime automation infrastructure designed for AI agents, providing four core systems that work together to solve automation challenges:
...
task-mind is suitable for various browser automation and data collection tasks:
...
task-mind run init "Research YouTube subtitle extraction methods"
# Output: Created Run instance: youtube-subtitle-research-abc123

# 2. Set as current working context (optional)
task-mind run set-context youtube-subtitle-research-abc123

# 3. Execute operations (automatically logged to current Run)
task-mind chrome navigate https://youtube.com/watch?v=...
task-mind chrome screenshot initial_page.png
task-mind chrome click 'button[aria-label="Show transcript"]'
...
task-mind run info youtube-subtitle-research-abc123

# 6. List all Run instances
task-mind run list

# 7. Archive completed Run
task-mind run archive youtube-subtitle-research-abc123
```
...
All CDP functionality is accessed through a unified CLI interface (`task-mind chrome <command>`).
...
# Take screenshot
task-mind chrome screenshot <output_file>

# Scroll page
task-mind chrome scroll <direction> <pixels>
...
task-mind chrome spotlight <selector> --duration 3

# Highlight element
task-mind chrome highlight <selector> --color "#FF6B6B" --duration 2

# Add annotation
task-mind chrome annotate <selector> "Annotation text" --position top

# Pointer indicator
task-mind chrome pointer <selector> --duration 2
```

### Proxy Configuration

task-mind's CDP integration supports proxy configuration for environments requiring network access through a proxy.
...
# Bypass proxy
task-mind chrome navigate https://example.com --no-proxy
```
...
# Filter by source
task-mind recipe list --source project   # Project-level only
task-mind recipe list --source user      # User-level only
task-mind recipe list --source example   # Example-level only

# Filter by type
task-mind recipe list --type atomic      # Atomic Recipes only
task-mind recipe list --type workflow    # Workflow Recipes only
```
...
task-mind recipe info youtube_extract_video_transcript

# View in JSON format (for AI parsing)
task-mind recipe info youtube_extract_video_transcript --format json
```
...
task-mind recipe run youtube_extract_video_transcript \
    --params '{"url": "https://youtube.com/watch?v=..."}' \
    --output-file transcript.txt

# Output to clipboard
task-mind recipe run upwork_extract_job_details_as_markdown \
    --params '{"url": "..."}' \
    --output-clipboard
...
```
1. Project (.task-mind/recipes/)     ← Highest priority (project-specific)
2. User (~/.task-mind/recipes/)      ← Medium priority (personal recipes)
...
task-mind recipe copy youtube_extract_video_transcript

# Recipe is now at: ~/.task-mind/recipes/atomic/chrome/youtube_extract_video_transcript.js
# Customize as needed
```
...
```
/task-mind.recipe create "Extract complete job information from Upwork and format as Markdown"
```
...
task-mind session list --status running   # Only running sessions
task-mind session list --agent claude     # Only Claude Code sessions

# Show session details
task-mind session show <session_id>
task-mind session show <session_id> --format json

# Watch session in real-time
task-mind session watch                    # Watch latest session
task-mind session watch <session_id>       # Watch specific session
task-mind session watch --json             # JSON output format
```

### Session Storage

Session data is stored in `~/.task-mind/sessions/{agent_type}/{session_id}/`:

```
~/.task-mind/sessions/claude/abc123-def456/
...
### Integration with task-mind agent

When running `task-mind agent "task"`, the session monitor automatically:
...
5. Persists session data to `~/.task-mind/sessions/`

```bash
# Execute task with session monitoring
task-mind agent "Extract data from website"
...
task-mind provides a browser-based GUI through a local web service. No extra installation needed—just Chrome browser.

### Starting Web Service

```bash
# Start background server (recommended)
task-mind server start      # Starts on port 8093

# Or start in foreground (for debugging)
task-mind server --debug    # Runs until Ctrl+C with logs
```

### Server Commands

```bash
task-mind server start      # Start background server
task-mind server stop       # Stop background server
task-mind server status     # Check if server is running
```
...
- **Storage**: Titles stored in `~/.task-mind/sessions.json`
...
task-mind provides slash commands for Claude Code integration, enabling AI-driven task execution.

### Available Slash Commands

```
/task-mind.run <task_description>
```
...
**Examples**:
```
/task-mind.run Research YouTube subtitle extraction methods: locate button, test extraction, save script

/task-mind.run Extract 20 Python jobs from Upwork and save as JSON

/task-mind.run Monitor iPhone 15 prices on Amazon and eBay, generate comparison report
```

---

```
/task-mind.recipe create <recipe_description>
```
...
**Examples**:
```
/task-mind.recipe create "Extract GitHub repository README and save as Markdown"

/task-mind.recipe create workflow "Batch extract subtitles from 10 YouTube videos"
```

---

```
/task-mind.exec <one_time_task>
```
Execute one-time tasks without creating Run instance.

**Examples**:
```
/task-mind.exec Take screenshot of current page and annotate the login button

/task-mind.exec Extract all links from https://news.ycombinator.com/
```
...
**Solutions**:
1. List available Recipes: `task-mind recipe list`
...
python3 -m pip install task-mind-cli
```
...
/Applications/Google\ Chrome.app/Contents/MacOS/Google\ Chrome \
    --remote-debugging-port=9222 \
    --user-data-dir=~/.task-mind/chrome_profile
...
py -m pip install task-mind-cli
...
& "C:\Program Files\Google\Chrome\Application\chrome.exe" `
    --remote-debugging-port=9222 `
    --user-data-dir="$env:USERPROFILE\.task-mind\chrome_profile"
...
**Solutions**:
```powershell
# You must install Node.js manually before running task-mind init
...
# Daily use
task-mind sync              # Push local changes and pull remote updates
task-mind sync --dry-run    # Preview what will be synced
task-mind sync --no-push    # Only pull, don't push
task-mind sync -m "message" # Custom commit message
```
...
|--------------|---------|----------|
| Skills | `task-mind-*` prefix | `~/.claude/skills/` |
| Recipes | All recipes | `~/.task-mind/recipes/` |
...
- **New to task-mind?** Start with [Use Cases](use-cases.md) for real-world examples
