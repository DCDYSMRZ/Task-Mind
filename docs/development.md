...
│   │   ├── task-mind.run.md         # AI-driven complex task execution
│   │   ├── task-mind.recipe.md      # Recipe management command (create/test)
│   │   └── task-mind.exec.md        # One-time task execution
│   └── settings.local.json          # Project configuration
│
├── src/                             # Core Python code
│   ├── task-mind/                      # task-mind core package
...
CDP functionality organized by type in `src/task-mind/cdp/commands/`:

```
src/task-mind/cdp/commands/
...
All CDP functionality accessed through unified CLI interface (`uv run task-mind <command>`).
...
1. **Script locations**:
   - CDP command implementations in `src/task-mind/cdp/commands/`
   - Recipe engine code in `src/task-mind/recipes/`
   - Run system code in `src/task-mind/run/`
   - Session monitoring code in `src/task-mind/session/`
   - GUI application code in `src/task-mind/gui/`
...
uv run python -m task-mind.tools.function_mapping

# Generate detailed HTML report
uv run python -m task-mind.tools.function_mapping --format html --output function_mapping_report.html

# Generate JSON report
uv run python -m task-mind.tools.function_mapping --format json --output function_mapping_report.json
```
...
- **Code-resource separation**:
  - `src/task-mind/recipes/` - Python engine code (no Recipe scripts)
