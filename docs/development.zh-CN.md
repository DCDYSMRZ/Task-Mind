...
│   │   ├── task-mind.start.md       # AI信息收集命令
│   │   ├── task-mind.storyboard.md  # AI分镜规划命令
│   │   ├── task-mind.generate.md    # AI视频生成命令（创作录制脚本）
│   │   ├── task-mind.evaluate.md    # AI素材评估命令
│   │   ├── task-mind.merge.md       # AI视频合成命令
│   │   └── task-mind.recipe.md      # Recipe管理命令（创建/更新/列表）
│   └── settings.local.json          # 项目配置
│
├── src/                             # 核心Python代码
│   ├── task_mind/                   # task-mind核心包
...
CDP功能按类型组织在 `src/task-mind/cdp/commands/` 目录下：
...
所有CDP功能通过统一的CLI接口（`uv run task-mind <command>`）访问。
...
- **代码与资源分离**：
  - `src/task-mind/recipes/` - Python引擎代码（不包含Recipe脚本）
  - `examples/atomic/chrome/` - 示例Recipe脚本 + 元数据文档
  - `~/.task-mind/recipes/` - 用户级Recipe（待实现）
  - `.task-mind/recipes/` - 项目级Recipe（待实现）
