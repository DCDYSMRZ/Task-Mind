...
> ## ⚠️ 安装前必读
>
> **task-mind 需要 Python + Node.js + Chrome。** 不同系统的准备工作不同：
>
> - **Linux**：可能没有预装 Python
...
> **Windows 用户 — 跳过这步必定报错**
>
> macOS/Linux 上，`task-mind init` 能帮你自动装 Node.js。**Windows 上不行。**
>
> 如果你没装 Node.js 就运行 `task-mind init`，会看到：
> ```
> Error: Windows 不支持自动安装 Node.js
> ```
>
> → [先装 Node.js](#安装-nodejstask-mind-init-之前必须安装)（只要 2 分钟）
...
4. **资源安装**
   ...
   - 安装示例 Recipe 到 `~/.task-mind/recipes/`
...
| 文件 | 用途 |
|------|------|
| `~/.task-mind/config.json` | task-mind 配置（认证方式、资源状态） |
| `~/.claude/settings.json` | Claude Code 设置（自定义 API 端点配置） |
| `~/.claude/commands/task-mind.*.md` | 已安装的 slash 命令 |
| `~/.task-mind/recipes/` | 用户级 Recipe |
...
在 Linux 上安装 task-mind 之前，请确保系统满足以下要求。

### 需要准备的依赖

| 依赖 | 用途 | 是否必须 |
|------|------|----------|
| **Python 3.9+** | task-mind 运行时 | 是 |
...
1. **安装系统级软件** - task-mind 无法执行 `sudo apt/dnf/pacman` 命令
...
在 macOS 上安装 task-mind 之前，请确保系统满足以下要求。
...
然后按照 [第二步](#第二步安装-task-mind) 安装 task-mind。
...
在 Windows 上安装 task-mind 之前，请确保系统满足以下要求。

> **重要**：与 macOS/Linux 不同，Windows 不支持通过 nvm 自动安装 Node.js。你必须在运行 `task-mind init` 之前手动安装 Node.js。
...
### 安装 Node.js（task-mind init 之前必须安装）
...
安装 task-mind 之前，在 PowerShell 中验证：

```powershell
# Python 3.9+ 已安装
python --version

# Node.js 20+ 已安装（task-mind init 之前必须安装）
node --version
