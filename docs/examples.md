...
# task-mind Example Reference

This document provides practical examples of using task-mind's three core systems (Run + Recipe + CDP) for various automation tasks.
...
task-mind chrome navigate https://www.youtube.com/watch?v=dQw4w9WgXcQ
...
task-mind chrome exec-js 'document.querySelector("button[aria-label*=\"transcript\"]")' --return-value
```
...
task-mind chrome click 'button[aria-label*="transcript"]'
task-mind chrome screenshot transcript_opened.png
```
...
```bash
uv run task-mind run info youtube-subtitle-research-abc123
```
...
cat > ~/.task-mind/recipes/atomic/chrome/youtube_extract_video_transcript.js <<'EOF'
(async () => {
  const button = document.querySelector('button[aria-label*="transcript"]');
  if (button) {
    button.click();
...
cat > ~/.task-mind/recipes/atomic/chrome/youtube_extract_video_transcript.md <<'EOF'
---
...
```
/task-mind.recipe create "Extract YouTube video subtitles" from run youtube-subtitle-research-abc123
```
...
```yaml
---
# ~/.task-mind/recipes/workflows/upwork_batch_extract.md
...
uv run task-mind run init "Batch extract Python jobs from Upwork"
...
---
Generated with task-mind | Run ID: iphone-15-price-monitoring-abc123
```
...
# Navigate to page
task-mind chrome navigate https://news.ycombinator.com/

# Wait for page load
task-mind chrome wait 2

# Click element
task-mind chrome click 'a.titlelink:first-child'

# Get page title
task-mind chrome exec-js 'document.title' --return-value
```
...
task-mind chrome screenshot hackernews_page.png

# Highlight specific element
task-mind chrome highlight '.storylink' --color "#FF6B6B" --duration 3

# Spotlight effect (dim surroundings)
task-mind chrome spotlight '.athing:first-child' --duration 5

# Add annotation
task-mind chrome annotate '.score' "Top story" --position top
```
...
```bash
# Extract all links
task-mind chrome exec-js 'Array.from(document.querySelectorAll("a")).map(a => a.href)' \
  --return-value

# Scroll to bottom
task-mind chrome exec-js 'window.scrollTo(0, document.body.scrollHeight)'

# Check element existence
task-mind chrome exec-js 'document.querySelector(".pagetop") !== null' \
  --return-value
```
...
uv run task-mind run list

# AI auto-discovery (fuzzy matching)
# User says: "Continue YouTube subtitle research"
# AI executes:
uv run task-mind run list --format json
# AI finds: youtube-subtitle-research-abc123 (95% match)
# AI resumes:
uv run task-mind run set-context youtube-subtitle-research-abc123
```
...
uv run task-mind run archive youtube-subtitle-research-abc123
...
uv run task-mind recipe copy youtube_extract_video_transcript
```
...
```bash
# ❌ Wrong: Relative path
task-mind chrome screenshot screenshot.png

# ✅ Correct: Absolute path
task-mind chrome screenshot $(pwd)/screenshot.png

# ✅ Correct: Within Run context
uv run task-mind run init "My task"
task-mind chrome screenshot screenshot.png  # Auto-saved to Run's screenshots/
```
