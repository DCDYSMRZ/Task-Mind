...
**存放位置**（三级优先级）：
1. `.task-mind/recipes/` - 项目级（最高优先级）
2. `~/.task-mind/recipes/` - 用户级
3. `examples/` - 示例级
...
## Claude Code 的概念（非 task-mind 原创）

以下概念来自 [Claude Code](https://docs.anthropic.com/en/docs/claude-code)，task-mind 基于这些概念进行扩展。
...
**示例**：`/task-mind.run`、`/task-mind.recipe`、`/task-mind.exec`、`/task-mind.test`

---

## task-mind 的贡献

task-mind 不是发明了新概念，而是**将 Claude Code 的 skill 与 task-mind 的 recipe 关联起来**，并提供了一套完整的工具链。
...
task-mind 提供四个 slash command，形成完整的工作闭环：

```
/task-mind.run     探索研究，积累经验（产出：Run 实例）
     ↓
/task-mind.recipe  将经验固化为配方（产出：Recipe）
/task-mind.test    验证配方正确性（趁上下文还在）
     ↓
...
**存放位置**：`~/.task-mind/sessions/{agent_type}/{session_id}/`
...
- **Recipe**（task-mind）：配方，具体怎么执行
- **Run**（task-mind）：任务实例，记录探索过程
- **Session**（task-mind）：Agent 会话，实时执行监控
- **task-mind 的贡献**：将所有概念关联，提供探索→固化→执行→监控的闭环工具
