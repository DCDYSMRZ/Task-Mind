...
This document explains the core concepts in the task-mind project and their origins.

![Concept Relationship Diagram](images/concepts-diagram-en_20251209_132837_0.jpg)

---

## Claude Code Concepts (Not task-mind Original)

The following concepts come from [Claude Code](https://docs.anthropic.com/en/docs/claude-code). task-mind extends upon these concepts.
...
**Examples**: `/task-mind.run`, `/task-mind.recipe`, `/task-mind.exec`, `/task-mind.test`

---

## task-mind Concepts

The following concepts are original designs from the task-mind project.
...
1. `.task-mind/recipes/` - Project level (highest priority)
2. `~/.task-mind/recipes/` - User level
...
## task-mind's Contribution

task-mind didn't invent new concepts. Instead, it **links Claude Code's skill with task-mind's recipe** and provides a complete toolchain.

### Linking Skill and Recipe

| | Skill (Claude Code) | Recipe (task-mind) |
|--|---------------------|----------------|
...
​```bash
uv run task-mind recipe run volcengine_tts_with_emotion \
...
task-mind provides four slash commands that form a complete workflow loop:

```
/task-mind.run     Explore and research, accumulate experience (Output: Run instance)
     ↓
/task-mind.recipe  Solidify experience into recipes (Output: Recipe)
/task-mind.test    Validate recipe correctness (while context is fresh)
     ↓
...
| | Workflow Nodes | task-mind Recipe |
|--|----------------|--------------|
...
**Storage location**: `~/.task-mind/sessions/{agent_type}/{session_id}/`

**Structure**:
```
~/.task-mind/sessions/claude/abc123/
...
- **Recipe** (task-mind): Recipe, specific execution steps
- **Run** (task-mind): Task instance, records exploration process
- **Session** (task-mind): Agent session, real-time execution monitoring
- **task-mind's contribution**: Links all concepts, provides explore → solidify → execute → monitor loop tools
