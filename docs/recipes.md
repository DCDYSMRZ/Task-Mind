...
#### Step 1: AI Invokes `/task-mind.recipe` Command

```
/task-mind.recipe create workflow "Batch extract 10 Upwork jobs and save as JSON"
```

#### Step 2: AI Automatically Generates Workflow Script

AI-generated file: `~/.task-mind/recipes/workflows/upwork_batch_extract.py`
...
**Output**:
```text
[DEBUG] RecipeRegistry: Scanning path /home/user/.task-mind/recipes
[DEBUG] RecipeRegistry: Found 5 Recipes
[DEBUG] RecipeRunner: Selected executor ChromeJSExecutor
[DEBUG] ChromeJSExecutor: Executing command: task-mind chrome exec-js ...
...
├─────────────────────────────────────────┤
│ 3. Project .task-mind/.env (current project)│
├─────────────────────────────────────────┤
│ 4. User ~/.task-mind/.env (all projects)    │
├─────────────────────────────────────────┤
...
2. Run `uv run task-mind recipe list` to view available Recipes
...
1. User proposes task
   ↓
2. AI invokes: uv run task-mind recipe list --format json
   ↓
...
4. AI decision:
   ...
   - No suitable Recipe → Invoke /task-mind.recipe to generate new Recipe
   ↓
5. AI executes: uv run task-mind recipe run <name> --params '{...}' [--output-file/--output-clipboard]
   ↓
...
4. **AI Generates Workflow**: `/task-mind.recipe` command enables AI to automatically create orchestrated Recipes
...
| **Create Recipe** | `/task-mind.recipe create` auto-generate | Manually write script and metadata |
