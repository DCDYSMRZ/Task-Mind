...
This document demonstrates task-mind's application in real-world scenarios, showcasing how the three core systems (Run System + Recipe System + Native CDP) work together to solve AI agents' automation challenges.

---

## Why task-mind for AI-Driven Automation

AI agents executing browser automation face a fundamental problem: each conversation starts fresh with no memory of past work. task-mind addresses this through three mechanisms:
...
|------|---------|------|--------|
| **🧠 Exploration** | `uv run task-mind run init` + CDP commands | Explore unknown workflows, accumulate context | JSONL logs + validated scripts + screenshots |
| **📚 Solidification** | `/task-mind.recipe create` | Transform exploration into reusable Recipe | Recipe files (.js/.py + .md metadata) |
| **⚡ Execution** | `uv run task-mind recipe run` or `/task-mind.run` | Complete specific tasks efficiently | Task results + execution logs |
...
task-mind chrome screenshot step1.png  # Where did this file go?
task-mind chrome click 'button[aria-label="Show transcript"]'
task-mind chrome screenshot step2.png  # Lost context from step1
...
uv run task-mind run init "Research YouTube subtitle extraction methods"
# Output: Created Run instance: youtube-subtitle-extraction-abc123

# 2. All subsequent operations automatically link to this Run
task-mind chrome navigate https://youtube.com/watch?v=dQw4w9WgXcQ
task-mind chrome screenshot step1_initial_page.png
...
task-mind chrome click 'button[aria-label="Show transcript"]'
task-mind chrome screenshot step2_transcript_opened.png
...
uv run task-mind run info youtube-subtitle-extraction-abc123
```

**Claude Code Method**:

```
/task-mind.run Research YouTube subtitle extraction methods: locate transcript button, test extraction, save working script
```
...
uv run task-mind run init "Find best way to extract Upwork job details"

# Interactive exploration
task-mind chrome navigate https://www.upwork.com/freelance-jobs/...
task-mind chrome screenshot job_page_layout.png
task-mind chrome exec-js 'document.querySelector(".job-title").textContent' --return-value
...
Using Claude Code:
```
/task-mind.recipe create "Extract Upwork job details as Markdown" from run upwork-job-extraction-abc123
```
...
4. Save to `~/.task-mind/recipes/atomic/chrome/upwork_extract_job_details_as_markdown.js`
...
CLI method:
```bash
uv run task-mind recipe run upwork_extract_job_details_as_markdown \
...
```
/task-mind.run Extract job details from https://www.upwork.com/freelance-jobs/... and save as Markdown
```

AI will:
1. Query available Recipes: `uv run task-mind recipe list --format json`
...
uv run task-mind recipe list --format table

# View Recipe details
uv run task-mind recipe info youtube_extract_video_transcript

# Execute Recipe
uv run task-mind recipe run youtube_extract_video_transcript \
...
```
/task-mind.run Extract subtitles from https://www.youtube.com/watch?v=dQw4w9WgXcQ
```

AI execution flow:
1. **Intent Recognition**: User wants to extract YouTube subtitles
2. **Recipe Discovery**: `uv run task-mind recipe list --format json`
...
4. **Execution**: `uv run task-mind recipe run youtube_extract_video_transcript ...`
...
```python
# ~/.task-mind/recipes/workflows/competitor_price_monitor.py
import sys, json
from task-mind.recipes import RecipeRunner
...
uv run task-mind run init "Monitor iPhone 15 competitor prices"

# Execute Workflow Recipe (operations auto-logged to Run)
uv run task-mind recipe run competitor_price_monitor \
  --params '{"product": "iPhone 15"}' \
  --output-file price_report.json

# Review complete workflow execution
uv run task-mind run info <run_id>
```

**Claude Code Method**:

```
/task-mind.run Monitor iPhone 15 prices on Amazon and eBay, generate comparison report and save as Markdown
```
...
```python
# ~/.task-mind/recipes/workflows/upwork_batch_extract.py
import sys, json
from task-mind.recipes import RecipeRunner
...
```bash
# CLI method
uv run task-mind recipe run upwork_batch_extract \
...
- **New to task-mind?** Start with [Installation Guide](installation.md)
