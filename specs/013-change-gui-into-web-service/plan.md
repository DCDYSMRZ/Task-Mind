...
**Storage**: N/A (uses existing session storage in `~/.task-mind/sessions/`)
...
- ✅ All changes maintain backward compatibility (`task-mind --gui` continues to work)
...
1. Create `src/task-mind/server/` module structure
2. Implement FastAPI application with static file serving
3. Add `task-mind serve` CLI command
...
1. Update `task-mind --gui` to use web service
