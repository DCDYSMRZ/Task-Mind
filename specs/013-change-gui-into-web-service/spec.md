...
**Created**: 2025-12-23
**Status**: Draft
**Input**: Replace pywebview-based GUI with a local HTTP web service, allowing users to control Task-Mind through any browser.
...
As a Task-Mind user, I want to start a local web server with `task-mind serve` so that I can access the GUI through my browser.

**Why this priority**: This is the foundation - without the server running, nothing else works.

**Independent Test**: Can be fully tested by running `task-mind serve` and verifying the browser opens with the GUI loaded.

**Acceptance Scenarios**:

1. **Given** user has task-mind installed, **When** user runs `task-mind serve`, **Then** server starts on `127.0.0.1:8080` and opens browser automatically
2. **Given** port 8080 is occupied, **When** user runs `task-mind serve`, **Then** server finds next available port and displays the URL
3. **Given** server is running, **When** user opens `http://127.0.0.1:8080` in any browser, **Then** the Task-Mind GUI loads correctly
...
As an existing user, I want `task-mind --gui` to still work, launching the web service and opening my default browser.

**Why this priority**: Smooth migration for existing users.

**Independent Test**: Can be tested by running `task-mind --gui` and verifying browser opens with GUI.

**Acceptance Scenarios**:

1. **Given** user runs `task-mind --gui`, **When** command executes, **Then** equivalent to `task-mind serve --open`
...
- **FR-015**: `task-mind serve` command MUST start the web server
- **FR-016**: `task-mind serve --port <port>` MUST allow custom port selection
- **FR-017**: `task-mind serve --open` MUST auto-open browser (default behavior)
- **FR-018**: `task-mind --gui` MUST behave as alias for `task-mind serve --open`
...
- **SC-005**: `task-mind --gui` continues to work for existing users
- **SC-006**: Users can access GUI from any modern browser (Chrome, Firefox, Edge, Safari)

## Migration Path

1. **v0.17.0**: Add `task-mind serve` command (web service mode available)
2. **v0.17.0**: `task-mind --gui` still uses pywebview (default unchanged)
3. **v0.18.0**: `task-mind --gui` uses web service by default
4. **v0.18.0**: `task-mind --gui-native` for legacy pywebview
