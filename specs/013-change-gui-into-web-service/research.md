...
1. **`FragoGuiApi` class** (`src/task-mind/gui/api.py`):
...
3. **Frontend** (`src/task-mind/gui/frontend/`):
...
1. **Server module** (`src/task-mind/server/`):
   - `app.py`: FastAPI application with routes
   - `websocket.py`: WebSocket connection manager

2. **CLI command** (`task-mind serve`):
   - New subcommand in `src/task-mind/cli/main.py`
