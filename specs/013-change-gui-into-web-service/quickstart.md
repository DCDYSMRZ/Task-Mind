...
- Existing Task-Mind installation
...
3. Display the Task-Mind GUI

### 2. Custom Port

```bash
task-mind serve --port 3000
```

### 3. Without Auto-Open

```bash
task-mind serve --no-open
```
...
```bash
task-mind --gui
# Equivalent to: task-mind serve --open
```
...
New module: `src/task-mind/server/`

```
src/task-mind/server/
├── __init__.py
...
Update API client: `src/task-mind/gui/frontend/src/api/`
...
- `task-mind --gui` continues to work exactly as before
- New option: `task-mind serve` for explicit web server mode
...
- Ensure the server is running (`task-mind serve`)
