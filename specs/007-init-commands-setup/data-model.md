# Data Model: task-mind init 命令与 Recipe 资源安装
...
| `recipe` | 示例 recipe | `~/.task-mind/recipes/` |
...
| 字段 | 类型 | 描述 |
|------|------|------|
| `name` | string | 文件名（如 `task-mind.run.md`） |
...
| `task_mind_version` | string | task-mind 版本号 |
...
扩展现有 `Config` 模型（`src/task-mind/init/models.py`）:
