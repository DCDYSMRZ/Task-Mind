# Research: task-mind init 命令与 Recipe 资源安装

## 研究问题

1. Python 包如何打包非代码资源文件？
2. 现有 task-mind 资源文件有哪些？
3. 如何区分系统资源与用户自定义内容？
...
**运行时资源访问** (`src/task-mind/init/resources.py`):
...
### 选择: 在 `src/task-mind/resources/` 下创建资源目录
...
| 放在 `src/task-mind/` 根目录 | 简单 | 与代码混在一起 |
...
| 资源类型 | 安装位置 |
|----------|----------|
| slash 命令 | `~/.claude/commands/task-mind.*.md` |
| skills | `~/.claude/skills/task-mind-*/` |
| 示例 recipe | `~/.task-mind/recipes/` |

### 理由

- `~/.claude/commands/` 是 Claude Code 标准的用户级命令目录
- `~/.claude/skills/` 是 Claude Code 标准的用户级 skills 目录
- `~/.task-mind/recipes/` 是 task-mind 项目约定的用户级 recipe 目录
...
| 文件类型 | 策略 |
|----------|------|
| slash 命令 (`task-mind.*.md`) | 始终覆盖（系统文件） |
| skills (`task-mind-*/`) | 始终覆盖（系统文件） |
...
def install_commands(source_dir: Path, target_dir: Path) -> list[str]:
    ...
    for file in source_dir.glob("task-mind.*.md"):
...
- 系统 slash 命令使用 `task-mind.` 前缀，易于识别
...
def is_system_command(filename: str) -> bool:
    """判断是否为系统 slash 命令"""
    return filename.startswith("task-mind.") and filename.endswith(".md")
...
| 文件名 | 大小 | 功能描述 |
|--------|------|----------|
| `task-mind.run.md` | 17.5KB | Run 命令系统，管理 AI 主持的自动化任务 |
| `task-mind.recipe.md` | 19.0KB | Recipe 创建和管理 |
| `task-mind.exec.md` | 12.8KB | 一次性复杂任务执行 |
| `task-mind.test.md` | 4.1KB | Recipe 测试验证 |

### skills（考虑打包）

| 目录名 | 功能描述 |
|--------|----------|
| `task-mind-browser-automation/` | 浏览器自动化基础 skill |
...
1. 创建 `src/task-mind/resources/` 目录结构
2. 将现有资源复制到该目录
3. 更新 pyproject.toml 配置
4. 实现 `src/task-mind/init/resources.py` 模块
5. 扩展 `task-mind init` 命令调用资源安装
