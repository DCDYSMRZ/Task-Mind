...
1. **Chrome浏览器已连接CDP**
   ```bash
   # 检查CDP连接状态
   uv run task-mind exec-js "console.log('CDP connected')"
   ```

2. **Python环境已配置**
   ```bash
   # 验证Frago安装
   uv run task-mind --help
   ```
...
```bash
/task-mind.recipe "提取YouTube视频字幕内容"
```
...
Claude Code会执行CDP命令：
```bash
uv run task-mind click '[aria-label="作者声明"]'
```
...
Claude Code执行：
```bash
uv run task-mind click '[aria-label="内容转文字"]'
```
...
Claude Code执行：
```bash
uv run task-mind exec-js "document.querySelector('.subtitle-text').innerText"
```
...
自动生成：
- `src/task-mind/recipes/youtube_extract_transcript.js`（可执行脚本）
- `src/task-mind/recipes/youtube_extract_transcript.md`（知识文档）
...
2. 执行配方脚本：
   ```bash
   uv run task-mind exec-js recipes/youtube_extract_transcript.js
   ```
...
```bash
/task-mind.recipe update youtube_extract_transcript "字幕按钮选择器失效了"
```
...
**无数据库**：所有配方和文档存储在 `src/task-mind/recipes/` 扁平目录
...
✅ **Prompt模板**（`.claude/commands/task_mind_recipe.md`）：完整的指令集
✅ **配方库目录**（`src/task-mind/recipes/`）：存放生成的.js和.md文件
✅ **配方执行器**：通过 `uv run task-mind exec-js` 执行配方
...
**A**: 使用 `/task-mind.recipe update <配方名>` 重新探索页面，系统会覆盖原文件生成更新版本，并在文档中记录更新原因。
...
**下一步**：阅读 [tasks.md](./tasks.md) 了解具体实施任务，或直接运行 `/task-mind.recipe` 体验配方创建流程（Phase 3完成后）。
