...
本文档研究基于简化架构的技能自动化生成系统的三个核心技术问题。核心思路是利用Claude Code本身作为智能探索引擎，通过prompt模板引导使用task-mind的CDP原子命令，从对话历史中提取关键信息生成JavaScript配方脚本，**无需构建独立的探索引擎类（RecipeExplorer）或复杂的状态管理（ExplorationSession）**。
...
如何设计 `.claude/commands/task_mind_recipe.md` 来引导Claude Code：
...
- 检查是否存在同名配方（使用Glob工具查看src/task-mind/recipes/目录）

## 阶段2: 交互式探索
- 确认Chrome已连接CDP（端口9222）
- 根据用户描述导航到目标页面
- 逐步执行用户描述的操作：
  - 使用 `uv run task-mind screenshot` 展示当前页面
  - 使用 `uv run task-mind click <selector>` 点击元素
  - 使用 `uv run task-mind exec-js <expression>` 提取内容
- **重要**：在对话中明确记录每一步使用的选择器和操作结果
- 探索失败时：截图 + 询问用户 + 提供候选元素（最多3次交互）

## 阶段3: 生成配方脚本
- 从对话历史提取操作序列（回顾阶段2的对话记录）
- 使用src/task-mind/recipe/templates.py的JavaScriptTemplate生成代码
- 包含降级逻辑（基于src/task-mind/recipe/selector.py的策略）
- 保存到 `src/task-mind/recipes/<配方名>.js`

## 阶段4: 生成知识文档
- 使用src/task-mind/recipe/templates.py的MarkdownTemplate生成6章节文档
- 从对话历史提取前置条件和操作步骤
- 标注脆弱的选择器（is_fragile=True）
- 保存到 `src/task-mind/recipes/<配方名>.md`

## 阶段5: 验证配方
- 在当前页面执行生成的配方：`uv run task-mind exec-js recipes/<配方名>.js`
- 检查输出是否符合预期
- 如有问题，询问用户是否需要调整
```
...
1. **明确的工具调用指令**：直接告诉Claude Code使用哪个CDP命令
   ```
   使用Bash工具执行：uv run task-mind screenshot /tmp/page.png
   使用Bash工具执行：uv run task-mind click '[aria-label="字幕"]'
   ```
...
3. **失败处理模板**：提供标准化的失败处理流程
   ```
   如果元素定位失败：
   1. 使用Bash执行：uv run task-mind screenshot /tmp/failed_step.png
...
4. **代码生成触发器**：明确何时开始生成代码
   ...
   1. 回顾对话历史，提取所有CDP命令（grep对话中的 "uv run task-mind" 命令）
   2. 提取选择器和上下文（查找 "使用的选择器：" 标记）
   3. 读取 src/task-mind/recipe/templates.py 了解代码生成模板
   4. 读取 src/task-mind/recipe/selector.py 了解选择器优先级逻辑
   5. 生成JavaScript脚本并保存
   6. 生成Markdown文档并保存
   ```
...
class ConversationHistoryParser:
    ...
    def extract_cdp_commands(conversation_text: str) -> List[Dict[str, str]]:
        ...
        识别模式：
        - "uv run task-mind click <selector>"
        - "uv run task-mind exec-js <expression>"
        - "uv run task-mind screenshot <path>"
        - "uv run task-mind navigate <url>"
        """
        patterns = {
            'click': r'uv run task-mind click\s+[\'"]?(.+?)[\'"]?(?:\s|$)',
            'exec-js': r'uv run task-mind exec-js\s+[\'"]?(.+?)[\'"]?(?:\s|$)',
            'screenshot': r'uv run task-mind screenshot\s+(.+?)(?:\s|$)',
            'navigate': r'uv run task-mind navigate\s+(.+?)(?:\s|$)',
        }
    ...
    def extract_selectors_with_context(conversation_text: str) -> List[Tuple[str, str]]:
        ...
        - "元素描述：<description>"
        - "点击<描述>: uv run task-mind click <selector>"
        ...
        # 模式2: 命令前的描述
        pattern2 = r'(?:点击|提取|定位)(.+?)[:：]\s*uv run task-mind \w+\s+[\'"]?(.+?)[\'"]?'
    ...
    def extract_user_description(conversation_text: str) -> str:
        ...
        识别第一条包含 "/task-mind.recipe" 的消息
        """
        pattern = r'/task-mind\.recipe\s+(?:create\s+)?[\'"]?(.+?)[\'"]?(?:\s|$)'
...
3. **选择器优先级**：使用selector.py的已有逻辑自动评估优先级
   ```python
   from task-mind.recipe.selector import create_selector_from_string
...
class RecipeGenerator:
    ...
    def generate_recipe_from_conversation(
        ...
        from task-mind.recipe.selector import create_selector_from_string, sort_selectors_by_priority
        ...
        # 3. 生成操作步骤代码
        from task-mind.recipe.selector import generate_fallback_logic
        from task-mind.recipe.templates import JavaScriptTemplate
        ...
        # 6. 生成知识文档
        from task-mind.recipe.templates import MarkdownTemplate
