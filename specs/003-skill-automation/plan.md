...
**Input**: Feature specification from `/Users/chagee/Repos/Task-Mind/specs/003-skill-automation/spec.md`

**Note**: 本计划基于2025-11-19的架构简化调整，删除了过度设计的探索引擎，采用Claude Code原生对话能力。

## Summary

通过 `/task-mind.recipe` 命令进入对话流程，用户逐步描述操作步骤，Claude Code（被prompt模板引导）使用task-mind的CDP原子命令实际执行每一步，记录操作历史和关键选择器，最后从对话历史中提取信息生成可复用的JavaScript配方脚本和配套知识文档。核心创新在于利用Claude Code本身作为智能探索引擎，无需构建复杂的状态管理系统。
...
- click（CLI框架，已有）
- pydantic（数据验证，已有）
- 现有CDP命令模块（src/task-mind/cdp/commands/）

**Storage**: 文件系统存储
- 配方脚本：`src/task-mind/recipes/*.js`
- 知识文档：`src/task-mind/recipes/*.md`
- 无需数据库或复杂状态存储
...
└── task_mind_recipe.md          # 【需创建】/task-mind.recipe prompt模板
```

**Structure Decision**:
- 采用**单项目结构**（Option 1），扩展现有Frago项目
- **完全删除 `src/task-mind/recipe/` Python模块**：不需要Python代码来生成代码
- 保留 `src/task-mind/recipes/` 扁平目录存储Claude Code生成的配方和文档
- 复用现有CDP模块，无需重复实现
- **核心组件**：`.claude/commands/task_mind_recipe.md` prompt模板教会Claude Code如何创建配方
