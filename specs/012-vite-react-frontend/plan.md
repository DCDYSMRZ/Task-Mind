...
将 Task-Mind GUI 前端从原生 JavaScript 迁移到 Vite + React + TypeScript + TailwindCSS 现代化技术栈，提升开发体验和代码可维护性。同时实现 Linux 系统依赖自动检测和安装功能，降低用户的安装门槛。
...
**Structure Decision**: 采用嵌入式 Web 前端结构。前端源码位于 `src/task-mind/gui/frontend/`，构建产物输出到 `src/task-mind/gui/assets/`，旧版代码移动到 `src/task-mind/gui/assets_legacy/` 并标记为 deprecated。
