...
**Created**: 2025-12-09
**Status**: Draft
**Input**: 将 Task-Mind GUI 前端从原生 JS 迁移到 Vite + React + TypeScript + TailwindCSS，并实现 Linux 系统依赖自动安装

## User Scenarios & Testing *(mandatory)*

### User Story 1 - 开发者体验改进 (Priority: P1)

作为 Task-Mind 开发者，希望前端代码采用现代化技术栈，以便获得更好的开发体验和代码可维护性。
...
作为 Task-Mind 用户，希望迁移后的 GUI 保持所有现有功能正常工作，不因技术栈变更而丧失任何能力。
...
1. **Given** 用户启动 Task-Mind GUI, **When** 点击 Tasks 标签, **Then** 显示任务列表，包含任务状态颜色指示
...
作为 Linux 用户，希望首次运行 `task-mind gui` 时系统能自动检测并安装缺失的 GUI 依赖，无需手动查找和安装包。

用户在新安装的 Linux 系统上运行 `task-mind gui` 命令，系统检测到缺少 WebKit/GTK 依赖后，询问是否自动安装。用户确认后，系统通过图形化密码对话框获取权限并完成安装，随后 GUI 自动启动。

**Why this priority**: 降低 Linux 用户的安装门槛，但需要前端迁移完成后才有意义

**Independent Test**: 可在缺少 GUI 依赖的 Linux 环境中运行 `task-mind gui` 命令，观察自动安装流程

**Acceptance Scenarios**:

1. **Given** Linux 系统缺少 WebKit 依赖, **When** 用户运行 `task-mind gui`, **Then** 系统提示"检测到缺少 GUI 系统依赖，是否自动安装？"
...
不同发行版的用户（Ubuntu、Fedora、Arch、openSUSE 等）运行 `task-mind gui` 时，系统能识别发行版并使用正确的包管理器和包名。对于不支持的发行版，提供通用的依赖说明。
...
3. **Given** 用户使用不支持的发行版, **When** 运行 `task-mind gui`, **Then** 系统显示"无法识别您的发行版，请手动安装以下依赖：..."
...
- 用户在没有图形界面的纯终端环境运行 `task-mind gui` 会发生什么？pkexec 无法工作时应回退到命令行提示
