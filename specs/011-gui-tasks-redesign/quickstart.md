# Quickstart: Task-Mind GUI Tasks Redesign
...
```bash
# 确保在项目根目录
cd /Users/chagee/Repos/task-mind # 确保虚拟环境激活
uv sync

# 检查 GUI 依赖
uv run task-mind gui-deps --check
```
...
```bash
# 启动 GUI（开发模式）
uv run task-mind gui --debug

# 或通过 --gui 标志
uv run task-mind --gui
```
...
3. **点击 Tasks** - 显示任务列表（如果有运行过的 task-mind agent）
...
| 文件 | 修改类型 | 说明 |
|-----|---------|------|
| `src/task-mind/gui/api.py` | 扩展 | 新增 `get_tasks()`, `get_task_detail()`, `get_task_steps()`, `subscribe_task_updates()` |
| `src/task-mind/gui/models.py` | 扩展 | 新增 `TaskItem`, `TaskDetail`, `TaskStep`, `TaskSummary`, `TaskStatus` |
| `src/task-mind/session/storage.py` | 扩展 | 新增 `read_steps_paginated()`, `count_sessions()` |

### 前端文件

| 文件 | 修改类型 | 说明 |
|-----|---------|------|
| `src/task-mind/gui/assets/index.html` | 重构 | 新增 Tips/Tasks/TaskDetail 页面结构，更新导航 |
| `src/task-mind/gui/assets/scripts/app.js` | 扩展 | 新增页面交互：`loadTasks()`, `openTaskDetail()`, `loadMoreSteps()` |
| `src/task-mind/gui/assets/styles/main.css` | 扩展 | 任务状态颜色样式，新页面布局 |
...
def get_tasks(self, limit: int = 50, offset: int = 0) -> Dict[str, Any]:
    ...
    # 获取会话列表
    from task-mind.session.storage import list_sessions, count_sessions
...
```python
# src/task-mind/session/storage.py
...
```html
<!-- src/task-mind/gui/assets/index.html -->
...
```javascript
// src/task-mind/gui/assets/scripts/app.js
...
```css
/* src/task-mind/gui/assets/styles/main.css */
...
uv run task-mind agent "帮我查看项目结构"
...
```bash
# 查看会话目录
ls -la ~/.task-mind/sessions/claude/

# 查看单个会话元数据
cat ~/.task-mind/sessions/claude/<session_id>/metadata.json | jq

# 查看步骤日志
head -20 ~/.task-mind/sessions/claude/<session_id>/steps.jsonl
```
...
**A**: 检查会话目录是否存在：
```bash
ls ~/.task-mind/sessions/claude/
```
如果为空，说明还没有运行过 `task-mind agent` 命令。
