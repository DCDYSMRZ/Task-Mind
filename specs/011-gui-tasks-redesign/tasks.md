# Tasks: Task-Mind GUI Tasks Redesign
...
- **Single project**: `src/task-mind/` at repository root
- **GUI Frontend**: `src/task-mind/gui/assets/`
...
- [X] T003 [P] 确认现有 session 模块可用（`src/task-mind/session/`）
...
- [X] T004 [P] 添加 `TaskStatus` 枚举到 `src/task-mind/gui/models.py`
  - 定义: RUNNING (黄), COMPLETED (绿), ERROR (红), CANCELLED (红)
  - 实现: `color`, `icon`, `label` 属性方法
  - 参考: data-model.md 1.2 节

- [X] T005 [P] 添加 `TaskItem` 模型到 `src/task-mind/gui/models.py`
...
- [X] T006 [P] 添加 `TaskStep` 模型到 `src/task-mind/gui/models.py`
  - 字段: step_id, type, timestamp, content, tool_name, tool_status
  - 方法: `from_session_step(SessionStep)` 类方法
  - 参考: data-model.md 1.4 节

- [X] T007 添加 `TaskSummary` 和 `ToolUsageStat` 模型到 `src/task-mind/gui/models.py`
...
- [X] T008 添加 `TaskDetail` 模型到 `src/task-mind/gui/models.py`（依赖 T005-T007）
...
- [X] T009 扩展 `PageType` 枚举在 `src/task-mind/gui/models.py`
  - 新增: TIPS, TASKS, TASK_DETAIL
  - 修改默认值: TIPS（原 home）
  - 参考: data-model.md 2.1 节

- [X] T010 扩展 `AppState` 模型在 `src/task-mind/gui/models.py`（依赖 T009）
...
- [X] T011 [P] 添加 `read_steps_paginated()` 函数到 `src/task-mind/session/storage.py`
...
- [X] T012 [P] 添加 `count_sessions()` 函数到 `src/task-mind/session/storage.py`
...
**Independent Test**: 启动 `uv run task-mind gui --debug`，验证默认页面为 Tips

**Success Criteria**: SC-001 (GUI 启动到显示 Tips 页面 ≤3 秒)

### Implementation for User Story 1

- [X] T013 [US1] 添加 Tips 页面 HTML 结构到 `src/task-mind/gui/assets/index.html`
  - 创建 `<section id="page-tips" class="page active">`
  - 添加空状态提示（图标 + 标题 + 描述）
  - 参考: quickstart.md 2.1 节

- [X] T014 [US1] 添加 Tips 页面样式到 `src/task-mind/gui/assets/styles/main.css`
  - 空状态组件样式: `.empty-state`, `.empty-state__icon`, `.empty-state__title`, `.empty-state__description`
  - 参考: quickstart.md 2.3 节

- [X] T015 [US1] 修改默认页面加载逻辑在 `src/task-mind/gui/assets/scripts/app.js`
  - 修改 `initApp()` 或类似初始化函数
  - 设置默认激活页面为 `page-tips`
  - 参考: research.md 页面结构建议

- [X] T016 [US1] 更新导航菜单在 `src/task-mind/gui/assets/index.html`
  - 将"主页"改为"Tasks"
  - 添加"Tips"导航项（第一个位置）
  - 更新导航项的 active 状态逻辑
...
**Independent Test**: 运行 `uv run task-mind agent "test"` 后，在 GUI Tasks 页面看到该任务及其状态
...
- [X] T017 [US2] 添加 `get_tasks()` API 方法到 `src/task-mind/gui/api.py`
...
- [X] T018 [US2] 添加 Tasks 页面 HTML 结构到 `src/task-mind/gui/assets/index.html`
...
- [X] T019 [P] [US2] 添加任务状态颜色样式到 `src/task-mind/gui/assets/styles/main.css`
...
- [X] T020 [P] [US2] 添加任务卡片样式到 `src/task-mind/gui/assets/styles/main.css`
  - `.task-card`, `.task-card:hover`
  - `.task-card__header`, `.task-card__name`, `.task-card__time`, `.task-card__stats`
  - 参考: quickstart.md 2.3 节

- [X] T021 [US2] 实现 `loadTasks()` 函数在 `src/task-mind/gui/assets/scripts/app.js`
  - 调用 `pywebview.api.get_tasks()`
  - 渲染任务卡片列表
  - 处理空状态显示
  - 参考: quickstart.md 2.2 节

- [X] T022 [US2] 实现 `refreshTasks()` 函数在 `src/task-mind/gui/assets/scripts/app.js`
  - 刷新按钮点击处理
  - 重新加载任务列表

- [X] T023 [US2] 实现辅助函数在 `src/task-mind/gui/assets/scripts/app.js`
...
- [X] T024 [US3] 添加 `get_task_detail()` API 方法到 `src/task-mind/gui/api.py`
  - 参数: session_id
...
- [X] T025 [US3] 添加 `get_task_steps()` API 方法到 `src/task-mind/gui/api.py`
  - 参数: session_id, limit=50, offset=0
  - 返回: TaskStepsResponse 格式
  - 调用: `read_steps_paginated()`
  - 参考: contracts/gui-api.md 3 节

- [X] T026 [US3] 添加任务详情页 HTML 结构到 `src/task-mind/gui/assets/index.html`
  - 创建 `<section id="page-task-detail" class="page">`
  - 添加页面头（返回按钮 + 标题）
  - 添加详情内容容器 `#task-detail-content`
  - 参考: quickstart.md 2.1 节

- [X] T027 [P] [US3] 添加任务详情页样式到 `src/task-mind/gui/assets/styles/main.css`
  - `.task-detail__info`, `.task-detail__status`, `.task-detail__meta`, `.task-detail__stats`
  - 参考: quickstart.md 2.3 节

- [X] T028 [P] [US3] 添加步骤列表样式到 `src/task-mind/gui/assets/styles/main.css`
...
- [X] T029 [US3] 实现 `openTaskDetail(sessionId)` 函数在 `src/task-mind/gui/assets/scripts/app.js`
  - 保存 Tasks 页面滚动位置
  - 设置 currentTaskId
  - 切换到详情页
...
- [X] T030 [US3] 实现 `renderTaskDetail(task)` 函数在 `src/task-mind/gui/assets/scripts/app.js`
  - 渲染任务基本信息
  - 渲染统计信息
  - 渲染步骤列表
...
- [X] T031 [US3] 实现 `renderSteps(steps)` 函数在 `src/task-mind/gui/assets/scripts/app.js`
  - 渲染步骤列表 HTML
  - 区分步骤类型样式
  - 参考: quickstart.md 2.2 节

- [X] T032 [US3] 实现 `loadMoreSteps()` 函数在 `src/task-mind/gui/assets/scripts/app.js`
  - 调用 `pywebview.api.get_task_steps()` 分页加载
  - 追加到现有步骤列表
  - 更新"加载更多"按钮状态

- [X] T033 [US3] 实现 `backToTasks()` 函数在 `src/task-mind/gui/assets/scripts/app.js`
  - 清除 currentTaskId
  - 切换回 Tasks 页面
  - 恢复滚动位置
  - 参考: quickstart.md 2.2 节

- [X] T034 [US3] 实现 `getStepTypeLabel(type)` 辅助函数在 `src/task-mind/gui/assets/scripts/app.js`
  - 返回步骤类型的中文标签
...
- [X] T035 [US4] 扩展 `switchPage(pageType)` 函数在 `src/task-mind/gui/assets/scripts/app.js`
  - 支持新页面类型: tips, tasks, task_detail
  - 更新导航菜单 active 状态
  - 处理页面特定的进入/离开逻辑
  - 参考: data-model.md 5.2 节

- [X] T036 [US4] 更新导航菜单样式在 `src/task-mind/gui/assets/styles/main.css`
  - 确保导航项 active 状态清晰
...
- [X] T038 [P] 添加轮询机制到 `src/task-mind/gui/assets/scripts/app.js`
...
- [X] T042 添加过渡动画到 `src/task-mind/gui/assets/styles/main.css`
...
- [X] T047 优化空状态显示
  ...
  - 详情页加载失败时的错误提示
  - 空状态带有 `task-mind agent` 使用提示

- [X] T048 [P] 验证代码可正常导入和实例化
  - API 实例创建成功
  - 所有模型导入成功
  - 启动命令 `uv run task-mind gui --debug`
