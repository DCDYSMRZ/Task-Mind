# Research: Task-Mind GUI Tasks Redesign
...
**会话存储位置**：`~/.task-mind/sessions/{agent_type}/{session_id}/`
```
├── metadata.json   # 会话元数据
...
**检测逻辑**：
1. 定期扫描 `~/.task-mind/sessions/claude/` 目录（5s 间隔）
...
**未来扩展**：
- 可通过 `~/.task-mind/tips.json` 或远程 API 加载内容
- 支持 Markdown 渲染
- 版本更新时显示新功能亮点
