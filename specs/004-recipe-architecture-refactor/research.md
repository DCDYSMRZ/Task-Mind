...
```bash
# stdout（默认）
$ uv run task-mind recipe run youtube_extract_transcript --params '{...}'

# 文件
$ uv run task-mind recipe run youtube_extract_transcript --params '{...}' --output-file transcript.json

# 剪贴板
$ uv run task-mind recipe run youtube_extract_transcript --params '{...}' --output-clipboard
```
...
### 决策
扩展 `/task-mind.recipe` 命令，支持 `create workflow` 子命令，AI 根据自然语言生成 Python Workflow 脚本。
...
### 命令示例
```bash
# AI 调用 /task-mind.recipe
/task-mind.recipe create workflow "批量提取 10 个 Upwork 职位并保存为 CSV"
```
...
↓
AI 思考过程:
1. 调用: uv run task-mind recipe list --format json
...
4. 执行: uv run task-mind recipe run youtube_extract_video_transcript \
           --params '{"url": "..."}' \
...
### 决策
保留现有 `uv run task-mind exec-js` 命令，新系统作为独立功能。
...
```bash
# 旧方式（继续支持）
uv run task-mind exec-js recipes/upwork_extract_job.js

# 新方式（推荐，AI 使用）
uv run task-mind recipe run upwork_extract_job --params '{...}'
```
...
def test_ai_discovers_and_runs_recipe():
    ...
    # 3. AI 执行 Recipe
    result = subprocess.run(
        ['uv', 'run', 'task-mind', 'recipe', 'run', target_recipe['name'],
         '--params', '{"url": "..."}'],
        capture_output=True, text=True
    )
