...
**查找路径优先级**:
1. 项目级: `.task-mind/recipes/` （当前工作目录）
2. 用户级: `~/.task-mind/recipes/` （用户家目录）
...
#### ChromeJSExecutor
- 调用命令: `uv run task-mind exec-js <script> <params_json>`
...
```bash
# stdout（默认）
uv run task-mind recipe run <name> --params '{...}'

# file
uv run task-mind recipe run <name> --params '{...}' --output-file result.json

# clipboard
uv run task-mind recipe run <name> --params '{...}' --output-clipboard
```
...
└── recipes: dict[str, Recipe]
        ...
        │   ├── script_path: /path/to/.task-mind/recipes/workflows/custom_workflow.py
        │   ├── metadata_path: /path/to/.task-mind/recipes/workflows/custom_workflow.md
        │   └── source: "Project"
        │
        ├─> Recipe (来源: User)
        │   ├── metadata: RecipeMetadata
        ...
        │   │   ├── type: "atomic"
        │   │   └── runtime: "chrome-js"
        │   ├── script_path: ~/.task-mind/recipes/atomic/chrome/upwork_extract_job.js
        │   └── source: "User"
        │
        └─> Recipe (来源: Example)
            ├── metadata: RecipeMetadata
...
执行流程（AI-first 视角）:
AI Agent → Bash tool: uv run task-mind recipe list --format json
         → RecipeRegistry.list_all() → JSON 元数据数组
         → AI 分析 description, use_cases, output_targets

AI Agent → Bash tool: uv run task-mind recipe run <name> --params '{...}' [--output-file/--output-clipboard]
...
**示例**:
```text
~/.task-mind/recipes/atomic/chrome/
