...
- Q: AI Agent 调用 Recipe 的主要接口方式？ → A: Bash 工具直接调用（`uv run task-mind recipe ...`），但要求 Recipe 元数据（YAML）中明确声明输出形态，让 AI 能预期产出
- Q: Workflow Recipe 的创建方式？ → A: AI 通过 `/task-mind.recipe` 命令自动生成 Workflow，用户仅需描述意图，AI 生成包含循环、条件、错误处理的完整脚本
...
1. 根据用户意图自动生成原子 Recipe 或编排 Workflow（通过 `/task-mind.recipe` 命令）
2. 通过 `recipe list` 发现现有 Recipe 的功能和输出形态
3. 使用 Bash 工具调用 Recipe（`uv run task-mind recipe run`），并明确预期输出去向（stdout/file/clipboard）
4. 规划任务执行路径时，组合使用多个 Recipe 完成复杂任务

**Why this priority**: 这是 Task-Mind 的核心差异化价值 - AI 驱动的自动化，而非人类手动操作工具。Recipe 系统必须设计为"AI 可理解、可调度的工具集"。

**Independent Test**: AI 接收任务"批量提取 10 个 Upwork 职位信息并保存为 Markdown 文件"，自动完成：1) 通过 `recipe list` 发现 `upwork_extract_job` Recipe；2) 判断需要编排 Workflow；3) 通过 `/task-mind.recipe` 生成批量处理 Workflow；4) 执行 Workflow 并输出到文件。

**Acceptance Scenarios**:

1. **Given** AI 收到任务"提取 YouTube 视频字幕"，**When** AI 执行 `uv run task-mind recipe list --format json`，**Then** 返回包含 `youtube_extract_video_transcript` Recipe 的 JSON，明确标注 `description`, `use_cases`, `output_targets`
2. **Given** AI 发现 `youtube_extract_video_transcript` Recipe 支持 `output_targets: [stdout, file]`，**When** AI 需要将结果保存为文件，**Then** 执行 `uv run task-mind recipe run youtube_extract_video_transcript --params '{...}' --output-file transcript.txt`
3. **Given** AI 需要批量处理多个视频，**When** AI 调用 `/task-mind.recipe create workflow "批量提取 YouTube 字幕"`，**Then** AI 自动生成包含循环、错误处理的 Workflow Recipe 并保存到用户目录
...
作为 Task-Mind 用户（人类或 AI），我希望能用不同的编程语言（JavaScript、Python、Shell）创建 Recipe，系统能统一调用和管理这些 Recipe，无需关心底层实现语言。
...
1. **Given** 用户编写了一个 JavaScript Recipe（`chrome/get_title.js`）并配置了元数据文件（`.md`），**When** 用户执行 `uv run task-mind recipe run get_title`，**Then** Recipe 成功执行并返回 JSON 格式的网页标题
2. **Given** 用户编写了一个 Python Recipe（`system/clipboard_read.py`），**When** 用户执行 `uv run task-mind recipe run clipboard_read`，**Then** Recipe 成功执行并返回 JSON 格式的剪贴板内容
3. **Given** 用户编写了一个 Shell Recipe（`system/file_copy.sh`），**When** 用户执行 `uv run task-mind recipe run file_copy --params '{"src": "/tmp/a", "dst": "/tmp/b"}'`，**Then** Recipe 成功执行并返回操作结果
...
作为 Task-Mind 开发者，我希望将 Task-Mind Python 包代码与用户的 Recipe 资源完全分离，使得用户可以在自己的目录管理 Recipe，而不影响包的升级和分发。
...
1. **Given** Task-Mind 已安装，**When** 用户首次运行 `uv run task-mind init`，**Then** 系统在 `~/.task-mind/recipes/` 创建目录结构（`atomic/chrome/`, `atomic/system/`, `workflows/`）
2. **Given** 用户在 `~/.task-mind/recipes/atomic/chrome/` 创建了 Recipe，**When** 执行 `uv run task-mind recipe list`，**Then** 列表显示该 Recipe 并标注为 `[User]` 来源
3. **Given** Task-Mind 包含官方示例 Recipe（位于 `examples/atomic/chrome/`），**When** 用户执行 `uv run task-mind recipe list`，**Then** 同时显示用户 Recipe 和示例 Recipe，清晰区分来源（`[User]` / `[Example]`）
4. **Given** 用户想复用官方示例，**When** 执行 `uv run task-mind recipe copy upwork_extract_job`，**Then** 示例 Recipe 被复制到 `~/.task-mind/recipes/atomic/chrome/` 供用户修改

---

### User Story 3 - 编排多个 Recipe 的工作流（Priority: P2）

作为 Task-Mind 用户，我希望能创建一个编排 Recipe（Workflow），在其中调用多个原子 Recipe，实现复杂的自动化流程（如批量提取多个网页、混合操作浏览器和系统应用）。
...
作为 Task-Mind 用户，我希望每个 Recipe 都有清晰的元数据（输入参数、输出格式、运行时类型、版本号），系统能自动校验和提示，避免错误调用。
...
3. **Given** 用户想查看某个 Recipe 的详细信息，**When** 执行 `uv run task-mind recipe info upwork_extract_job`，**Then** 显示名称、类型、运行时、输入输出定义、版本号、文档说明
...
作为在特定项目中使用 Task-Mind 的用户，我希望能在项目目录下创建项目专属的 Recipe（位于 `.task-mind/recipes/`），与全局 Recipe 隔离，避免污染用户级配置。
...
1. **Given** 用户在项目根目录创建 `.task-mind/recipes/workflows/custom.py`，**When** 在项目目录执行 `uv run task-mind recipe run custom`，**Then** 系统优先使用项目级 Recipe
...
- 用户修改了示例 Recipe（位于 `examples/`），升级 Task-Mind 后修改是否会丢失？
...
- **FR-001**: 系统必须支持三种 Recipe 运行时：`chrome-js`（通过 `uv run task-mind exec-js` 执行）、`python`（通过 Python 解释器执行）、`shell`（通过 Shell 执行）
...
- **FR-005**: 系统必须支持三级 Recipe 查找路径：项目级（`.task-mind/recipes/`）> 用户级（`~/.task-mind/recipes/`）> 示例级（`examples/`）
- **FR-006**: 系统必须在用户首次运行 `uv run task-mind init` 时创建用户级 Recipe 目录结构（`~/.task-mind/recipes/atomic/chrome/`, `atomic/system/`, `workflows/`）
- **FR-007**: 系统必须提供 `uv run task-mind recipe list` 命令，支持 `--format json` 输出结构化数据，包含每个 Recipe 的 `name`, `type`, `description`, `use_cases`, `tags`, `output_targets` 字段，便于 AI 解析和选择
- **FR-008**: 系统必须提供 `uv run task-mind recipe info <name>` 命令，显示指定 Recipe 的完整元数据（包括 AI 可理解的语义字段）
- **FR-009**: 系统必须提供 `uv run task-mind recipe run <name>` 命令，支持以下输出选项：`--output-file <path>`（保存到文件）、`--output-clipboard`（复制到剪贴板）、默认输出到 stdout
- **FR-010**: 系统必须提供 `uv run task-mind recipe copy <name>` 命令，将示例 Recipe 复制到用户级目录供修改
...
- **FR-020**: `/task-mind.recipe` 命令必须支持生成 Workflow Recipe，接受自然语言描述（如 `/task-mind.recipe create workflow "批量提取 10 个 Upwork 职位"`），AI 自动生成包含循环、错误处理的 Python 脚本
...
- **FR-013**: 系统必须将现有的 `src/task-mind/recipes/` 目录内容迁移到 `examples/atomic/chrome/`，并在 `src/task-mind/recipes/` 创建引擎代码（`runner.py`, `registry.py`）
...
- **SC-002**: AI Agent 通过 `/task-mind.recipe` 命令生成的 Workflow Recipe 能 100% 成功执行（包含正确的循环、错误处理、Recipe 调用）
...
- **SC-008**: Task-Mind 升级后，用户的自定义 Recipe（位于 `~/.task-mind/recipes/`）100% 保持可用（无数据丢失）
- **SC-009**: 代码与资源完全分离后，Task-Mind Python 包体积减少（示例 Recipe 不打包），安装速度提升 20%
...
- 用户有权限创建 `~/.task-mind/` 目录（家目录写权限）
- Recipe 脚本文件具有可执行权限（Shell 和 Python Recipe）
- 项目级 Recipe 位于项目根目录下的 `.task-mind/recipes/`（通过当前工作目录判断）

### AI Agent 假设

- **主要使用者是 Claude Code AI Agent**，通过 Bash 工具调用 `uv run task-mind` 命令
- AI Agent 能理解 JSON 格式的元数据和输出，并基于语义描述（`description`, `use_cases`）选择合适的 Recipe
- AI Agent 通过 `/task-mind.recipe` 命令生成的 Recipe 代码是可执行的（AI 理解 Python/JavaScript/Shell 语法）
...
- 现有 Task-Mind 模块：`task-mind.cdp`（Chrome CDP 操作）、`task-mind.cli`（CLI 框架）
- 外部工具：Node.js（执行 JavaScript Recipe，可选）、Bash/Zsh（执行 Shell Recipe）
- 已有的 Recipe 脚本（位于 `src/task-mind/recipes/`），需迁移到 `examples/atomic/chrome/`
