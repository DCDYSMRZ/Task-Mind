...
**验证 JSON 格式输出**（AI 使用）:
```bash
uv run task-mind recipe list --format json | jq '.[0]'
```
...
```bash
uv run task-mind recipe info upwork_extract_job_details_as_markdown
```
...
```json
{
  ...
  "data": {
    ...
    "project_info": {
      "name": "TestProject",
      "cwd": "/home/yammi/repos/Task-Mind",
      ...
    }
  },
  "execution_time": 0.xx,
...
uv run task-mind recipe run file_copy \
...
**测试 Workflow**:
```bash
uv run task-mind recipe info upwork_batch_extract
```
...
**测试缺少必需参数**:
```bash
uv run task-mind recipe run upwork_batch_extract --params '{}'
```
...
cat > .task-mind/recipes/workflows/test_priority.md <<'EOF'
---
name: test_priority
type: workflow
runtime: python
...
cat > .task-mind/recipes/workflows/test_priority.py <<'EOF'
#!/usr/bin/env python3
import json
...
# 2. 查看优先级
uv run task-mind recipe info test_priority
```
...
**测试 Recipe 不存在**:
```bash
uv run task-mind recipe run nonexistent_recipe
```
...
**测试无效 JSON 参数**:
```bash
uv run task-mind recipe run project_specific_task --params 'invalid-json'
```
...
# 验证已复制
ls ~/.task-mind/recipes/atomic/chrome/ | grep upwork
```

**预期**:
- 显示成功复制的消息
- 文件存在于 `~/.task-mind/recipes/atomic/chrome/`
- 包含 `.md` 和 `.js` 两个文件
...
```bash
# AI 获取 Recipe 列表
RECIPES=$(uv run task-mind recipe list --format json)
...
```bash
uv run task-mind recipe --help
uv run task-mind recipe list --help
uv run task-mind recipe run --help
uv run task-mind recipe info --help
uv run task-mind recipe copy --help
```
...
uv run pytest tests/unit/recipe/ tests/integration/recipe/ \
  --cov=src/task-mind/recipes --cov-report=term-missing
```
...
# 2. Recipe 列表正常
uv run task-mind recipe list --format json | jq 'length'
