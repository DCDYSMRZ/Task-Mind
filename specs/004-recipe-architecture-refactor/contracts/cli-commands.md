...
本文档定义 Task-Mind Recipe 系统的所有 CLI 命令契约，包括命令格式、参数、输出格式和错误处理。

---

## 命令组: `task-mind recipe`
...
**成功**:
```text
✓ Recipe 目录已初始化: /home/user/.task-mind/recipes
  - atomic/chrome/
  - atomic/system/
  - workflows/
```
...
**已存在（非 force）**:
```text
ℹ Recipe 目录已存在: /home/user/.task-mind/recipes
  使用 --force 选项强制重新创建
```
...
## 命令 2: `task-mind recipe list`

**描述**: 列出所有可用的 Recipe，标注来源。

### 用法
```bash
uv run task-mind recipe list [OPTIONS]
```
...
[
  {
    ...
    "runtime": "python",
    "version": "1.0.0",
    "path": "/path/to/.task-mind/recipes/workflows/custom_upwork_workflow.py"
  },
  {
    "source": "User",
    "type": "atomic",
    ...
    "runtime": "chrome-js",
    "version": "1.0.0",
    "path": "/home/user/.task-mind/recipes/atomic/chrome/upwork_extract_job_details_as_markdown.js"
  }
]
```
...
## 命令 3: `task-mind recipe info`

**描述**: 显示指定 Recipe 的详细信息。

### 用法
```bash
uv run task-mind recipe info <NAME> [OPTIONS]
```
...
版本:     1.0.0
来源:     User
路径:     /home/user/.task-mind/recipes/atomic/chrome/upwork_extract_job_details_as_markdown.js
...
```json
{
  ...
  "version": "1.0.0",
  "source": "User",
  "script_path": "/home/user/.task-mind/recipes/atomic/chrome/upwork_extract_job_details_as_markdown.js",
  "metadata_path": "/home/user/.task-mind/recipes/atomic/chrome/upwork_extract_job_details_as_markdown.md",
  "inputs": {
    "url": {
      "type": "string",
      "required": true,
...
已搜索路径:
  - /path/to/project/.task-mind/recipes
  - /home/user/.task-mind/recipes
  - /path/to/task-mind/examples

提示: 使用 'uv run task-mind recipe list' 查看所有可用 Recipe
```
...
## 命令 4: `task-mind recipe run`

**描述**: 执行指定的 Recipe。

### 用法
```bash
uv run task-mind recipe run <NAME> [OPTIONS]
```
...
## 命令 5: `task-mind recipe copy`

**描述**: 将示例 Recipe 复制到用户目录供修改。

### 用法
```bash
uv run task-mind recipe copy <NAME> [OPTIONS]
```
...
1. 在示例目录查找指定 Recipe
2. 确定目标位置：
   - 如在项目目录（存在 `.task-mind/`），默认复制到项目级
   - 否则复制到用户级 `~/.task-mind/recipes/`
...
文件位置:
  脚本:   /home/user/.task-mind/recipes/atomic/chrome/upwork_extract_job_details_as_markdown.js
  元数据: /home/user/.task-mind/recipes/atomic/chrome/upwork_extract_job_details_as_markdown.md
...
文件:
  /home/user/.task-mind/recipes/atomic/chrome/upwork_extract_job_details_as_markdown.js
  /home/user/.task-mind/recipes/atomic/chrome/upwork_extract_job_details_as_markdown.md
...
提示: 使用 'uv run task-mind recipe list --source example' 查看所有示例
```
...
## 命令 6: `task-mind recipe validate`（可选，未来增强）

**描述**: 验证 Recipe 元数据和脚本的有效性。

### 用法
```bash
uv run task-mind recipe validate <NAME_OR_PATH> [OPTIONS]
```
