...
- **Python 包代码**: `src/task-mind/` （打包到 wheel）
...
- [X] T002 [P] 创建 Recipe 引擎目录结构：`src/task-mind/recipes/` 及 `__init__.py`, `runner.py`, `registry.py`, `metadata.py`, `output_handler.py`, `exceptions.py`
...
- [X] T006 [P] 在 `src/task-mind/recipes/exceptions.py` 中实现 Recipe 专用异常类：`RecipeNotFoundError`, `RecipeExecutionError`, `RecipeValidationError`, `MetadataParseError`
- [X] T007 [P] 在 `src/task-mind/recipes/metadata.py` 中实现 `RecipeMetadata` 数据类，包含所有必需字段（name, type, runtime, version, inputs, outputs）和 AI 字段（description, use_cases, tags, output_targets）
- [X] T008 在 `src/task-mind/recipes/metadata.py` 中实现 `parse_metadata_file(path: Path) -> RecipeMetadata` 函数，解析 Markdown 文件的 YAML frontmatter
- [X] T009 在 `src/task-mind/recipes/metadata.py` 中实现 `validate_metadata(metadata: RecipeMetadata) -> None` 函数，验证必需字段、枚举值、AI 字段完整性
- [X] T010 [P] 在 `src/task-mind/recipes/registry.py` 中实现 `RecipeRegistry` 类，包含 `search_paths`, `recipes` 属性和 `scan()`, `find(name)`, `list_all()` 方法
- [X] T011 在 `src/task-mind/recipes/registry.py` 中实现三级查找路径逻辑：项目级（`.task-mind/recipes/`）> 用户级（`~/.task-mind/recipes/`）> 示例级（`examples/`）
- [X] T012 [P] 在 `src/task-mind/recipes/output_handler.py` 中实现 `OutputHandler` 静态类，包含 `handle(data, target, options)` 方法，支持 stdout, file, clipboard 三种输出目标
- [X] T013 [P] 在 `src/task-mind/recipes/runner.py` 中实现 `RecipeRunner` 类框架，包含 `registry` 属性和 `run(name, params, output_target)` 方法签名
...
- [X] T014 [P] [US0] 在 `src/task-mind/cli/recipe_commands.py` 中实现 `recipe list` 命令，支持 `--format json` 选项，输出包含 AI 元数据字段（description, use_cases, tags, output_targets）的 JSON 数组
- [X] T015 [P] [US0] 在 `src/task-mind/cli/recipe_commands.py` 中实现 `recipe info <name>` 命令，显示 Recipe 的所有元数据，包括 AI 可理解字段
- [X] T016 [US0] 在 `src/task-mind/cli/recipe_commands.py` 中实现 `recipe run <name>` 命令，支持 `--params`, `--params-file`, `--output-file`, `--output-clipboard` 选项
- [X] T017 [US0] 在 `src/task-mind/cli/main.py` 中注册 `recipe` 命令组，关联到 `recipe_commands.py` 的命令函数
- [X] T018 [P] [US0] 迁移现有 Recipe 到示例目录：将 `src/task-mind/recipes/upwork_extract_job_details_as_markdown.js` 移动到 `examples/atomic/chrome/`
...
- [X] T021 [US0] 更新 `.claude/commands/task-mind-recipe.md` 文档，说明 `/task-mind.recipe` 命令如何支持 AI 生成 Workflow Recipe（包含 RecipeRunner 导入和调用示例）
...
- [X] T022 [P] [US1] 在 `src/task-mind/recipes/runner.py` 中实现 `_run_chrome_js(script_path, params)` 方法，调用 `uv run task-mind exec-js`，返回 JSON 结果
- [X] T023 [P] [US1] 在 `src/task-mind/recipes/runner.py` 中实现 `_run_python(script_path, params)` 方法，调用 `python3 <script> <params_json>`，返回 JSON 结果
- [X] T024 [P] [US1] 在 `src/task-mind/recipes/runner.py` 中实现 `_run_shell(script_path, params)` 方法，调用 `<script> <params_json>`，检查执行权限，返回 JSON 结果
- [X] T025 [US1] 在 `src/task-mind/recipes/runner.py` 的 `run()` 方法中实现运行时选择逻辑：根据 `metadata.runtime` 调用对应的执行器（chrome-js/python/shell）
- [X] T026 [US1] 在 `src/task-mind/recipes/runner.py` 中实现错误处理：捕获执行失败、JSON 解析错误、超时等，返回统一格式的 `RecipeExecutionError`
...
**Goal**: 将 Task-Mind Python 包代码与用户 Recipe 资源完全分离，用户可在 `~/.task-mind/recipes/` 管理自定义 Recipe

**Independent Test**: 安装 Task-Mind 后，用户在 `~/.task-mind/recipes/` 创建自定义 Recipe，卸载并重新安装 Task-Mind，自定义 Recipe 依然可用

### Implementation for User Story 2

- [X] T030 [P] [US2] 在 `src/task-mind/cli/commands.py` 中实现 `init` 命令，创建用户级目录结构：`~/.task-mind/recipes/atomic/chrome/`, `atomic/system/`, `workflows/`
- [X] T031 [P] [US2] 在 `src/task-mind/cli/recipe_commands.py` 中实现 `recipe copy <name>` 命令，将示例 Recipe 复制到用户级目录（脚本 + 元数据文件）
- [X] T032 [US2] 在 `src/task-mind/recipes/registry.py` 的 `scan()` 方法中实现来源标注逻辑：标记 Recipe 为 `Project` / `User` / `Example` 来源
- [X] T033 [US2] 在 `src/task-mind/cli/recipe_commands.py` 的 `list` 命令中显示来源标签（表格格式的 SOURCE 列，JSON 格式的 source 字段）
...
- [X] T037 [P] [US3] 在 `src/task-mind/recipes/__init__.py` 中导出 `RecipeRunner` 类，供 Workflow Recipe 代码导入使用
- [X] T038 [P] [US3] 在 `src/task-mind/recipes/runner.py` 中实现 `run()` 方法返回值规范化：返回 `{"success": bool, "data": dict, "error": dict | None}` 格式
...
- [X] T041 [US3] 在 `src/task-mind/recipes/registry.py` 的 `scan()` 方法中实现依赖检查：验证 Workflow 的 `dependencies` 中列出的 Recipe 是否存在
- [X] T042 [US3] 在 `src/task-mind/recipes/runner.py` 中实现异常传播：Workflow 调用原子 Recipe 失败时，抛出 `RecipeExecutionError` 供 Workflow 捕获
...
**Independent Test**: 创建一个带有完整元数据的 Recipe（`.md` 文件包含 YAML frontmatter），执行 `uv run task-mind recipe info <name>` 显示所有元数据信息

### Implementation for User Story 4

- [X] T044 [P] [US4] 在 `src/task-mind/recipes/metadata.py` 中实现参数验证逻辑：检查必需参数（`inputs.*.required: true`）是否提供
- [X] T045 [P] [US4] 在 `src/task-mind/recipes/metadata.py` 中实现类型检查逻辑：简单验证参数类型（string, number, boolean, array, object）
- [X] T046 [US4] 在 `src/task-mind/recipes/runner.py` 的 `run()` 方法中调用参数验证：执行前验证 `params` 符合 `metadata.inputs` 定义
- [X] T047 [US4] 在 `src/task-mind/cli/recipe_commands.py` 的 `info` 命令中实现完整元数据展示：名称、类型、运行时、版本、输入参数、输出字段、依赖、AI 字段（description, use_cases, tags, output_targets）
- [X] T048 [US4] 在 `src/task-mind/cli/recipe_commands.py` 中实现参数缺失时的友好错误提示：明确指出缺少哪个必需参数
...
**Goal**: 支持在项目目录下创建项目专属 Recipe（位于 `.task-mind/recipes/`），与全局 Recipe 隔离

**Independent Test**: 在项目目录创建 `.task-mind/recipes/workflows/project_workflow.py`，执行 `uv run task-mind recipe run project_workflow`，系统优先使用项目级 Recipe

### Implementation for User Story 5

- [X] T051 [P] [US5] 在 `src/task-mind/recipes/registry.py` 中实现项目级路径检测：检查当前工作目录的 `.task-mind/recipes/` 是否存在
- [X] T052 [US5] 在 `src/task-mind/recipes/registry.py` 的 `scan()` 方法中实现优先级逻辑：项目级 > 用户级 > 示例级，同名 Recipe 优先使用项目级
- [X] T053 [US5] 在 `src/task-mind/cli/recipe_commands.py` 的 `list` 和 `info` 命令中添加优先级提示：当存在同名 Recipe 时，显示使用哪个来源
...
- [X] T056 [P] 在 `src/task-mind/recipes/output_handler.py` 中添加 clipboard 支持的依赖检查：如果 `pyperclip` 未安装，提示友好错误信息
- [X] T057 [P] 在 `src/task-mind/recipes/runner.py` 中添加超时机制：Recipe 执行超过合理时间（如 5 分钟）自动终止
- [X] T058 [P] 在 `src/task-mind/recipes/runner.py` 中添加输出大小检查：Recipe 输出 JSON 超过 10MB 时拒绝解析并报错
...
```bash
# 可同时启动的任务（不同文件，无依赖）:
Task T014: "在 src/task-mind/cli/recipe_commands.py 中实现 recipe list 命令"
Task T015: "在 src/task-mind/cli/recipe_commands.py 中实现 recipe info 命令"
