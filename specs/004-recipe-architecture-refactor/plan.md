...
重构 Task-Mind 的 Recipe 系统架构，核心目标是**让 AI Agent（Claude Code）能够自主创建、发现、选择和执行 Recipe**。实现代码与资源分离、支持多语言 Recipe（JavaScript、Python、Shell）、通过元数据驱动的方式让 AI 理解每个 Recipe 的能力和输出形态。Recipe 系统设计为"AI 可调度的工具集"，而非传统的"人类操作的 CLI 工具"。

技术方法：
1. **元数据增强**：Recipe 元数据包含 `description`, `use_cases`, `tags`, `output_targets` 等 AI 可理解的字段
2. **混合接口**：AI 通过 Bash 工具调用 CLI 命令，人类也可手动执行（次要场景）
3. **AI 生成 Workflow**：扩展 `/task-mind.recipe` 命令，支持 AI 根据自然语言描述自动生成编排 Recipe
...
**Constraints**:
  - 必须向后兼容现有 `uv run task-mind exec-js` 命令
...
- ✅ **CLI 优先**: 所有功能通过 `uv run task-mind` CLI 暴露（AI 通过 Bash 工具调用）
- ✅ **语言统一**: Python 作为主实现语言
...
.claude/commands/
├── task-mind-recipe.md         # 🔄 更新 /task-mind.recipe 命令（支持生成 Workflow）
└── ...                      # 其他命令
...
1. `src/task-mind/recipes/` 从存放 Recipe 脚本改为存放引擎代码
