# Implementation Plan: Task-Mind GUI 应用模式
...
为 task-mind 添加 GUI 应用模式，通过 `task-mind --gui` 命令启动一个无边框的桌面窗口。该窗口使用 pywebview 渲染 HTML 页面，提供 App 式 UI 界面，包含输入区域、配方列表、skills 列表等功能模块，支持调用 `task-mind agent` 并实时显示 stream-json 响应。

## Technical Context

**Language/Version**: Python 3.9+（符合现有 pyproject.toml 要求）
**Primary Dependencies**: pywebview>=6.1, click>=8.1.0（现有依赖）
**Storage**: 文件系统（`~/.task-mind/` 用户配置、`~/.claude/` 会话上下文）
...
**Constraints**: 离线时提供有限功能，同时只运行一个 task-mind 任务
...
> **注意**: 项目 constitution.md 为模板状态，无具体约束条件。以下为基于 task-mind 项目惯例的自检：

| 检查项 | 状态 | 说明 |
|--------|------|------|
| 符合现有技术栈 | ✅ | Python + Click CLI，pywebview 为新增轻量依赖 |
| 遵循项目结构 | ✅ | 新增 `src/task-mind/gui/` 模块，符合现有 src 布局 |
...
**Structure Decision**: 采用单项目结构（Option 1），在现有 `src/task-mind/` 下新增 `gui/` 模块。前端资源（HTML/CSS/JS）作为 assets 子目录打包，随 PyPI 包分发。
