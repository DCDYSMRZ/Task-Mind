...
window = webview.create_window('Task-Mind', 'index.html', js_api=Api())
```
...
`task-mind agent` 返回 stream-json 格式（每行一个 JSON）。前端需要：
...
CONFIG_DIR = Path.home() / '.task-mind'
...
```bash
# 基本安装（使用系统默认 WebView）
pip install task-mind-cli[gui]

# Linux GTK 后端
pip install task-mind-cli[gui-gtk]

# Linux QT 后端
pip install task-mind-cli[gui-qt]
```
