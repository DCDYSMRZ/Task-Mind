...
```bash
# 检查Python环境
cd /Users/chagee/Repos/Task-Mind
...
# 测试Python CLI对应功能
python -m task-mind.cli navigate https://www.google.com
```
...
```bash
# 生成详细HTML报告
python -m task-mind.tools.function_mapping --format html --output function_mapping_report.html
...
```bash
# 测试Python CLI代理支持
python -m task-mind.cli navigate https://www.google.com --proxy-host proxy.example.com --proxy-port 8080

# 测试绕过代理
python -m task-mind.cli navigate https://www.google.com --no-proxy
```
...
```python
# 1. 在 src/task-mind/cdp/commands/ 下创建新模块
# 例如: src/task-mind/cdp/commands/new_feature.py
...
# 调用Python CLI
python -m task-mind.cli new-feature --param1 "${PARAM1}" --param2 "${PARAM2}"
```

### 3. 注册CLI命令

```python
# 在 src/task-mind/cli/commands.py 中添加
@click.command()
...
# 在 src/task-mind/cli/main.py 中注册
...
```bash
# 重新生成功能映射报告
python -m task-mind.tools.function_mapping --verbose

# 检查具体功能差异
python -m task-mind.tools.function_mapping --function cdp_navigate
```
...
# Python CLI调试模式
python -m task-mind.cli navigate https://www.google.com --debug
