...
**Created**: 2025-12-23
**Status**: Draft
**Input**: User description: "`uv run task-mind server` runs server in background mode (daemon), unless --debug flag is used. Default port is 8093 only. Web page at 127.0.0.1:8093 requires complete frontend redesign following FRONTEND_STYLE_GUIDE.md from Agentic-LLM project. Layout uses mainstream admin panel style with collapsible left sidebar menu. Menu order: Dashboard (new), Tasks, Recipes, Skills, Settings. Default landing page is Tasks. All pages retain existing GUI functionality except Dashboard which needs new design (replaces Tips)."

## Clarifications

### Session 2025-12-23

- Q: How should `task-mind server` implement "background mode" on Windows? → A: Windows runs as a hidden-window subprocess, functionally equivalent to Unix daemon mode

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Start Server in Background Mode (Priority: P1)

A user wants to start the Task-Mind web service and have it run persistently in the background while they continue working on other tasks in their terminal. They run `uv run task-mind server` and the command returns immediately to the shell prompt, with the server running as a daemon process accessible at port 8093.
...
**Independent Test**: Can be fully tested by running `task-mind server`, verifying the command returns to prompt within 3 seconds, and confirming the web service responds at http://127.0.0.1:8093

**Acceptance Scenarios**:

1. **Given** no Task-Mind server is running, **When** user runs `task-mind server`, **Then** the command returns to prompt within 3 seconds AND server process runs in background AND http://127.0.0.1:8093 responds
2. **Given** a Task-Mind server is already running on port 8093, **When** user runs `task-mind server`, **Then** user sees a message indicating server is already running with its process ID
3. **Given** server is running in background, **When** user wants to stop the server, **Then** user can run `task-mind server stop` to terminate the background process

---

### User Story 2 - Navigate Admin Panel Layout (Priority: P1)

A user accesses the Task-Mind web interface at http://127.0.0.1:8093 and sees a professional dark-themed admin panel with a collapsible left sidebar menu containing five navigation items: Dashboard, Tasks, Recipes, Skills, and Settings. The interface automatically opens on the Tasks page.
...
A developer needs to debug the Task-Mind server and wants to see all logs in real-time. They run `task-mind server --debug` and the server runs in foreground mode with verbose output displayed in the terminal, blocking the terminal until stopped with Ctrl+C.
...
**Independent Test**: Can be fully tested by running `task-mind server --debug`, verifying terminal shows live logs and does not return to prompt, then pressing Ctrl+C to confirm graceful shutdown

**Acceptance Scenarios**:

1. **Given** user runs `task-mind server --debug`, **When** server starts, **Then** terminal shows verbose log output AND does not return to prompt AND server listens on port 8093
...
A user opens the Settings page and can configure all Task-Mind settings including General, Sync, Secrets, Appearance, and About sections. Functionality matches existing GUI with new styling.
...
A user opens the Dashboard page and sees an overview of their Task-Mind environment including system status, recent activity summary, quick action shortcuts, and resource statistics. This replaces the Tips page from the original GUI.
...
- What happens when user runs `task-mind server` while server is already running?
...
- What happens when system reboots?
  ...
  - User must manually run `task-mind server` again
...
- What happens on Windows when running `task-mind server`?
  - Server spawns as a hidden-window subprocess (no console window visible)
  - `task-mind server stop` and `task-mind server status` work identically to Unix
...
- **FR-001**: System MUST run the web server as a background daemon process by default when `task-mind server` is executed
...
- **FR-004**: System MUST provide `task-mind server stop` command to terminate background server process
- **FR-005**: System MUST provide `task-mind server status` command to check if server is running and display its PID
