...
**Storage**: `~/.task-mind/server.pid` (PID only, other fields derived at runtime)

**State Transitions**:
```
[Not Running] --task-mind server--> [Starting] --success--> [Running]
                                          --failure--> [Error]
[Running] --task-mind server stop--> [Stopping] --> [Not Running]
...
|--------|------------------|--------|
| ServerProcess PID | `~/.task-mind/server.pid` | Plain text (PID number) |
| Server logs | `~/.task-mind/server.log` | Plain text |
