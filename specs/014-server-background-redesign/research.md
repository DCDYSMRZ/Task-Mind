...
# Unix
subprocess.Popen(
    [sys.executable, "-m", "task-mind.server.runner", "--port", "8093"],
...
**Decision**: Store PID in `~/.task-mind/server.pid` with lock file validation
...
- Port probe only: Can't distinguish Task-Mind from other services on same port

**Implementation pattern**:
```python
PID_FILE = Path.home() / ".task-mind" / "server.pid"
...
2. PID file at `~/.task-mind/server.pid` with `psutil` validation
