...
|------------------|---------|------|
| command | CLI命令 | uv run task-mind navigate |
...
```json
{
  ...
  "data": {
    "command": "uv run task-mind navigate https://example.com",
    "exit_code": 0,
    "duration_ms": 1200
  }
}
...
采用 **轻量级JSON文件** 方案：
- 文件路径：`.task-mind/current_run` （无扩展名，Git忽略）
- 格式：JSON，包含 `run_id`、`last_accessed`、`theme_description`
- 读取优先级：环境变量 `FRAGO_CURRENT_RUN` > `.task-mind/current_run` 文件
...
def get_current_run_context() -> Optional[dict]:
    ...
    # 2. 读取配置文件
    config_path = Path('.task-mind/current_run')
...
每个关键步骤后：
```bash
uv run task-mind run log --step "..." --status "success" --action-type "..." --execution-method "..." --data '{...}'
```
...
- 现有命令：`.claude/commands/task-mind.recipe.md`（611行，提供详细执行流程）
- 现有命令：`.claude/commands/task-mind.test.md`（134行，清晰的步骤划分）
...
- 扫描 `runs/` 目录，读取每个run的 `.task-mind/current_run` 或 `logs/execution.jsonl` 首行提取主题
...
1. **Phase 1: 核心命令**（无依赖）
   - `uv run task-mind run init <description>` - 使用pypinyin+slugify
   - `uv run task-mind run set-context <run_id>` - 读写.task-mind/current_run
   - `uv run task-mind run log ...` - JSONL追加写入

2. **Phase 2: 截图和状态管理**（依赖Phase 1）
   - `uv run task-mind run screenshot <description>` - 基于文件扫描编号
   - `uv run task-mind run list` - 扫描runs/目录

3. **Phase 3: 智能发现**（依赖Phase 1+2）
   - 自动发现机制 - 使用RapidFuzz匹配相似run
   - `/task-mind.run` slash命令 - 集成所有子命令
...
| `.task-mind/current_run` 损坏 | 命令失败 | 提示用户重新set-context；考虑备份机制 |
