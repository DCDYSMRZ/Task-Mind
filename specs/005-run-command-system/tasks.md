...
- [X] T001 按实施计划创建 Run 系统目录结构（`src/task-mind/run/`, `tests/unit/test_run/`, `.task-mind/`, `runs/`）
- [X] T002 添加依赖到 pyproject.toml（pypinyin>=0.51.0, python-slugify>=8.0.0, rapidfuzz>=3.0.0）
- [X] T003 [P] 在 .gitignore 中添加 `runs/` 和 `.task-mind/current_run`
...
- [X] T004 创建数据模型在 src/task-mind/run/models.py（RunInstance, LogEntry, Screenshot, CurrentRunContext）
- [X] T005 [P] 创建自定义异常在 src/task-mind/run/exceptions.py（RunNotFoundError, InvalidRunIDError, ContextNotSetError, CorruptedLogError）
- [X] T006 [P] 实现主题slug生成逻辑在 src/task-mind/run/utils.py（使用 pypinyin + python-slugify）
- [X] T007 [P] 实现上下文管理器在 src/task-mind/run/context.py（读写 .task-mind/current_run，支持环境变量优先级）
- [X] T008 实现日志记录器在 src/task-mind/run/logger.py（JSONL格式化、schema验证、追加写入）
- [X] T009 实现 Run 实例管理器在 src/task-mind/run/manager.py（创建、查找、列表、归档）
...
**Independent Test**: 运行 `uv run task-mind run init "测试任务"` 两次，第二次应自动发现第一次创建的run

### Implementation for User Story 2

- [X] T010 [P] [US2] 实现 init 子命令在 src/task-mind/cli/run_commands.py（调用 manager.create_run）
- [X] T011 [P] [US2] 实现 set-context 子命令在 src/task-mind/cli/run_commands.py（调用 context.set_current_run）
- [X] T012 [P] [US2] 实现 list 子命令在 src/task-mind/cli/run_commands.py（支持 --format table/json, --status active/archived/all）
- [X] T013 [P] [US2] 实现 info 子命令在 src/task-mind/cli/run_commands.py（显示run详情、统计信息、最近日志）
- [X] T014 [P] [US2] 实现 archive 子命令在 src/task-mind/cli/run_commands.py（更新状态为archived，清空当前上下文）
- [X] T015 [US2] 实现 run 实例发现逻辑在 src/task-mind/run/discovery.py（扫描 runs/ 目录，使用 RapidFuzz 计算相似度）
- [X] T016 [US2] 在 src/task-mind/cli/commands.py 中注册 run 命令组（集成到主 CLI）
...
**Independent Test**: 执行 `uv run task-mind run log --step "测试" --status "success" --action-type "analysis" --execution-method "manual" --data '{}'`，验证日志正确写入

### Implementation for User Story 3

- [X] T017 [P] [US3] 实现 log 子命令在 src/task-mind/cli/run_commands.py（验证枚举值、调用 logger.write_log）
- [X] T018 [P] [US3] 实现截图自动编号机制在 src/task-mind/run/screenshot.py（扫描现有文件、原子性写入）
- [X] T019 [US3] 实现 screenshot 子命令在 src/task-mind/cli/run_commands.py（调用 CDP 截图、自动编号、记录日志）
...
**Goal**: 通过 /task-mind.run slash 命令支持 AI 主持的任务执行，作为信息中心

**Independent Test**: 在 Claude Code 中运行 `/task-mind.run "访问example.com并提取页面标题"`，验证任务执行和日志记录

### Implementation for User Story 1

- [X] T022 [US1] 创建 /task-mind.run slash 命令文档在 .claude/commands/task-mind.run.md（包含执行流程、工具使用指引、数据记录规范）
- [X] T023 [US1] 在 task-mind.run.md 中添加 run 实例发现流程（调用 list --format json，展示交互式菜单）
- [X] T024 [US1] 在 task-mind.run.md 中添加 Recipe 集成指引（如何发现和调用现有 Recipe）
- [X] T025 [US1] 在 task-mind.run.md 中添加代码文件处理约束（必须保存为 scripts/ 文件，禁止直接存储代码到日志）
- [X] T026 [US1] 在 task-mind.run.md 中添加进度展示要求（每5步输出摘要）和用户交互指引（使用 AskUserQuestion）
- [X] T027 [US1] 在 task-mind.run.md 中添加日志示例（6种 execution_method 的完整示例）

**Checkpoint**: /task-mind.run slash 命令可用，AI 可以执行复杂任务并正确记录日志

---

## Phase 6: User Story 4 - 清理过时的视频制作命令 (Priority: P3)

**Goal**: 删除旧的视频制作命令，将 Task-Mind 定位转变为多运行时自动化基建

**Independent Test**: 尝试执行 `/task-mind.start`，确认命令不存在

### Implementation for User Story 4

- [X] T028 [P] [US4] 删除 .claude/commands/task-mind.start.md
- [X] T029 [P] [US4] 删除 .claude/commands/task-mind.storyboard.md
- [X] T030 [P] [US4] 删除 .claude/commands/task-mind.generate.md
- [X] T031 [P] [US4] 删除 .claude/commands/task-mind.evaluate.md
- [X] T032 [P] [US4] 删除 .claude/commands/task-mind.merge.md
...
- [X] T046 [P] 添加类型注解到所有 run 模块（src/task-mind/run/*.py）
...
- **User Story 1 (P1)**: Depends on US2 + US3 - /task-mind.run 需要调用所有 CLI 命令
...
```bash
# 并行创建基础模块（不同文件）:
Task: "创建自定义异常在 src/task-mind/run/exceptions.py"
Task: "实现主题slug生成逻辑在 src/task-mind/run/utils.py"
Task: "实现上下文管理器在 src/task-mind/run/context.py"

# 等待数据模型完成后，并行创建依赖模块:
Task: "实现日志记录器在 src/task-mind/run/logger.py"
Task: "实现 Run 实例管理器在 src/task-mind/run/manager.py"
```
...
Task: "[US2] 实现 init 子命令在 src/task-mind/cli/run_commands.py"
Task: "[US2] 实现 set-context 子命令在 src/task-mind/cli/run_commands.py"
Task: "[US2] 实现 list 子命令在 src/task-mind/cli/run_commands.py"
Task: "[US2] 实现 info 子命令在 src/task-mind/cli/run_commands.py"
Task: "[US2] 实现 archive 子命令在 src/task-mind/cli/run_commands.py"
```
...
2. Complete Phase 5: User Story 1（/task-mind.run slash命令）
3. **STOP and VALIDATE**: 在 Claude Code 中测试 /task-mind.run
...
3. After US2 + US3 complete:
   - Developer A: User Story 1（/task-mind.run slash命令）
...
- ✅ AI 在 /task-mind.run 中能识别并调用现有 Recipe（SC-007）
