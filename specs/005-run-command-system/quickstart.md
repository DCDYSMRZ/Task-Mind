...
Run 命令系统为 Task-Mind 提供持久化的任务执行管理能力，包括：
- **CLI 子命令组**（`uv run task-mind run`）：管理 run 实例生命周期
- **/task-mind.run slash 命令**：在 Claude Code 中执行 AI 主持的任务
...
```bash
$ uv run task-mind run init "测试run系统"
{
...
```bash
$ uv run task-mind run set-context ce-shi-run-xi-tong
{
  "run_id": "ce-shi-run-xi-tong",
...
```bash
$ uv run task-mind run info ce-shi-run-xi-tong
...
```bash
$ uv run task-mind run init "在Upwork上搜索Python职位"
{
...
$ uv run task-mind run set-context zai-upwork-shang-sou-suo-python-zhi-wei
```
...
```bash
# 导航到搜索页
$ uv run task-mind navigate https://upwork.com/search
$ uv run task-mind run log \
  --step "导航到Upwork搜索页" \
  --status "success" \
  --action-type "navigation" \
  --execution-method "command" \
  --data '{"command": "uv run task-mind navigate https://upwork.com/search", "exit_code": 0}'

# 截图
$ uv run task-mind run screenshot "搜索页面"
{
...
# 提取职位（使用Recipe）
$ uv run task-mind recipe run upwork_extract_job_list --params '{"keyword": "Python"}'
$ uv run task-mind run log \
  --step "提取Python职位列表" \
  --status "success" \
...
```bash
$ uv run task-mind run info zai-upwork-shang-sou-suo-python-zhi-wei
...
```bash
# 列出现有 run
$ uv run task-mind run list --format table
...
## 在 Claude Code 中使用 /task-mind.run

### 场景 1：创建新 run

```
Human: /task-mind.run 在Upwork上搜索Python职位
...
```
Human: /task-mind.run 继续分析这些职位的技能要求
...
```bash
$ uv run task-mind run list --format json | jq '.runs[].run_id'
...
```bash
$ uv run task-mind run archive zai-upwork-shang-sou-suo-python-zhi-wei
{
  "run_id": "zai-upwork-shang-sou-suo-python-zhi-wei",
...
# 归档后不会显示在默认列表中
$ uv run task-mind run list --status active
(无结果)

# 查看归档的 run
$ uv run task-mind run list --status archived
RUN_ID                                  STATUS    CREATED_AT           LAST_ACCESSED
...
$ uv run task-mind run log --step "测试" --status "success" ...
...
```bash
$ uv run task-mind run log ...
Error: Current run context not set. Run 'uv run task-mind run set-context <run_id>' first.
```

**解决方法**：
```bash
$ uv run task-mind run set-context <run_id>
```

### 错误：Run 'xxx' not found

```bash
$ uv run task-mind run set-context invalid-id
...
# 使用正确的 run_id
$ uv run task-mind run set-context <正确的run_id>
```

### 日志文件损坏

```bash
$ uv run task-mind run info my-run
