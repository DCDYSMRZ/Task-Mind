...
本文档定义 `uv run task-mind run` 子命令组的 CLI 接口契约。
...
```bash
uv run task-mind run <subcommand> [options]
```
...
```bash
uv run task-mind run init <description>
```
...
```bash
$ uv run task-mind run set-context find-job-on-upwork
{
  "run_id": "find-job-on-upwork",
  "theme_description": "在Upwork上搜索Python职位",
  "set_at": "2025-11-21T10:32:00Z"
...
1. 追加到 `runs/<run_id>/logs/execution.jsonl`：
   ```json
   {"timestamp":"2025-11-21T10:35:00Z","step":"提取到5个职位","status":"success","action_type":"extraction","execution_method":"command","schema_version":"1.0","data":{"command":"uv run task-mind recipe run ...","jobs":[...],"total":5}}
   ```
...
$ uv run task-mind run log \
  ...
  --data '{"command": "uv run task-mind recipe run ...", "jobs": [...], "total": 5}'
...
```bash
uv run task-mind run screenshot <description>
```
...
```bash
$ uv run task-mind run screenshot "搜索结果页面"
{
  "file_path": "runs/find-job-on-upwork/screenshots/001_search-page.png",
  "sequence_number": 1,
  "saved_at": "2025-11-21T10:36:00Z"
...
```bash
uv run task-mind run list [--format <format>] [--status <status>]
```
...
```bash
$ uv run task-mind run list --format json --status active
{
  "runs": [...],
  "total": 2
}
...
```bash
uv run task-mind run info <run_id> [--format <format>]
```
...
```bash
uv run task-mind run archive <run_id>
```
...
2. 如果是当前上下文的 run，清空 `.task-mind/current_run`
