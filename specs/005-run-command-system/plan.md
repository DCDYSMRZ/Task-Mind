...
1. **CLI `uv run task-mind run` 子命令组**：管理运行实例的生命周期（init、set-context、log、screenshot）
2. **/task-mind.run slash 命令**：在 Claude Code 中执行 AI 主持的浏览器自动化任务
...
- click (CLI 框架，已用于现有 task-mind CLI)
- 现有 Task-Mind CDP 客户端模块
- 现有 Recipe 系统模块
...
- 配置：`.task-mind/current_run`（存储当前 run 上下文）
...
**Project Type**: Single project（扩展现有 Task-Mind CLI）
...
- /task-mind.run slash 命令需集成到 Claude Code
...
│   ├── context.py           # 上下文管理（读写 .task-mind/current_run）
...
.claude/
└── commands/
    ├── task-mind.recipe.md      # 现有
    ├── task-mind.test.md        # 现有
    └── task-mind.run.md         # 🆕 AI 主持的任务执行 slash 命令
...
**Structure Decision**: Single project 结构，扩展现有 `src/task-mind/` 模块。新增 `run/` 子模块处理核心逻辑，CLI 命令集成到 `cli/run_commands.py`。Run 实例数据存储在项目根目录的 `runs/` 目录（与现有 `projects/`、`examples/` 目录并列）。
