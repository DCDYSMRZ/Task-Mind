# Developer Quickstart: task-mind init
...
本文档帮助开发者快速搭建本地开发环境，运行和测试 `task-mind init` 命令。
...
```bash
# 运行 task-mind 命令确认安装成功
uv run task-mind --version
...
**src/task-mind/cli/main.py** (现有文件，需添加 init 命令)：

```python
import click
from task-mind.cli import commands, recipe_commands
from task-mind.cli.init_command import init  # 新增导入

@click.group()
@click.version_option()
def cli():
    """Task-Mind - AI-driven multi-runtime automation framework"""
    pass
...
| 文件 | 路径 | 用途 |
|------|------|------|
| **主配置** | `~/.task-mind/config.json` | 持久化配置 |
| **临时状态** | `~/.task-mind/.init_state.json` | Ctrl+C 恢复用 |
| **配置备份** | `~/.task-mind/config.json.bak` | 覆盖时自动创建 |

### 查看配置

```bash
# 使用 init 命令查看
uv run task-mind init --show-config

# 或直接查看文件
cat ~/.task-mind/config.json | python -m json.tool

# 查看临时状态
cat ~/.task-mind/.init_state.json | python -m json.tool
```
...
# 或保留配置，仅删除临时状态
rm ~/.task-mind/.init_state.json

# 使用 --reset 选项
uv run task-mind init --reset
```
...
```bash
# 创建自定义配置用于测试
mkdir -p ~/.task-mind cat > ~/.task-mind/config.json << 'EOF'
{
  "schema_version": "1.0",
...
# 查看测试覆盖率
uv run pytest tests/unit/init/ --cov=task-mind.init --cov-report=term-missing
```
...
```bash
# 生成 HTML 覆盖率报告
uv run pytest tests/unit/init/ --cov=task-mind.init --cov-report=html
...
```bash
# 运行测试时设置环境变量
FRAGO_TEST_INSTALL_FAIL=1 uv run task-mind init
```
...
uv run task-mind init --reset --skip-deps
```
...
4. **添加测试**：
   ...
   def test_install_something_success():
       with patch('task-mind.init.installer.run_external_command') as mock_run:
...
```bash
# 使用 time 命令
time uv run task-mind init --non-interactive
...
```bash
# 检查 task-mind 是否正确安装
uv run which task-mind uv run task-mind --help

# 重新安装
uv sync --reinstall

# 验证 entry points
python -c "import pkg_resources; print(pkg_resources.get_distribution('task-mind').get_entry_map())"
```
...
# 修复权限
chmod 755 ~/.task-mind chmod 644 ~/.task-mind/config.json

# 检查所有者
ls -l ~/.task-mind/config.json

# 如果需要，修改所有者
sudo chown $USER:$USER ~/.task-mind/config.json
```
...
grep -n "init" src/task-mind/cli/main.py

# 验证 Click 组
uv run python -c "from task-mind.cli.main import cli; print(cli.commands)"
```
...
- [ ] 代码格式化：`uv run black src/task-mind/init/`
- [ ] 代码 lint：`uv run ruff check src/task-mind/init/`
- [ ] 类型检查：`uv run mypy src/task-mind/init/`
- [ ] 单元测试通过：`uv run pytest tests/unit/init/ -v`
- [ ] 集成测试通过：`uv run pytest tests/integration/test_init_command.py -v`
- [ ] 测试覆盖率 >= 80%：`uv run pytest --cov=task-mind.init --cov-report=term`
...
# 代码格式化
uv run black src/task-mind/init/
uv run ruff check --fix src/task-mind/init/
...
| 用途 | 路径 |
|------|------|
| 配置文件 | `~/.task-mind/config.json` |
| 临时状态 | `~/.task-mind/.init_state.json` |
| 主入口 | `src/task-mind/cli/main.py` |
| Init 命令 | `src/task-mind/cli/init_command.py` |
| 数据模型 | `src/task-mind/init/models.py` |
...
# 自定义配置目录
export FRAGO_CONFIG_DIR=/tmp/task_mind_test
...
本快速入门指南涵盖了开发 `task-mind init` 命令所需的所有基础知识：
...
**下一步**: 开始实现 `src/task-mind/init/` 模块，参考 `data-model.md` 和 `contracts/cli_commands.md`。
