...
2. **用户基础**：Task-Mind 主要面向开发者，Windows 用户通常已有 Node.js 环境或有能力手动安装
...
2. **同步风格**：与现有 Task-Mind 代码风格一致（非 async 项目）
...
| 方案 | 优点 | 缺点 | 为何未采用 |
|------|------|------|-----------|
| **inquirer / questionary** | UI 更美观；支持复杂交互（多选列表） | 额外依赖；终端兼容性问题；样式与 Task-Mind 不一致 | 过度设计；MVP 不需要复杂 UI |
...
# 使用示例
def init_command():
    state_file = Path.home() / ".task-mind" / ".init_state.json"
    ...
    try:
        ...
        if handler.interrupted:
            ...
            click.echo("进度已保存。下次运行 'task-mind init' 时可继续。")
