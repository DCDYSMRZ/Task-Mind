# CLI Command Contracts: task-mind init

**Feature**: 006-init-command
**Date**: 2025-11-25
**Related**: [spec.md](../spec.md) | [data-model.md](../data-model.md)

本文档定义 `task-mind init` 命令的完整接口契约，包括命令签名、选项、退出码和输出格式。
...
```bash
task-mind init [OPTIONS]
```

### 描述

初始化 Task-Mind 开发环境，检查并安装必要依赖（Node.js, Claude Code），配置认证方式和可选组件。
...
- **行为**:
  1. 删除 `~/.task-mind/.init_state.json`（如果存在）
  2. 不删除 `~/.task-mind/config.json`（保留已有配置）
  3. 重新执行完整 init 流程
...
| `3` | `CONFIG_ERROR` | 配置文件错误（格式错误、权限问题） | 删除 `~/.task-mind/config.json` 或修复权限 |
...
case $exit_code in
  ...
  1)
    ...
    ;;
  2)
    echo "User cancelled. Run 'task-mind init' again when ready."
    exit 0
    ;;
  3)
    echo "Config error. Try: rm ~/.task-mind/config.json && task-mind init"
    exit 1
    ;;
  *)
...
Next steps:
  1. Run: task-mind recipe list
  2. Try: task-mind navigate https://example.com

Config saved to: /home/user/.task-mind/config.json
```
...
```bash
# 清除状态 + 非交互模式
task-mind init --reset --non-interactive

# 跳过依赖 + 显示配置
task-mind init --skip-deps --show-config  # ⚠️ 冲突：--show-config 会直接退出
```
