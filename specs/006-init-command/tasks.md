# Tasks: Task-Mind 环境初始化命令

**Input**: Design documents from `/home/yammi/repos/Task-Mind/specs/006-init-command/`
...
- **Single project**: `src/task-mind/`, `tests/` at repository root
- 遵循现有 Task-Mind 项目结构
...
- [x] T001 在 `src/task-mind/init/` 创建模块目录结构
- [x] T002 [P] 创建 `src/task-mind/init/__init__.py` 导出模块
...
- [x] T005 [P] 在 `src/task-mind/init/models.py` 实现 Config 数据模型（Pydantic）
- [x] T006 [P] 在 `src/task-mind/init/models.py` 实现 APIEndpoint 嵌套模型
- [x] T007 [P] 在 `src/task-mind/init/models.py` 实现 TemporaryState 数据模型
- [x] T008 [P] 在 `src/task-mind/init/models.py` 实现 InstallationStep 状态机
- [x] T009 [P] 在 `src/task-mind/init/models.py` 实现 DependencyCheckResult 模型
- [x] T010 [P] 在 `src/task-mind/init/exceptions.py` 创建自定义异常类（CommandError, InitErrorCode）
...
**Goal**: 实现 task-mind init 的核心功能 - 并行检查 Node.js 和 Claude Code 的安装状态，智能决定需要安装的组件

**Independent Test**: 在全新系统上运行 `uv run task-mind init`，系统能够：
...
- [x] T019 [P] [US1] 在 `src/task-mind/init/checker.py` 实现 check_node() 函数（检测 Node.js 版本）
- [x] T020 [P] [US1] 在 `src/task-mind/init/checker.py` 实现 check_claude_code() 函数
- [x] T021 [US1] 在 `src/task-mind/init/checker.py` 实现 parallel_dependency_check() 使用 ThreadPoolExecutor
- [x] T022 [P] [US1] 在 `src/task-mind/init/installer.py` 实现 run_external_command() 包装器（错误处理）
- [x] T023 [P] [US1] 在 `src/task-mind/init/installer.py` 实现 install_node() 函数（通过 nvm）
- [x] T024 [P] [US1] 在 `src/task-mind/init/installer.py` 实现 install_claude_code() 函数（npm install）
- [x] T025 [US1] 在 `src/task-mind/cli/init_command.py` 创建 Click 命令框架（@click.command）
- [x] T026 [US1] 在 `src/task-mind/cli/init_command.py` 实现依赖检查流程（调用 checker）
- [x] T027 [US1] 在 `src/task-mind/cli/init_command.py` 实现安装流程（调用 installer）
- [x] T028 [US1] 在 `src/task-mind/cli/init_command.py` 添加安装失败时的错误处理（立即终止）
- [x] T029 [US1] 在 `src/task-mind/cli/main.py` 注册 init 命令到 CLI 组
...
**Independent Test**: 运行 `uv run task-mind init`（依赖已满足），系统能够：
1. 显示两种认证方式选项
2. 用户选择官方登录或自定义端点
3. 根据选择引导完成相应配置
4. 配置保存到 `~/.task-mind/config.json`，标记选择的认证方式
...
- [x] T036 [P] [US2] 在 `src/task-mind/init/configurator.py` 实现 prompt_auth_method() 使用 Click.choice
- [x] T037 [P] [US2] 在 `src/task-mind/init/configurator.py` 实现 configure_official_auth() 函数
- [x] T038 [P] [US2] 在 `src/task-mind/init/configurator.py` 实现 configure_custom_endpoint() 函数
- [x] T039 [P] [US2] 在 `src/task-mind/init/configurator.py` 实现 load_config() 和 save_config() 函数
- [x] T040 [US2] 在 `src/task-mind/cli/init_command.py` 集成认证配置流程（调用 configurator）
- [x] T041 [US2] 在 `src/task-mind/cli/init_command.py` 实现认证方式切换时的警告提示
...
**Independent Test**: 在已有完整配置的系统上运行 `uv run task-mind init`：
1. 系统检测到所有依赖已满足
2. 系统读取并显示当前配置摘要
3. 询问用户是否需要更新配置
4. 根据用户选择进入对应的更新流程
...
- [x] T046 [P] [US3] 在 `src/task-mind/init/configurator.py` 实现 display_config_summary() 函数
- [x] T047 [P] [US3] 在 `src/task-mind/init/configurator.py` 实现 prompt_config_update() 函数
- [x] T048 [P] [US3] 在 `src/task-mind/init/configurator.py` 实现 select_config_items_to_update() 函数
- [x] T049 [US3] 在 `src/task-mind/cli/init_command.py` 实现配置检测逻辑（所有依赖已满足且配置存在）
- [x] T050 [US3] 在 `src/task-mind/cli/init_command.py` 实现配置更新分支逻辑
...
**Independent Test**: 运行 `uv run task-mind init` 并选择"自定义端点"：
1. 系统显示支持的端点类型列表（Deepseek、Aliyun、M2、自定义）
2. 用户选择端点类型
3. 系统提示输入 API Key
4. 配置保存到 `~/.task-mind/config.json`
...
- [x] T056 [P] [US4] 在 `src/task-mind/init/configurator.py` 实现 prompt_endpoint_type() 函数
- [x] T057 [P] [US4] 在 `src/task-mind/init/configurator.py` 实现 prompt_api_key() 函数（使用 hide_input=True）
- [x] T058 [P] [US4] 在 `src/task-mind/init/configurator.py` 实现 prompt_custom_endpoint_url() 函数
- [x] T059 [P] [US4] 在 `src/task-mind/init/configurator.py` 实现 validate_endpoint_url() 函数
- [x] T060 [P] [US4] 在 `src/task-mind/init/configurator.py` 实现预设端点 URL 映射（Deepseek/Aliyun/M2）
- [x] T061 [US4] 在 `src/task-mind/cli/init_command.py` 集成自定义端点配置逻辑到认证流程
...
**Independent Test**: 运行 `uv run task-mind init` 并选择"使用 Claude Code Router"：
1. 系统询问是否安装 CCR
2. 用户同意后，系统安装 CCR
3. 系统提供 CCR 配置模板
4. 用户完成配置后，系统保存设置
...
- [ ] T066 [P] [US5] 在 `src/task-mind/init/checker.py` 实现 check_ccr() 函数
- [ ] T067 [P] [US5] 在 `src/task-mind/init/installer.py` 实现 install_ccr() 函数
- [ ] T068 [P] [US5] 在 `src/task-mind/init/configurator.py` 实现 create_ccr_config_template() 函数
- [ ] T069 [P] [US5] 在 `src/task-mind/init/configurator.py` 实现 prompt_ccr_enable() 函数
- [ ] T070 [US5] 在 `src/task-mind/cli/init_command.py` 集成 CCR 配置流程（可选步骤）
...
1. 检查 `~/.task-mind/config.json` 文件存在
...
- [x] T074 [P] [US6] 在 `src/task-mind/init/configurator.py` 完善 save_config() 函数（原子写入）
- [x] T075 [P] [US6] 在 `src/task-mind/init/configurator.py` 实现 format_final_summary() 函数
- [x] T076 [P] [US6] 在 `src/task-mind/init/configurator.py` 实现 suggest_next_steps() 函数
- [x] T077 [US6] 在 `src/task-mind/cli/init_command.py` 在流程结束时调用配置保存和摘要显示
...
- [x] T078 [P] 在 `src/task-mind/init/recovery.py` 实现 GracefulInterruptHandler 类
- [x] T079 [P] 在 `src/task-mind/init/recovery.py` 实现 load_temp_state() 函数
- [x] T080 [P] 在 `src/task-mind/init/recovery.py` 实现 save_temp_state() 函数
- [x] T081 [P] 在 `src/task-mind/init/recovery.py` 实现 delete_temp_state() 函数
- [x] T082 [P] 在 `src/task-mind/init/recovery.py` 实现 prompt_resume() 函数
...
- [x] T086 在 `src/task-mind/cli/init_command.py` 集成 GracefulInterruptHandler（信号处理）
- [x] T087 在 `src/task-mind/cli/init_command.py` 实现启动时的状态恢复检测
- [x] T088 在 `src/task-mind/cli/init_command.py` 在每个步骤完成后更新临时状态
...
- [x] T089 [P] 在 `src/task-mind/cli/init_command.py` 添加 --reset 选项（Click.option）
- [x] T090 [P] 在 `src/task-mind/cli/init_command.py` 添加 --show-config 选项
- [x] T091 [P] 在 `src/task-mind/cli/init_command.py` 添加 --skip-deps 选项
- [x] T092 [P] 在 `src/task-mind/cli/init_command.py` 添加 --non-interactive 选项
- [x] T093 [P] 在 `src/task-mind/init/configurator.py` 实现 --show-config 显示逻辑
...
- [x] T097 [P] 在 `src/task-mind/init/formatter.py` 创建错误消息格式化模块
- [x] T098 [P] 在 `src/task-mind/init/formatter.py` 实现 format_error_message() 函数
- [x] T099 [P] 在 `src/task-mind/init/formatter.py` 实现 format_success_message() 函数
- [x] T100 [P] 在 `src/task-mind/init/formatter.py` 实现 format_dependency_status() 函数
- [x] T101 [P] 在 `src/task-mind/cli/init_command.py` 集成格式化的错误和成功消息
...
- [x] T103 [P] 在 `src/task-mind/init/__init__.py` 添加完整的模块文档字符串
- [x] T104 [P] 在所有 `src/task-mind/init/*.py` 文件添加类型注解（mypy 检查）
...
- [x] T109 运行完整测试套件确保 >= 80% 覆盖率：`uv run pytest --cov=task-mind.init --cov-report=term`
...
Task: "在 src/task-mind/init/checker.py 实现 check_node() 函数"
Task: "在 src/task-mind/init/checker.py 实现 check_claude_code() 函数"
Task: "在 src/task-mind/init/installer.py 实现 run_external_command() 包装器"
Task: "在 src/task-mind/init/installer.py 实现 install_node() 函数"
Task: "在 src/task-mind/init/installer.py 实现 install_claude_code() 函数"
```
...
Task: "在 src/task-mind/init/configurator.py 实现 prompt_auth_method() 使用 Click.choice"
Task: "在 src/task-mind/init/configurator.py 实现 configure_official_auth() 函数"
Task: "在 src/task-mind/init/configurator.py 实现 configure_custom_endpoint() 函数"
Task: "在 src/task-mind/init/configurator.py 实现 load_config() 和 save_config() 函数"
```
