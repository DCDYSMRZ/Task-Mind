# Feature Specification: Task-Mind 环境初始化命令

**Feature Branch**: `006-init-command`
**Created**: 2025-11-25
**Status**: Draft
**Input**: User description: "当前项目还缺少 init 命令, 该命令应该用在用户通过 uv tool install task-mind 后,对其本地环境进行初始化"
...
新用户在通过 `uv tool install task-mind` 安装 Task-Mind 后，需要立即能够使用该工具。系统应并行检测所有依赖（Node.js 和 Claude Code）的状态，根据检测结果智能决定需要安装的组件，避免不必要的操作。

**Why this priority**: 这是所有后续功能的基础。通过并行检查，用户可以快速了解当前环境状态，只安装缺失的组件，提供最优的初始化体验。

**Independent Test**: 在全新的系统上运行 `task-mind init`，系统能够：
...
1. **Given** 用户系统上既没有 Node.js 也没有 Claude Code, **When** 用户运行 `task-mind init`, **Then** 系统并行检测后显示"需要安装：Node.js 20+, Claude Code"，询问是否继续
2. **Given** 用户系统上已有 Node.js 20+ 但没有 Claude Code, **When** 用户运行 `task-mind init`, **Then** 系统显示"Node.js ✓ (v20.x), 需要安装：Claude Code"，仅安装 Claude Code
3. **Given** 用户系统上已有 Claude Code 但 Node.js 版本低于 20, **When** 用户运行 `task-mind init`, **Then** 系统提示"Claude Code ✓, Node.js 需要升级 (当前 v18.x → 要求 v20+)"
4. **Given** 用户系统上已安装 Node.js 20+ 和 Claude Code, **When** 用户运行 `task-mind init`, **Then** 系统显示"所有依赖已满足 ✓"，跳转到配置更新流程
...
**Why this priority**: 认证配置是使用 Task-Mind 的前提。通过明确的互斥选择，避免配置冲突，简化用户决策流程。

**Independent Test**: 运行 `task-mind init`，在依赖检查完成后：
1. 系统显示两种认证方式选项
2. 用户选择官方登录或自定义端点
3. 根据选择引导完成相应配置
...
用户在所有依赖已满足且配置文件已存在时重新运行 `task-mind init`，系统应显示当前配置状态，并询问用户是否需要更新特定配置项（如更换 API 端点、启用/禁用 CCR 等）。

**Why this priority**: 支持配置更新是重要的运维需求。用户可能需要切换 API 端点、更新凭证或调整设置，而无需完全重新初始化。

**Independent Test**: 在已有完整配置的系统上运行 `task-mind init`：
1. 系统检测到所有依赖已满足
2. 系统读取并显示当前配置摘要
3. 询问用户是否需要更新配置
4. 根据用户选择进入对应的更新流程
...
1. **Given** 所有依赖已安装且配置文件存在, **When** 用户运行 `task-mind init`, **Then** 系统显示"环境已配置 ✓"和配置摘要（认证方式、端点类型、CCR 状态等）
...
**Independent Test**: 运行 `task-mind init` 并选择"自定义端点"：
...
2. **Given** 用户选择 Deepseek 端点, **When** 用户输入 API Key, **Then** 系统保存端点配置到 `~/.task-mind/config.json`，包含端点类型和 API Key
...
**Independent Test**: 运行 `task-mind init` 并选择"使用 Claude Code Router"：
1. 系统询问是否安装 CCR
2. 用户同意后，系统安装 CCR
3. 系统提供 CCR 配置模板
4. 用户完成配置后，系统保存设置
...
2. **Given** CCR 安装完成, **When** 系统准备配置, **Then** 系统在 `~/.task-mind/` 下创建 CCR 配置模板文件，并提示用户编辑
3. **Given** 用户完成 CCR 配置, **When** 用户确认配置, **Then** 系统更新 `~/.task-mind/config.json`，标记 CCR 已启用
...
1. **Given** 用户完成所有 init 步骤, **When** 流程结束, **Then** 系统在 `~/.task-mind/` 目录下创建 `config.json` 文件
2. **Given** 配置文件创建完成, **When** 系统写入配置, **Then** 文件包含结构化的 JSON 数据：Node 版本、Claude Code 安装路径、登录状态、端点配置、CCR 状态等
3. **Given** 配置保存成功, **When** 系统显示摘要, **Then** 摘要包括：已安装的组件、配置的端点、下一步建议（如"运行 task-mind recipe list 查看可用配方"）
4. **Given** 配置文件已存在, **When** 用户重新运行 `task-mind init`, **Then** 系统检测到现有配置，询问是否重新配置或更新特定部分
...
- **配置文件冲突**：`~/.task-mind/config.json` 已存在但格式错误或损坏，系统应备份旧文件并创建新配置
...
- **FR-001**: 系统 MUST 在运行 `task-mind init` 时并行检查 Node.js 和 Claude Code 的安装状态和版本
...
- **FR-016**: 系统 MUST 在用户主目录下创建 `~/.task-mind/` 目录（如果不存在）
- **FR-017**: 系统 MUST 将所有用户配置保存到 `~/.task-mind/config.json` 文件，使用 JSON 格式
...
- **SC-006**: 配置文件 `~/.task-mind/config.json` 具有清晰的 JSON 结构，用户可以手动编辑而不破坏功能
- **SC-007**: 用户在配置摘要中能够清楚了解所有已配置的组件和下一步操作

## Assumptions

- 用户已通过 `uv tool install task-mind` 成功安装 Task-Mind CLI
...
- 用户主目录（`~`）可写，可以创建 `.task-mind` 目录
- 默认 Node.js 版本为 LTS 20.x 或更高版本
- 用户在首次运行 init 时没有现有的 `~/.task-mind/config.json` 文件
