...
存储用户的完整配置信息，持久化到 `~/.task-mind/config.json`。
...
保存 Ctrl+C 中断时的进度，支持恢复执行。存储于 `~/.task-mind/.init_state.json`（隐藏文件）。
...
| 文件路径 | 用途 | 格式 | 生命周期 |
|---------|------|------|---------|
| `~/.task-mind/config.json` | 主配置文件 | JSON | 永久（手动删除） |
| `~/.task-mind/.init_state.json` | 临时状态 | JSON | 7 天自动过期 |
| `~/.task-mind/config.json.bak` | 配置备份 | JSON | 覆盖时创建 |
