...
**用法**:
```bash
task-mind session list [--limit N] [--agent-type TYPE] [--json]
```
...
**用法**:
```bash
task-mind session show <session_id> [--steps] [--tools] [--json]
```
...
#### `task-mind session watch [session_id]`

实时监控当前或指定会话。

**用法**:
```bash
task-mind session watch [session_id] [--json]
```
...
#### `task-mind session clean [--before DATE] [--keep N]`

清理历史会话数据。

**用法**:
```bash
task-mind session clean [--before DATE] [--keep N] [--dry-run]
```
...
| 变量 | 默认值 | 描述 |
|------|--------|------|
| `FRAGO_SESSION_DIR` | `~/.task-mind/sessions` | 会话数据存储目录 |
