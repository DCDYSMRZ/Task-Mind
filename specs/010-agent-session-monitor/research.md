...
```
~/.claude/projects/
├── -home-yammi-repos-Task-Mind/              # 项目目录（路径编码）
│   ├── {uuid}.jsonl                      # 标准会话文件
...
**命名约定**:
- 项目目录：绝对路径用连字符编码（`/home/yammi/repos/Task-Mind` → `-home-yammi-repos-Task-Mind`）
...
| 模块 | 位置 | 职责 |
|------|------|------|
| Run 系统 | `src/task-mind/run/` | 会话管理、日志记录、上下文跟踪 |
| Agent 命令 | `src/task-mind/cli/agent_command.py` | 智能路由、流式执行 |
| GUI 系统 | `src/task-mind/gui/` | 桌面应用、历史记录 |
...
- 与现有 `task-mind run` 命令体系保持一致
- 复用 `LogEntry`、`InsightEntry` 等模型

### 2.3 会话关联难点解决方案

**问题**: `task-mind agent` 启动 Claude Code 子进程后，如何关联到正确的会话文件？

**决策**: 基于时间窗口 + 项目路径 + 文件监听的三重匹配

**方案细节**:
1. 在 `task-mind agent` 启动时记录启动时间戳 `T0`
...
- 与现有 Task-Mind 命令风格一致

---

## 4. 目录结构设计

**决策**: `~/.task-mind/sessions/{agent_type}/{session_id}/`
...
**问题**: 多个终端同时执行 `task-mind agent`，如何确保各自监控正确的会话？

**解决方案**:
- 每个 `task-mind agent` 进程独立记录启动时间戳
- 通过时间窗口过滤，只匹配自身启动后的新会话
- 在 `~/.task-mind/sessions/` 中使用不同的 session_id 目录隔离
