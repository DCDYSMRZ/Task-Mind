# task-mind Development Guidelines

> **Naming Convention**: `task-mind` is the only correct casing. NEVER USE `Task-Mind`.
>
...
CLAUDE_MD.excludes = [
    "task_mind_product_usage_instructions",
    "command_reference",
    "api_documentation",
    "agent_behavioral_specs",  # see ~/.claude/commands/task-mind/
...
```python
PROJECT_STRUCTURE = {
    "src/task-mind/":           "Core code",
    "src/task-mind/resources/": "PyPI package resources (generated by dev pack)",
}
```
...
subprocess.run(["task-mind", "agent", "--prompt-file", str(prompt_file)])
...
# See: task-mind/session/sync.py → encode_project_path, decode_project_path

# --- subprocess Calls ---
from task-mind.compat import prepare_command_for_windows
...
USER_RESOURCES = {
    "~/.claude/": {
        "commands/": {
            "task-mind.*.md":  "task-mind commands",
            "task-mind/":      "Command rules and scripts",
        },
        "skills/": {
            "task-mind-*/":    "task-mind skills",
        },
    },
    "~/.task-mind/": {
...
def dev_pack():
    sources = [
        "~/.claude/commands/task-mind.*.md",
        "~/.claude/skills/task-mind-*",
        "~/.task-mind/recipes/",
    ]
    target = "src/task-mind/resources/"

    for src in sources:
        sync(src, target)

# Usage: task-mind dev pack
```
...
DOCS = {
    ...
    "~/.claude/commands/task-mind/":    "Behavioral specifications for Agents using task-mind",
}
# Read as needed
```
...
- N/A (uses existing session storage in `~/.task-mind/sessions/`) (013-change-gui-into-web-service)
...
- File-based (existing `~/.task-mind/` directory structure) (014-server-background-redesign)
