# task-mind i18n (Internationalization) Guide

## Overview

task-mind supports bilingual interface (English/Chinese) with:
- Web UI language switching via react-i18next
- Language preference stored in `~/.task-mind/gui_config.json`
- AI prompts output respecting user's language preference

## Translation Files Location

```
src/task-mind/server/web/src/i18n/
...
```bash
cd src/task-mind/server/web && pnpm run build
```
...
3. Update `src/types/pywebview.d.ts`:
   ...
   ```
4. Update backend `src/task-mind/server/services/config_service.py`:
   ```python
...
- `src/task-mind/session/title_manager.py` - Title generation
- `src/task-mind/cli/agent_command.py` - Agent routing

Pattern for language-aware prompts:
```python
from task-mind.server.services.config_service import ConfigService
...
| File | Purpose |
|------|---------|
| `src/task-mind/server/services/config_service.py` | Language preference storage |
| `src/task-mind/server/web/src/stores/appStore.ts` | Frontend language state management |
| `src/task-mind/server/web/src/components/settings/AppearanceSettings.tsx` | Language selector UI |
