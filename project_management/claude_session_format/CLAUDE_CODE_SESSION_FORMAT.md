...
Where:
- `encoded_project_path`: Project path with `/` replaced by `-` (e.g., `/Users/chagee/Repos/task-mind` → `-Users-chagee-Repos-task-mind`)
...
**task-mind Handling**: Skipped by parser.
...
```json
{
  ...
  "isSidechain": false,
  "userType": "external",
  "cwd": "/Users/chagee/Repos/task-mind",
  "sessionId": "6c75e29d-01ec-4987-acf7-096af67c8031",
  "version": "2.0.76",
  "gitBranch": "main",
...
**task-mind Handling**: Skipped by parser, used for status inference.
...
**task-mind Handling**: Currently discarded (not stored).
...
```json
{
  ...
  "message": {
    "role": "user",
    "content": [{ "type": "text", "text": "# /task-mind.run - Research Explorer\n..." }]
  }
}
```
...
**task-mind Handling**: Sidechain sessions are skipped during sync.

---

## task-mind Implementation Reference
...
|-----------|-----------|---------|
| Record Parser | `src/task-mind/session/parser.py` | Parse JSONL records, extract content |
| Data Models | `src/task-mind/session/models.py` | SessionStep, ToolCallRecord, etc. |
| Storage | `src/task-mind/session/storage.py` | Read/write steps.jsonl, metadata.json |
| Sync Logic | `src/task-mind/session/sync.py` | Sync from Claude → task-mind format |
| GUI Models | `src/task-mind/gui/models.py` | TaskItem, TaskStep, TaskDetail |
| API Adapter | `src/task-mind/server/adapter.py` | HTTP API wrapper |

### Data Transformation

| Claude Field | task-mind Field | Notes |
|--------------|-------------|-------|
...
uv run python project_management/claude_session_format/check_claude_session_format.py ~/.claude/projects/-Users-chagee-Repos-task-mind/
```
...
Update this document and relevant task-mind code when changes are detected.
