...
Provides resource installation functionality for task-mind init command:
- Install Claude Code slash commands to ~/.claude/commands/
- Install example recipes to ~/.task-mind/recipes/
"""
...
from task_mind.init.models import InstallResult, ResourceStatus, ResourceType
...
INSTALL_TARGETS = {
    ...
    "recipes": Path.home() / ".task-mind" / "recipes",
}
...
def get_package_resources_path(resource_type: str) -> Path:
    ...
    try:
        ...
        package_files = files("task_mind.resources")
        ...
        import task_mind.resources
        base_path = Path(task_mind.resources.__file__).parent
...
def install_commands(source_dir: Optional[Path] = None, target_dir: Optional[Path] = None) -> InstallResult:
    ...
    try:
        ...
        command_files = list(source_dir.glob("task-mind.*.md"))
        if not command_files:
            result.errors.append(f"Source resource directory is empty or corrupted: no task-mind.*.md files in {source_dir}")
            return result

        # Ensure target directory exists
        target_dir.mkdir(parents=True, exist_ok=True)

        # Copy all task-mind.*.md files (always overwrite)
        for src_file in command_files:
            target_file = target_dir / src_file.name
            shutil.copy2(src_file, target_file)
            result.installed.append(src_file.name)

        # Copy task-mind/ subdirectory (if exists)
        task_mind_subdir = source_dir / "task-mind"
        if task_mind_subdir.exists() and task_mind_subdir.is_dir():
            target_task_mind_dir = target_dir / "task-mind"
            if target_task_mind_dir.exists():
                shutil.rmtree(target_task_mind_dir)
            shutil.copytree(
                task_mind_subdir,
                target_task_mind_dir,
                ignore=shutil.ignore_patterns("__pycache__", "*.pyc"),
            )
            result.installed.append("task-mind/ (subdirectory)")
...
def install_recipes(
    ...
    Args:
        source_dir: Source directory, defaults to package resources
        target_dir: Target directory, defaults to ~/.task-mind/recipes/
...
def install_all_resources(skip_recipes: bool = False, force_update: bool = False) -> ResourceStatus:
    ...
    from task_mind import __version__

    status = ResourceStatus(
        task_mind_version=__version__,
...
def count_installed_commands(target_dir: Optional[Path] = None) -> int:
    """
    Count installed task-mind commands

    Args:
        target_dir: Target directory, defaults to ~/.claude/commands/

    Returns:
        Number of installed task-mind.*.md files
    """
    if target_dir is None:
        target_dir = get_target_path("commands")
    ...
    return len(list(target_dir.glob("task-mind.*.md")))
...
def get_resources_status() -> dict:
    ...
    if commands_path.exists():
        command_files = [f.name for f in commands_path.glob("task-mind.*.md")]
...
def format_resources_status() -> str:
    ...
    lines.append(f"  Task-Mind version: {status['task_mind_version']}")
