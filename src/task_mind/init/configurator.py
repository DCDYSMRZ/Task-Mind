...
- Configuration persistence to ~/.task-mind/config.json
...
from task_mind.init.models import Config, APIEndpoint
...
def get_config_path() -> Path:
    ...
    Returns:
        Configuration file path (~/.task-mind/config.json)
    """
...
# Import from task_mind.init.config_manager instead
# These re-exports are kept for backward compatibility but will be removed in a future version
from task_mind.init.config_manager import load_config, save_config  # noqa: F401
...
def configure_custom_endpoint(existing_config: Optional[Config] = None) -> Config:
    ...
    # Create APIEndpoint object for task-mind configuration
    if endpoint_type == "custom":
    ...
    # Update task-mind configuration
    if existing_config:
        data = existing_config.model_dump()
...
def format_final_summary(config: Config) -> str:
    ...
    """
    lines = ["", "🎉 Task-Mind initialization completed!", ""]
    lines.append("=" * 40)
    lines.append("")
    ...
    if config.auth_method == "official":
        ...
        lines.append("   - Method: Task-Mind configured API endpoint")
...
def suggest_next_steps(config: Config) -> list[str]:
    ...
    if config.auth_method == "official":
        ...
        steps.append("Use `task-mind recipe list` to view available automation recipes")
    else:
        steps.append("Use `task-mind recipe list` to view available automation recipes")
        steps.append("Run `task-mind recipe run <name>` to execute a recipe")

    steps.append("View documentation: https://github.com/DCDYSMRZ/Task-Mind")
