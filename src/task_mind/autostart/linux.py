...
class LinuxAutostartManager(AutostartManager):
    ...
    'task-mind server start' at user login.
    """

    SERVICE_NAME = "task-mind-server.service"
    ...
    def _get_task_mind_path(self) -> str:
        ...
        for path in [
            Path.home() / ".local" / "bin" / "task-mind",
            Path("/usr/local/bin/task-mind"),
        ]:
            if path.exists():
                return str(path)
    ...
    def _get_python_path(self) -> str:
        """Get the Python interpreter path that can run task-mind.

        Returns the Python from the same environment where task-mind is installed.
        """
        task_mind_path = self._get_task_mind_path()

        # If task-mind is in a venv, use that venv's Python
        task_mind_bin_dir = Path(task_mind_path).parent
        possible_pythons = [
            task_mind_bin_dir / "python3",
            task_mind_bin_dir / "python",
        ]
    ...
    def _generate_service(self) -> str:
        ...
        return dedent(f"""\
            [Unit]
            Description=Task-Mind AI Automation Server
