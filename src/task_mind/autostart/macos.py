...
class MacOSAutostartManager(AutostartManager):
    ...
    def _get_task_mind_path(self) -> str:
        ...
        for path in [
            ...
            Path("/opt/homebrew/bin/task-mind"),
        ]:
            if path.exists():
                return str(path)
    ...
    def _generate_plist(self) -> dict:
        ...
        log_path = Path.home() / ".task-mind" / "launchd.log"

        return {
            "Label": self.LABEL,
            "ProgramArguments": [task_mind_path, "server", "start"],
            "RunAtLoad": True,
