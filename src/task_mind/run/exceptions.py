...
class ContextNotSetError(RunException):
    ...
    def __init__(self):
        super().__init__(
            "Current run context not set. "
            "Run 'uv run task-mind run set-context <run_id>' first."
        )
...
class ContextAlreadySetError(RunException):
    ...
    def __init__(self, existing_run_id: str):
        ...
        super().__init__(
            ...
            f"Run 'uv run task-mind run release' to release it first, "
            f"or 'uv run task-mind run set-context {existing_run_id}' to continue it."
        )
