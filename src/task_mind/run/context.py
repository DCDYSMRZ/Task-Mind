...
Responsible for reading and writing ~/.task-mind/current_run configuration file, with environment variable priority support
"""
...
class ContextManager:
    ...
    def __init__(self, task_mind_home: Path, projects_dir: Path):
        """Initialize context manager

        Args:
            task_mind_home: Task-Mind user directory (~/.task-mind)
            projects_dir: projects directory path
        """
        self.task_mind_home = task_mind_home
        self.projects_dir = projects_dir
        self.config_dir = task_mind_home
        self.config_file = self.config_dir / "current_run"

    def get_current_run(self) -> CurrentRunContext:
        """Get current run context

        Priority: TASK_MIND_CURRENT_RUN environment variable > configuration file
        ...
        env_run_id = os.getenv("TASK_MIND_CURRENT_RUN")
        if env_run_id:
