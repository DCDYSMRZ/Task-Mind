...
class RecipeRegistry:
    ...
    def _setup_search_paths(self) -> None:
        ...
        user_path = Path.home() / '.task-mind' / 'recipes'
        if user_path.exists():
            self.search_paths.append(user_path)

        # 2. Community recipes
        community_path = Path.home() / '.task-mind' / 'community-recipes'
        if community_path.exists():
            self.search_paths.append(community_path)

        # 3. Official recipes (from package resources)
        try:
            from task_mind.init.resources import get_package_resources_path
    ...
    def _get_source_label(self, path: Path) -> str:
        ...
        if 'community-recipes' in path_str:
            return 'Community'
        elif '.task-mind/recipes' in path_str or '.task-mind\\recipes' in path_str:
            return 'User'
        else:
            return 'Official'
