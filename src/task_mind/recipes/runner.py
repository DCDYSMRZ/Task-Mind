...
class RecipeRunner:
    ...
    def _run_chrome_js(
        ...
        Raises:
            RecipeExecutionError: Execution failed
        """
        # If there are parameters, inject them into window.__TASK_MIND_PARAMS__ first
        if params:
            params_json = json.dumps(params)
            inject_cmd = [
                'uv', 'run', 'task-mind', 'chrome', 'exec-js',
                f'window.__TASK_MIND_PARAMS__ = {params_json}'
            ]
            try:
        ...
        # Build command: uv run task-mind chrome exec-js <script_path> --return-value
        cmd = [
            'uv', 'run', 'task-mind', 'chrome', 'exec-js',
            str(script_path),
            '--return-value'
    ...
    def _run_shell(
        ...
        # Build command: <script_path>
        # Pass params via stdin instead of command line argument
        params_json = json.dumps(params)
        cmd = [str(script_path)]
