"""
Sync module - Sync ~/.task-mind/ as a Git repository

Treat ~/.task-mind/ as a Git working directory and sync to user-configured remote repository.
...
# ~/.task-mind/.claude/ subdirectory (Git tracked)
TASK_MIND_CLAUDE_DIR = TASK_MIND_HOME / ".claude"
TASK_MIND_SKILLS_DIR = TASK_MIND_CLAUDE_DIR / "skills"
TASK_MIND_RECIPES_DIR = TASK_MIND_HOME / "recipes"
...
# Commands directory (managed by task-mind itself, not synced)
.claude/commands/
...
"""
    ...
    if not gitignore_path.exists():
        ...
        needed_rules = [
            ...
            # Commands directory (managed by task-mind itself)
            ".claude/commands/",
            # System files
...
def _classify_file(file_path: str) -> tuple[str, str]:
    ...
    if file_path.startswith(".claude/skills/"):
        # Skills are directories: .claude/skills/task-mind-xxx/... -> task-mind-xxx
...
def _clone_or_init_repo(repo_url: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if result.returncode == 0:
            # Clone successful, move content to ~/.task-mind/
            # Preserve existing runtime directories
            preserved = {}

            for item in runtime_items:
                src = TASK_MIND_HOME / item
                if src.exists():
                    preserved[item] = temp_dir.parent / f".task_mind_preserve_{item}"
                    if src.is_file():
            ...
            # Move cloned content
            if TASK_MIND_HOME.exists():
                shutil.rmtree(TASK_MIND_HOME)
            shutil.move(str(temp_dir), str(TASK_MIND_HOME))

            # Restore runtime directories
            for item, preserved_path in preserved.items():
                target = TASK_MIND_HOME / item
        ...
        for item in runtime_items:
            preserve_path = TASK_MIND_HOME.parent / f".task_mind_preserve_{item}"
...
def _sync_claude_to_task_mind(result: SyncResult, dry_run: bool = False) -> None:
    """
    Sync changes from ~/.claude/ to ~/.task-mind/.claude/

    Check ~/.claude/skills/task-mind-*
    Only copy when version in ~/.claude/ is newer than ~/.task-mind/.claude/
    """
    # Sync skills
    if CLAUDE_SKILLS_DIR.exists():
        TASK_MIND_SKILLS_DIR.mkdir(parents=True, exist_ok=True)

        for skill_dir in CLAUDE_SKILLS_DIR.iterdir():
            if not skill_dir.is_dir():
                continue
            if not skill_dir.name.startswith("task-mind-"):
                continue

            target_skill_dir = TASK_MIND_SKILLS_DIR / skill_dir.name
            ...
            if not _dir_files_identical(skill_dir, target_skill_dir) and _is_source_newer(skill_dir, target_skill_dir):
                ...
                change = _get_file_change_info(TASK_MIND_HOME, file_path)
...
def _sync_task_mind_to_claude(result: SyncResult, dry_run: bool = False) -> None:
    """
    Sync content from ~/.task-mind/.claude/ to ~/.claude/

    After fetching updates from repository, deploy resources to Claude Code runtime directory
    """
    # Sync skills
    if TASK_MIND_SKILLS_DIR.exists():
        CLAUDE_SKILLS_DIR.mkdir(parents=True, exist_ok=True)

        for skill_dir in TASK_MIND_SKILLS_DIR.iterdir():
            if not skill_dir.is_dir():
                continue
            if not skill_dir.name.startswith("task-mind-"):
                continue
...
def _save_local_changes(result: SyncResult, message: Optional[str], dry_run: bool = False) -> bool:
    ...
    """
    if not _has_uncommitted_changes(TASK_MIND_HOME):
        return False

    changed_files = _get_changed_files(TASK_MIND_HOME)

    if dry_run:
        return True

    # git add
    _run_git(["add", "."], TASK_MIND_HOME)

    # git commit
    commit_message = message or f"sync: Save local changes ({len(changed_files)} files)"
    _run_git(["commit", "-m", commit_message], TASK_MIND_HOME)
...
def _pull_remote_updates(result: SyncResult, dry_run: bool = False) -> bool:
    ...
    remote_result = _run_git(["remote", "-v"], TASK_MIND_HOME, check=False)
    if not remote_result.stdout.strip():
        return False

    # fetch
    click.echo("Fetching latest resources from your repository...")
    fetch_result = _run_git(["fetch", "origin"], TASK_MIND_HOME, check=False)
    if fetch_result.returncode != 0:
        # Possibly new repository, no remote branch
        return False

    # Check if remote branch exists
    branch_result = _run_git(["rev-parse", "--abbrev-ref", "HEAD"], TASK_MIND_HOME, check=False)
    current_branch = branch_result.stdout.strip() or "main"

    # Check if remote branch exists
    remote_branch_result = _run_git(
        ["rev-parse", "--verify", f"origin/{current_branch}"], TASK_MIND_HOME, check=False
    )
    if remote_branch_result.returncode != 0:
        return False

    # Save current HEAD for later comparison
    old_head_result = _run_git(["rev-parse", "HEAD"], TASK_MIND_HOME, check=False)
    old_head = old_head_result.stdout.strip() if old_head_result.returncode == 0 else None

    # Try rebase
    rebase_result = _run_git(["rebase", f"origin/{current_branch}"], TASK_MIND_HOME, check=False)

    if rebase_result.returncode != 0:
        # There are conflicts
        _run_git(["rebase", "--abort"], TASK_MIND_HOME, check=False)

        # Get conflict files
        result.conflicts = _get_changed_files(TASK_MIND_HOME)
        return True

    # Get new HEAD
    new_head_result = _run_git(["rev-parse", "HEAD"], TASK_MIND_HOME, check=False)
...
def _push_to_remote(result: SyncResult, dry_run: bool = False) -> bool:
    ...
    log_result = _run_git(["log", "--oneline", "-1"], TASK_MIND_HOME, check=False)
    ...
    push_result = _run_git(["push", "-u", "origin", current_branch], TASK_MIND_HOME, check=False)
...
def sync(
    ...
    try:
        ...
        if not _is_git_repo(TASK_MIND_HOME):
            if not repo_url:
                result.errors.append("Sync repository not configured. Please configure using task-mind sync --set-repo <url>")
                return result
        ...
        if not dry_run:
            removed_env_files = _check_and_remove_tracked_env(TASK_MIND_HOME)
        ...
        if not actual_repo_url:
            ...
            remote_result = _run_git(["remote", "get-url", "origin"], TASK_MIND_HOME, check=False)
        ...
        if actual_repo_url and actual_repo_url.startswith("git@github.com:"):
            ...
            if ssh_match:
                ...
                if not dry_run:
                    ...
                    _run_git(["remote", "set-url", "origin", https_url], TASK_MIND_HOME, check=False)
        ...
        # 1. Safety check - Sync changes from ~/.claude/ to ~/.task-mind/.claude/
        click.echo("Checking local resource changes...")
        _sync_claude_to_task_mind(result, dry_run)

        # 1b. Save local changes
        if result.local_changes or _has_uncommitted_changes(TASK_MIND_HOME):
        ...
        _sync_task_mind_to_claude(result, dry_run)
