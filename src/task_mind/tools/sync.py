...
Sync Recipes from ~/.task-mind/recipes/ to src/task-mind/resources/recipes/
Sync commands from ~/.claude/commands/ to src/task-mind/resources/commands/
Sync Skills from ~/.claude/skills/ to src/task-mind/resources/skills/
...
Developers edit resources directly in ~/.claude/ and ~/.task-mind/, then use dev pack to sync to package.
"""
...
class CommandSync:
    ...
    def __init__(
        ...
        Args:
            ...
            target_dir: Target directory (src/task-mind/resources/commands/), defaults to auto-detect
        """
        # Auto-detect project root directory (for target_dir)
        current_file = Path(__file__).resolve()
        # src/task-mind/tools/sync.py -> project_root
        ...
        self.target_dir = target_dir or (
            project_root / "src" / "task-mind" / "resources" / "commands"
        )

    def find_commands(self, pattern: Optional[str] = None) -> list[Path]:
        """
        Find all task-mind.*.md command files
        ...
        # Find task-mind.*.md files (user directory uses official naming)
        for cmd_file in self.source_dir.glob("task-mind.*.md"):
    ...
    def get_target_name(self, source_name: str) -> str:
        ...
        Args:
            source_name: Source filename, e.g. task-mind.recipe.md
    ...
    def list_synced(self) -> list[Path]:
        ...
        for cmd_file in self.target_dir.glob("task-mind.*.md"):
            if cmd_file.is_file():
...
class RecipeSync:
    ...
    def __init__(
        ...
        Args:
            source_dir: Source directory (~/.task-mind/recipes/), defaults to user directory
            target_dir: Target directory (src/task-mind/resources/recipes/), defaults to auto-detect
        """
        # Auto-detect project root directory (for target_dir)
        current_file = Path(__file__).resolve()
        # src/task-mind/tools/sync.py -> project_root
        ...
        # Change source directory to user directory
        self.source_dir = source_dir or (Path.home() / ".task-mind" / "recipes")
        self.target_dir = target_dir or (
            project_root / "src" / "task-mind" / "resources" / "recipes"
        )
...
class SkillSync:
    ...
    def __init__(
        ...
        Args:
            ...
            target_dir: Target directory (src/task-mind/resources/skills/), defaults to auto-detect
        """
        # Auto-detect project root directory (for target_dir)
        current_file = Path(__file__).resolve()
        # src/task-mind/tools/sync.py -> project_root
        ...
        self.target_dir = target_dir or (
            project_root / "src" / "task-mind" / "resources" / "skills"
        )

    def find_skills(self, pattern: Optional[str] = None) -> list[Path]:
        """
        Find all Skill directories (starting with task-mind-)
        ...
        for skill_dir in self.source_dir.iterdir():
            ...
            if skill_dir.name == "__pycache__":
                continue
            # Only sync skills starting with task-mind-
