#!/usr/bin/env python3
"""
Task-Mind CLI command implementations
...
COMMAND_EXAMPLES = {
    ...
    "navigate": [
        ...
        "task-mind chrome navigate https://example.com --wait-for '.content-loaded'",
    ],
    "click": [
        "task-mind chrome click <selector>",
        "task-mind chrome click 'button.submit'",
        "task-mind chrome click '#login-btn' --wait-timeout 15",
    ],
    "screenshot": [
        "task-mind chrome screenshot <output_file>",
        "task-mind chrome screenshot page.png",
        "task-mind chrome screenshot full.png --full-page --quality 90",
    ],
    "exec-js": [
        "task-mind chrome exec-js <script>",
        "task-mind chrome exec-js 'document.title'",
        "task-mind chrome exec-js 'return window.scrollY' --return-value",
        "task-mind chrome exec-js ./script.js  # Load from file",
    ],
    "get-title": [
    ...
    "get-content": [
        ...
        "task-mind chrome get-content  # Default: get body",
        "task-mind chrome get-content 'article.main' --desc 'article-content'",
    ],
    "scroll": [
        "task-mind chrome scroll <distance>",
        "task-mind chrome scroll 500      # Scroll down 500px",
        "task-mind chrome scroll -300     # Scroll up 300px",
        "task-mind chrome scroll down     # Alias: scroll down 500px",
        "task-mind chrome scroll up       # Alias: scroll up 500px",
    ],
    "scroll-to": [
        "task-mind chrome scroll-to <selector>",
        "task-mind chrome scroll-to 'article'",
        "task-mind chrome scroll-to --text 'Section Title'",
        "task-mind chrome scroll-to '#footer' --block end",
    ],
    "wait": [
    ...
    "zoom": [
        "task-mind chrome zoom <factor>",
        "task-mind chrome zoom 1.5   # Zoom to 150%",
        "task-mind chrome zoom 0.8   # Zoom to 80%",
        "task-mind chrome zoom 1     # Reset to original size",
    ],
    "clear-effects": [
        "task-mind chrome clear-effects",
    ],
    "highlight": [
        "task-mind chrome highlight <selector>",
        "task-mind chrome highlight 'button.primary'",
        "task-mind chrome highlight '#target' --color red --width 5",
        "task-mind chrome highlight '.element' --longlife  # Permanent display",
    ],
    "pointer": [
        "task-mind chrome pointer <selector>",
        "task-mind chrome pointer 'button.submit'",
        "task-mind chrome pointer '#element' --life-time 10",
    ],
    "spotlight": [
        "task-mind chrome spotlight <selector>",
        "task-mind chrome spotlight '.highlight-me'",
        "task-mind chrome spotlight '#focus' --longlife",
    ],
    "annotate": [
        "task-mind chrome annotate <selector> <text>",
        "task-mind chrome annotate 'button' 'Click here'",
        "task-mind chrome annotate '#form' 'Fill this' --position bottom",
    ],
    "underline": [
        "task-mind chrome underline <selector>",
        "task-mind chrome underline 'article p'",
        "task-mind chrome underline --text 'Important text'",
        "task-mind chrome underline '.content' --color blue --width 2",
    ],
    "start": [
        "task-mind chrome start",
        "task-mind chrome start --headless",
        "task-mind chrome start --void --keep-alive",
        "task-mind chrome start --port 9333 --width 1920 --height 1080",
    ],
    "stop": [
        "task-mind chrome stop",
        "task-mind chrome stop --port 9333",
    ],
    "list-tabs": [
        "task-mind chrome list-tabs",
    ],
    "switch-tab": [
        "task-mind chrome switch-tab <tab_id>",
        "task-mind chrome switch-tab ABC123  # Supports partial ID matching",
    ],
    # Top-level commands
    "status": [
        "task-mind status",
        "task-mind chrome status  # Equivalent",
    ],
    "init": [
        "task-mind init",
        "task-mind init --force",
    ],
    # Chrome command group itself (showing subcommand overview)
    "chrome": [
        "task-mind chrome <command>",
        "task-mind chrome start      # Start browser",
        "task-mind chrome navigate   # Navigate to URL",
        "task-mind chrome click      # Click element",
        "task-mind chrome screenshot # Take screenshot",
    ],
    # Task-Mind top-level commands
    "task-mind": [
        "task-mind <command>",
        "task-mind chrome start      # Start browser",
        "task-mind status            # Check CDP connection status",
        "task-mind --gui             # Launch GUI app",
    ],
}
    ...
    def convert(self, value, param, ctx):
        ...
        try:
            ...
            self.fail(
                ...
                "Correct usage:\n"
                "  task-mind scroll 500       # Scroll down 500 pixels\n"
                "  task-mind scroll -300      # Scroll up 300 pixels\n"
                "  task-mind scroll down      # Scroll down 500 pixels (alias)\n"
                "  task-mind scroll up        # Scroll up 500 pixels (alias)\n"
                "  task-mind scroll-to 'selector'  # Scroll to element",
                param,
                ctx,
            )
    ...
    def convert(self, value, param, ctx):
        try:
            ...
            self.fail(
                ...
                "Correct usage:\n"
                "  task-mind zoom 1.5    # Zoom to 150%\n"
                "  task-mind zoom 0.8    # Zoom to 80%\n"
                "  task-mind zoom 1      # Reset to original size",
                param,
                ctx,
            )
    ...
    def convert(self, value, param, ctx):
        try:
            ...
            self.fail(
                ...
                "Correct usage:\n"
                "  task-mind wait 2      # Wait 2 seconds\n"
                "  task-mind wait 0.5    # Wait 0.5 seconds",
                param,
                ctx,
            )
...
def _get_projects_dir() -> Path:
    ...
    Uses ~/.task-mind/projects/
    """
    return Path.home() / ".task-mind" / "projects"
...
def _get_run_dir() -> Path:
    ...
    """
    task_mind_home = Path.home() / ".task-mind"
    projects_dir = _get_projects_dir()

    try:
        from ..run.context import ContextManager
        ctx_mgr = ContextManager(task_mind_home, projects_dir)
...
def scroll_to(ctx, selector: Optional[str], text: Optional[str], block: str = 'center'):
    ...
    \b
    Examples:
      task-mind scroll-to "article"                    # CSS selector
      task-mind scroll-to --text "Just canceled"       # Find by text
    """
    import json
...
def underline(ctx, selector: Optional[str], text: Optional[str], color: str, width: int, duration: int, life_time: int, longlife: bool):
    ...
    \b
    Examples:
      task-mind underline "article"                    # CSS selector
      task-mind underline --text "Just canceled"       # Find by text
    """
    import json
...
(function(selector, searchText, color, width, duration, lifetime) {
    ...
    elements.forEach(el => {
        ...
        lines.forEach((line, index) => {
            ...
            u.className = 'task-mind-underline';
...
def init(force: bool):
    """
    Initialize Task-Mind user-level directory structure

    Creates ~/.task-mind/recipes/ directory and subdirectories:
    ...
    user_home = Path.home()
    task_mind_dir = user_home / '.task-mind'
    recipes_dir = task_mind_dir / 'recipes'
    ...
    click.echo("Use 'task-mind recipe copy <name>' to copy sample Recipes to this directory")
...
@click.option(
    '--profile-dir',
    type=click.Path(),
    help='Chrome user data directory (default ~/.task-mind/chrome_profile)'
)
@click.option(
    '--no-kill',
    is_flag=True,
...
def chrome_start(headless: bool, void: bool, port: int, width: int, height: int,
    ...
    \b
    Examples:
      task-mind chrome                    # Normal launch
      task-mind chrome --headless         # Headless mode
      task-mind chrome --void             # Void mode
      task-mind chrome --port 9333        # Use different port
      task-mind chrome --keep-alive       # Keep running after launch
    """
