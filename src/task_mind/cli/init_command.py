"""
task-mind init command implementation
...
def print_banner() -> None:
    ...
    lines = TASK_MIND_BANNER.rstrip().split("\n")
...
def init(
    ...
    """
    Initialize Task-Mind development environment
    ...
    print_banner()
    print_section("Task-Mind Environment Initialization")
...
def _offer_autostart_configuration() -> None:
    """Offer to configure task-mind server autostart on system boot."""
    try:
        from task_mind.autostart import get_autostart_manager
        from task_mind.init.ui import ask_question
        ...
        if is_enabled:
            ...
            options = [
                {
                    "label": "Enable",
                    "description": "Start task-mind server automatically on system boot"
                },
                {
                    "label": "Skip",
                    "description": "Configure later with 'task-mind autostart enable'"
                }
            ]

        answer = ask_question(
            question="Configure task-mind server autostart?",
        ...
        if answer == "Enable":
            ...
            click.echo("  Run 'task-mind autostart enable' to configure later")
...
def _print_completion_summary(config: Config) -> None:
    ...
    if config.auth_method == "official":
        ...
        items.append(("Authentication", f"Task-Mind managed ({endpoint_type})"))

    print_summary(items, "Configuration")

    click.secho("Run 'task-mind --help' to get started", fg="cyan")
    click.echo()
...
def _show_current_config() -> None:
    ...
    if not config_exists():
        print_section("Task-Mind Configuration")
        click.secho("Not initialized. Run 'task-mind init' to configure.", dim=True)
        click.echo()
        return
...
def _show_restart_required_message(remaining_deps: list) -> None:
    ...
    """
    from task_mind.init.installer import _get_shell_config_file
    ...
    click.secho("    task-mind init", fg="cyan")
    click.echo()
...
def _install_resources(force_update: bool = False) -> bool:
    ...
    try:
        ...
        click.secho("  Ensure write permissions for ~/.claude/ and ~/.task-mind/", dim=True, err=True)
        return False
