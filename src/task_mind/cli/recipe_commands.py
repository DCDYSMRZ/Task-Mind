...
from task_mind.recipes import RecipeRegistry, RecipeRunner, OutputHandler
from task_mind.recipes.exceptions import RecipeError, MetadataParseError, RecipeValidationError
from task_mind.recipes.metadata import parse_metadata_file, validate_metadata
...
def install_recipe(source: str, force: bool, name_override: Optional[str], output_format: str):
    ...
    \b
    - community:<name>     Install from task-mind community recipes
    - /path/to/recipe      Install from local directory

    \b
    Examples:
      task-mind recipe install community:stock-monitor
      task-mind recipe install /path/to/recipe --name custom-name
      task-mind recipe install community:stock-monitor --force
    """
    from task_mind.recipes.installer import RecipeInstaller
    from task_mind.recipes.exceptions import RecipeAlreadyExistsError, RecipeInstallError
    ...
    try:
        ...
        if output_format == 'json':
            ...
            click.echo()
            click.echo(f"Run 'task-mind recipe info {recipe_name}' to see details")
...
def uninstall_recipe(name: str, yes: bool, output_format: str):
    ...
    \b
    Examples:
      task-mind recipe uninstall stock-monitor
      task-mind recipe uninstall stock-monitor --yes
    """
    from task_mind.recipes.installer import RecipeInstaller
...
def update_recipe(name: Optional[str], update_all: bool, output_format: str):
    ...
    \b
    Examples:
      task-mind recipe update stock-monitor
      task-mind recipe update --all
    """
    from task_mind.recipes.installer import RecipeInstaller
    from task_mind.recipes.exceptions import RecipeInstallError
...
def search_recipes(query: Optional[str], output_format: str):
    ...
    \b
    Examples:
      task-mind recipe search twitter
      task-mind recipe search "twitter|x"
      task-mind recipe search
    """
    from task_mind.recipes.installer import RecipeInstaller
    ...
    if output_format == 'json':
        ...
        click.echo("Install with: task-mind recipe install community:<name>")
...
def share_recipe(name: str, yes: bool, output_format: str):
    ...
    2. Fork the task-mind repository (if needed)
    ...
    \b
    Examples:
      task-mind recipe share my-recipe
      task-mind recipe share my-recipe --yes
    """
    import subprocess
    import tempfile
    ...
    try:
        ...
        if output_format == 'json':
            ...
            click.echo("Only user recipes (in ~/.task-mind/recipes/) can be shared", err=True)
        sys.exit(1)
    ...
    try:
        ...
        if output_format == 'json':
            ...
            click.echo(f"Run: task-mind recipe validate {recipe.base_dir}", err=True)
        sys.exit(1)
    ...
    from task_mind.init.config_manager import load_config
    config = load_config()
...
*Submitted via `task-mind recipe share`*
"""
            result = run_cmd([
                "gh", "pr", "create",
                "--repo", UPSTREAM_REPO,
