#!/usr/bin/env python3
"""
Task-Mind Session Commands - Session Management Command Group
...
from task_mind.session.formatter import (
    TerminalFormatter,
    format_duration,
    format_timestamp,
    get_step_icon,
...
from task_mind.session.models import AgentType, SessionStatus, StepType
from task_mind.session.storage import (
    clean_old_sessions,
    delete_session,
    get_session_data,
    list_sessions,
...
def list_cmd(
    ...
    Examples:
      ...
      task-mind session list --agent-type claude
      task-mind session list --status running
      task-mind session list --limit 20 --json
    """
    # Convert parameters
...
def show_cmd(
    ...
    Examples:
      ...
      task-mind session show 48c10a46 --json
    """
...
def watch_cmd(
    ...
    \b
    Examples:
      task-mind session watch              # Monitor latest active session
      task-mind session watch 48c10a46     # Monitor specified session
      task-mind session watch --json       # Output in JSON format
    """
    from task_mind.session.monitor import watch_latest_session, watch_session
...
def clean_cmd(
    ...
    \b
    Examples:
      task-mind session clean              # Clean sessions older than 30 days
      task-mind session clean --days 7     # Clean sessions older than 7 days
      task-mind session clean --dry-run    # Preview sessions to be deleted
    """
...
def delete_cmd(
    ...
    \b
    Examples:
      task-mind session delete 48c10a46
      task-mind session delete 48c10a46 --force
    """
    agent = AgentType(agent_type)
...
def sync_cmd(
    ...
    Syncs session files from ~/.claude/projects/ to ~/.task-mind/sessions/claude/.
    By default, only syncs the project corresponding to the current working directory.

    \b
    Examples:
      task-mind session sync           # Sync current project
      task-mind session sync --all     # Sync all projects
      task-mind session sync --force   # Force re-sync
    """
    import os

    from task_mind.session.sync import sync_all_projects, sync_project_sessions
