"""Task-Mind server command group - Background web service management.

Provides commands to start, stop, and check status of the Task-Mind
...
def server_group(ctx: click.Context, debug: bool) -> None:
    """Manage the Task-Mind web service.
    ...
    \b
    Examples:
        task-mind server              # Start in background
        task-mind server --debug      # Start in foreground with logs
        task-mind server stop         # Stop the running server
        task-mind server restart      # Restart the server
...
def start(debug: bool) -> None:
    """Start the Task-Mind web service.
...
def _run_background() -> None:
    ...
    from task_mind.server.daemon import start_daemon
...
def _run_foreground() -> None:
    ...
    from task_mind.server.daemon import SERVER_HOST, SERVER_PORT, is_server_running
    ...
    click.echo("  Task-Mind Web Service (Debug Mode)")
    click.echo("  ─────────────────────────────────")
...
def stop() -> None:
    """Stop the running Task-Mind web service."""
    from task_mind.server.daemon import stop_daemon
...
def status() -> None:
    """Check if the Task-Mind web service is running."""
    from task_mind.server.daemon import get_server_status

    status_info = get_server_status()

    if status_info["running"]:
        click.echo("Task-Mind server is running")
        ...
        click.echo("Task-Mind server is not running")
        raise SystemExit(1)
