...
PROJECTS_DIR = TASK_MIND_HOME / "projects"
...
def get_context_manager() -> ContextManager:
    ...
    return ContextManager(TASK_MIND_HOME, PROJECTS_DIR)
...
def run_group():
    ...
    \b
    Typical workflow:
    1. Create run: task-mind run init "task description"
    2. Set context: task-mind run set-context <run_id>
    3. Log entry: task-mind run log --step "..." --status "success" ...
    4. Check progress: task-mind run info <run_id>
    5. Archive: task-mind run archive <run_id>
    """
    pass
...
def init(description: str, run_id: Optional[str]):
    ...
    \b
    Examples:
        task-mind run init "Search Python jobs on Upwork"
        task-mind run init "Test task" --run-id custom-id
    """
    try:
        manager = get_manager()
...
def set_context(run_id: str):
    ...
    \b
    Examples:
        task-mind run set-context find-job-on-upwork
    """
    try:
        manager = get_manager()
...
def list(format: str, status: str):
    ...
    Examples:
        task-mind run list
        task-mind run list --format json --status active
    """
    try:
        manager = get_manager()
...
def info(run_id: str, format: str):
    ...
    \b
    Examples:
        task-mind run info find-job-on-upwork
        task-mind run info find-job-on-upwork --format json
    """
    try:
        manager = get_manager()
        instance = manager.find_run(run_id)
...
def archive(run_id: str):
    ...
    \b
    Examples:
        task-mind run archive find-job-on-upwork
    """
    try:
        manager = get_manager()
...
def log(step: str, status: str, action_type: str, execution_method: str, data: str, insight: tuple):
    ...
    Examples:
        task-mind run log \\
          ...
          --data '{"command": "task-mind chrome navigate https://upwork.com"}'
...
def screenshot(description: str):
    ...
    \b
    Examples:
        task-mind run screenshot "Search results page"
    """
    try:
        # Get current context
        context_mgr = get_context_manager()
