"""sync command - Synchronize ~/.task-mind/ resources to your repository"""
...
from task_mind.tools.sync_repo import SyncResult, sync
...
def _format_result(result: SyncResult, dry_run: bool) -> None:
    ...
    if result.conflicts:
        ...
        click.echo()
        click.echo("Please manually resolve conflicts in ~/.task-mind/ directory and sync again")
...
def sync_cmd(
    ...
    Sync Task-Mind resources from ~/.task-mind/ and ~/.claude/ to the configured repository,
    ...
    \b
    First-time usage:
      task-mind sync --set-repo https://github.com/user/my-resources.git

    \b
    Daily usage:
      task-mind sync              # Sync resources
      task-mind sync --dry-run    # Preview what will be synced
      task-mind sync --no-push    # Only fetch updates, do not push
    ...
    try:
        ...
        if not repo_url:
            ...
            click.echo("  task-mind sync --set-repo https://github.com/user/my-resources.git", err=True)
            sys.exit(1)
