#!/usr/bin/env python3
"""
Task-Mind Agent Command - Execute non-interactive AI tasks via Claude CLI

Authentication strategy:
Based on ~/.task-mind/config.json configuration written by `task-mind init`:
...
def get_task_mind_config_path() -> Path:
    """Get task-mind configuration file path"""
    return Path.home() / ".task-mind" / "config.json"


def load_task_mind_config() -> Optional[dict]:
    """
    Load task-mind configuration

    Returns:
        Configuration dict, or None if not found or corrupted
    """
    config_path = get_task_mind_config_path()
...
def should_use_ccr(config: Optional[dict], force_ccr: bool = False) -> Tuple[bool, Optional[dict]]:
    ...
    Args:
        config: task-mind configuration
...
def get_available_slash_commands() -> dict:
    """
    Get all available task-mind slash commands

    Returns:
        Command name to description mapping, e.g. {"/task-mind.dev.run": "Execute AI-hosted..."}
    """
    commands = {}
    ...
    for search_dir in search_dirs:
        ...
        # Find task-mind.*.md files
        for md_file in search_dir.glob("task-mind*.md"):
            cmd_name = "/" + md_file.stem  # task-mind.dev.run.md → /task-mind.dev.run
...
def _build_agent_prompt(user_prompt: str) -> str:
    ...
    key_commands = ["/task-mind.run", "/task-mind.do", "/task-mind.recipe", "/task-mind.test"]
    ...
    commands_to_send = f"""# Task-Mind Agent
...
- **Explore/Research/Learn** → /task-mind.run
- **Execute/Complete/Do** → /task-mind.do
- **Create Recipe/Automation** → /task-mind.recipe
- **Test/Verify Recipe** → /task-mind.test
...
def agent(
    ...
    - Explore/Research → /task-mind.run
    - Execute Task → /task-mind.do
    - Create Recipe → /task-mind.recipe
    - Test Recipe → /task-mind.test

    \b
    Examples:
      task-mind agent Help me find Python jobs on Upwork
      task-mind agent Research YouTube subtitle extraction API
      task-mind agent Write a recipe to extract Twitter comments
      task-mind agent --direct List current directory     # Skip mode determination, execute directly
    ...
    if prompt_file:
        ...
        # Interactive mode for resume: prompt user to input
        click.echo(f"Resuming session {resume[:8]}...")
        click.echo("Enter your message (press Ctrl+D when done):")
        prompt_text = click.get_text_stream('stdin').read().strip()
        if not prompt_text:
            click.echo("No input provided, exiting.", err=True)
            sys.exit(0)
    else:
    ...
    # Step 2: Load task-mind configuration
    task_mind_config = load_task_mind_config()
    if task_mind_config:
        auth_method = task_mind_config.get("auth_method", "official")
        if auth_method == "official":
            click.echo("[OK] Authentication: Claude CLI native")
        else:
            click.echo("[OK] Authentication: Custom API endpoint")
    else:
        click.echo("[!] Task-Mind config not found, using Claude CLI default authentication")
        click.echo("  Tip: Run 'task-mind init' to initialize configuration")

    # Step 3: Determine whether to use CCR
    env = os.environ.copy()
    use_ccr_mode, ccr_info = should_use_ccr(task_mind_config, use_ccr)
    ...
    monitor = None
    monitor_enabled = not no_monitor and os.environ.get("TASK_MIND_MONITOR_ENABLED", "1") != "0"

    if monitor_enabled:
        try:
            from task_mind.session.monitor import SessionMonitor

            start_time = datetime.now(timezone.utc)
            project_path = os.getcwd()

            from task_mind.session.models import SessionSource
...
def agent_status():
    ...
    # Load task-mind configuration
    click.echo("Task-Mind Configuration:")
    task_mind_config = load_task_mind_config()
    if task_mind_config:
        auth_method = task_mind_config.get("auth_method", "official")
        ccr_enabled = task_mind_config.get("ccr_enabled", False)
        init_completed = task_mind_config.get("init_completed", False)

        click.echo(f"  Config file: {get_task_mind_config_path()}")
        ...
        click.echo(f"  Tip: Run 'task-mind init' to initialize configuration")

    click.echo()

    # Check CCR status (if enabled)
    if task_mind_config and task_mind_config.get("ccr_enabled"):
