#!/usr/bin/env python3
"""
Task-Mind CLI - Chrome DevTools Protocol command-line interface
...
@click.version_option(version=__version__, prog_name="task-mind")
@click.option(
    '--gui',
    is_flag=True,
    help='[DEPRECATED] Use "task-mind server start" instead'
)
@click.option(
    '--gui-background',
    is_flag=True,
...
def cli(ctx, gui: bool, gui_background: bool, debug: bool, timeout: int, host: str, port: int,
    ...
    """
    Task-Mind - AI Agent Multi-Runtime Automation Infrastructure
    ...
    \b
    GUI Mode (deprecated):
      task-mind --gui    Use 'task-mind server start' instead
    """
    ...
    if gui:
        ...
        click.echo("  task-mind server            # Start the web server")
        click.echo("  task-mind server stop       # Stop the web server")
        click.echo("  task-mind server status     # Check server status")
        click.echo("")
    ...
    if gui_background:
        ...
        click.echo("Please use 'task-mind server start' instead.")
        return
