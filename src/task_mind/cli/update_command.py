"""update command - self-update task-mind"""
...
from task_mind.init.checker import compare_versions


PACKAGE_NAME = "task-mind-cli"  # PyPI package name
TOOL_NAME = "task-mind"  # CLI command name / entry point
REPO_URL = "git+https://github.com/DCDYSMRZ/Task-Mind.git"
PYPI_URL = f"https://pypi.org/pypi/{PACKAGE_NAME}/json"


def is_tool_installed() -> bool:
    """Check if task-mind is installed via uv tool"""
    try:
...
def update(check_only: bool, reinstall: bool, from_repo: bool):
    """
    Update task-mind to the latest version

    Defaults to updating from PyPI, use --repo to install latest code from GitHub repository.

    \b
    Examples:
      task-mind update              # Update from PyPI
      task-mind update --repo       # Update from GitHub repository
      task-mind update --check      # Check if updates are available on PyPI
      task-mind update --reinstall  # Force reinstall
    """
    ...
    if check_only:
        ...
        if latest:
            ...
            if cmp >= 0:
                ...
                click.echo(f"Run 'task-mind update' to update")
        else:
