...
task-mind chrome navigate "https://example.com/api/v2/docs"

# ❌ Prohibited: Have you verified the parameters?
task-mind chrome navigate "https://upwork.com/search?q=python&sort=relevance"

# ❌ Prohibited: Fabricated user page
task-mind chrome navigate "https://twitter.com/elonmusk/status/123456789"
```
...
task-mind chrome navigate "<real URL from previous step>"
```
...
```bash
# ✅ Use Google search
task-mind chrome navigate "https://google.com/search?q=site:example.com+api+documentation"

# Then extract links from search results
task-mind chrome exec-js "Array.from(document.querySelectorAll('a[href]')).filter(a => a.href.includes('example.com')).map(a => a.href)" --return-value
```
