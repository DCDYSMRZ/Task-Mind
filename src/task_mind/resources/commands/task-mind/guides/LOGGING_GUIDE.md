...
The following situations **must** manually call `task-mind run log`:
...
1. `command` - CLI command execution (e.g., `task-mind chrome navigate`)
...
```bash
# Execute command
task-mind chrome navigate https://upwork.com/search
...
task-mind run log \
  ...
  --data '{"command": "task-mind chrome navigate https://upwork.com/search", "exit_code": 0}'
```
...
```bash
# Execute Recipe
task-mind recipe run upwork_extract_job_list --params '{"keyword": "Python"}'
...
```bash
# Save script
cat > ~/.task-mind/projects/<id>/scripts/filter_jobs.py <<EOF
import json
...
task-mind run log \
  ...
  --data '{
    ...
    "language": "python",
    "command": "uv run python ~/.task-mind/projects/<id>/scripts/filter_jobs.py",
    "exit_code": 0,
...
## IV. _insights Mandatory Recording (/task-mind.run Exclusive)
...
task-mind chrome click '.job-card'  # Failed, auto-record error log
...
task-mind run log \
  ...
  --execution-method "analysis" \
  --data '{
    "command": "task-mind chrome click .job-card",
...
## V. Research Success Criteria (/task-mind.run Exclusive)
...
1. **Simple commands**: Use `task-mind <command>` directly, record as `execution_method: command`
...
# ✅ Correct approach
# 1. Save script
with open('~/.task-mind/projects/<id>/scripts/filter_jobs.py', 'w') as f:
