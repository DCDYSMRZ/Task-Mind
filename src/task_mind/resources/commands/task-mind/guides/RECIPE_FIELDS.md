...
Applies to: `/task-mind.recipe`, `/task-mind.test`

## Validation Commands

```bash
task-mind recipe validate <recipe directory or recipe.md path>
task-mind recipe validate <path> --format json
```
...
Recipes can declare required environment variables, which will be automatically loaded from `~/.task-mind/.env` at runtime.
...
### ~/.task-mind/.env Configuration

Configure commonly used environment variables in `~/.task-mind/.env`:
...
`task-mind recipe validate` checks the following:
...
env:
  API_KEY:
    required: true
    description: "API key, configure in ~/.task-mind/.env"
  TIMEOUT:
    required: false
    default: "30"
