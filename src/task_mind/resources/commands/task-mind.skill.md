...
high-freq code → recipe (/task-mind.recipe)
...
if UNVERIFIED_IDEA:
    use VALIDATE_IDEA:
        ...
        2. prototype: test key steps (task-mind tools, API calls, etc.)
        3. iterate: refine based on results
        for each step that works:
            if HIGH_FREQ && SCRIPTABLE → create recipe now (/task-mind.recipe)
...
if VALIDATED or VERIFIED_METHOD:
    use IDENTIFY_AUTOMATION:
        for each step:
            if HIGH_FREQ && SCRIPTABLE → recipe (/task-mind.recipe)
...
if SCOPED:
    use NAMING:
        format: <domain>-<action>-<target>
            good: task-mind-browser-automation, git-linear-history
            bad: my-skill, skill1 (non-descriptive)

if NAMED:
    use GENERATE:
        location decision:
            task-mind-specific → ~/.claude/skills/task-mind-<name>/
...
<commands>
/task-mind.skill "<context or method>"      => analyze and create skill
/task-mind.skill from conversation          => extract patterns from current chat
/task-mind.skill update <name>              => iterate existing skill
</commands>
