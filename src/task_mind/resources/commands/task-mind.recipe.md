...
# /task-mind.recipe - Recipe Creator
...
~/.claude/commands/task-mind/scripts/recipe_workflow.sh
...
Atomic:
    ...
    SUPPORTED DIRS (ONLY these two):
        ~/.task-mind/recipes/atomic/chrome/   => runtime: chrome-js
        ~/.task-mind/recipes/atomic/system/   => runtime: python / shell

Workflow:
    use: orchestrate multiple recipes
    runtime: python
    dir: ~/.task-mind/recipes/workflows/
</types>
...
if EXPLORING:
    for each step:
        execute => wait => verify
        run `task-mind chrome click '[selector]'`
        run `task-mind chrome wait 0.5`
        run `task-mind chrome screenshot /tmp/step1.png`
        run `task-mind chrome exec-js "document.querySelector('.success') !== null" --return-value`
...
if READY:
    use GENERATE_FILES:
        Atomic (chrome):
            ~/.task-mind/recipes/atomic/chrome/<name>/
                recipe.md (metadata)
                recipe.js (script)

        Atomic (system):
            ~/.task-mind/recipes/atomic/system/<name>/
                recipe.md (metadata)
                recipe.py or recipe.sh (script)

        Workflow:
            ~/.task-mind/recipes/workflows/<name>/
                recipe.md (metadata)
                recipe.py (script)
                examples/ (optional)
    ...
    1. declare in recipe.md frontmatter:
       ...
       env:
         ...
         TIMEOUT:
           default: "30"
       ```
    2. add actual values to ~/.task-mind/.env (auto-loaded by task-mind recipe run)
</env_vars>
...
❌ CLI env override: `task-mind recipe run <name> -e KEY=VALUE`
❌ hardcode secrets in recipe scripts

✅ ONLY USE: `task-mind recipe run <name> --params '{...}'`
</forbidden>
...
<commands>
/task-mind.recipe create "<desc>"           => create Atomic
/task-mind.recipe create workflow "<desc>"  => create Workflow
/task-mind.recipe update <name> "<reason>"  => update recipe
/task-mind.recipe list                      => list all
/task-mind.recipe validate <path>           => validate
</commands>
...
from task-mind.recipes import RecipeRunner, RecipeExecutionError
import json
import sys
