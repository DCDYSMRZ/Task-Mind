...
# /task-mind.run - Research Explorer
...
if EXISTING_PROJECT_FOUND:
    use REUSE:
        run `task-mind run info <project_id>`
        run `cat ~/.task-mind/projects/<id>/logs/execution.jsonl | jq`
...
FORMAT:
    run `task-mind run log --step "..." --status warning --action-type analysis --execution-method analysis --data '{"_insights": [{"type": "pitfall", "summary": "..."}]}'`
</insights_mandatory>
