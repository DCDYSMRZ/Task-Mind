---
description: "Execute one-time complex tasks (using complete task-mind toolset)"
---

# /task-mind.do - Task Executor
...
~/.claude/commands/task-mind/rules/EXECUTION_PRINCIPLES.md
~/.claude/commands/task-mind/rules/NAVIGATION_RULES.md
~/.claude/commands/task-mind/rules/SCREENSHOT_RULES.md
~/.claude/commands/task-mind/rules/TOOL_PRIORITY.md
~/.claude/commands/task-mind/rules/WORKSPACE_RULES.md
~/.claude/commands/task-mind/guides/LOGGING_GUIDE.md
~/.claude/commands/task-mind/guides/SELECTOR_PRIORITY.md
...
if START:
    use ENV_SENSE (via Task tool, subagent_type=Explore):
        run `task-mind skill list | grep -E "keyword1|keyword2"`
        run `task-mind recipe list | grep -E "keyword1|keyword2"`
        run `task-mind run list | grep -E "keyword1|keyword2"`
        if SKILL_MATCH:
...
if TASK_RECEIVED:
    use CLARIFY:
        ...
        if BROWSER_NEEDED:
            ...
            run `task-mind chrome start`  # or --headless
            <info>
            TIP: first try `task-mind recipe list | grep <keyword>` to find existing recipes,
...
if READY:
    use PROJECT_INIT:
        ...
        run `task-mind run init "<desc>"`
        run `task-mind run set-context <id>`

if EXECUTING:
    PRIORITY: Recipe > task-mind > system > Claude tools
    run `task-mind recipe list | grep keyword`
    run `task-mind recipe run <name> --params '{}' --output-file result.json`

if DONE:
    use SAVE:
        run `task-mind run screenshot "result"`
        run `task-mind run log --step "done" --status success --action-type analysis --execution-method analysis --data '{"task_completed": true}'`

    use METADATA_UPDATE:
        run `cat ~/.task-mind/projects/<id>/.metadata.json`
        ...
        <info>
        WHY: agent can learn from `task-mind run info <id>` for future similar tasks
        </info>

    use RELEASE:
        run `task-mind run release`
...
TOOL_PRIORITY:
    Recipe > task-mind > get-content > screenshot
    => rules/TOOL_PRIORITY.md

WORKSPACE_ISOLATION:
    all outputs in ~/.task-mind/projects/<id>/
...
if Markdown or HTML:
    read ~/.claude/skills/task-mind-previewable-content/SKILL.md
    <info>
    HOW-TO:
...
<completion_summary>
    ...
    Logs: ~/.task-mind/projects/<id>/logs/execution.jsonl
</completion_summary>
