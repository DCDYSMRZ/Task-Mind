...
**Recipe Executor Note**: The generated recipe is essentially JavaScript code that is injected into the browser via CDP's Runtime.evaluate interface. Therefore, the standard way to execute the recipe is to use the `uv run task-mind chrome exec-js` command.

1. Ensure Chrome is started with CDP debugging port enabled (default 9222)
2. Navigate to the target tweet page:
   ```bash
   uv run task-mind chrome navigate "https://x.com/username/status/123456789"
   ```
3. Execute the recipe:
   ```bash
   # Inject the recipe JS file as a script into the browser and get the return value
   uv run task-mind chrome exec-js src/task-mind/recipes/x_extract_tweet_with_comments.js --return-value
   ```
...
- If X.com redesign causes script failure, use `/task-mind.recipe update x_extract_tweet_with_comments` to update the recipe
