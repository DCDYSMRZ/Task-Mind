...
uv run task-mind recipe run file_copy --params '{"source_path": "/path/to/source.txt", "dest_path": "/path/to/dest.txt"}'

# Via parameter file
echo '{"source_path": "/path/to/source.txt", "dest_path": "/path/to/dest.txt"}' > params.json
uv run task-mind recipe run file_copy --params-file params.json

# Output to file
uv run task-mind recipe run file_copy --params '{"source_path": "a.txt", "dest_path": "b.txt"}' --output-file result.json
```
