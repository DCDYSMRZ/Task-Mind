...
inputs:
  ...
  save_config:
    type: boolean
    required: false
    description: "Whether to save selected tab config to ~/.task-mind/current_target.json, default true"
outputs:
  tabs:
    type: array
...
By default, task-mind automatically connects to the first available page. But in complex scenarios (such as simultaneously opening multiple websites for data collection), you need to explicitly specify the target tab.
...
**List all tabs**:
```bash
uv run task-mind recipe run cdp_list_tabs_and_select_target
# Or with parameters
uv run task-mind recipe run cdp_list_tabs_and_select_target \
  --params '{"action": "list"}'
```
...
```json
{
  ...
  "config_saved": true,
  "config_path": "/home/user/.task-mind/current_target.json",
  "usage_hint": "Use in subsequent commands: uv run task-mind --target-id 3EAFDE3EDB2CF5BDDF32862E38D66FC4 <command>"
}
```
...
- **Config Persistence**: Selected tab information is saved in `~/.task-mind/current_target.json`, available for other tools to read
