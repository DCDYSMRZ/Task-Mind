...
def save_target_config(tab: dict, config_path: Path = None) -> Path:
    ...
    Subsequent task-mind commands can use this configuration via --target-config parameter
    """
    if config_path is None:
        config_path = Path.home() / ".task-mind" / "current_target.json"
    ...
    try:
        ...
        if action == "list":
            ...
            output = {
                ...
                "usage_hint": f"Use in subsequent commands: uv run task-mind --target-id {selected_tab['id']} <command>"
            }
            print(json.dumps(output, ensure_ascii=False, indent=2))
