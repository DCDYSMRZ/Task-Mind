---
name: task-mind-view-content-generate-tips-json
description: JSON file content generation guide. Use this skill when you need to create JSON files that can be previewed via `task-mind view`. Covers formatted display and best practices.
---

# JSON File Content Generation Guide

Preview JSON files via `task-mind view` with automatic formatting and syntax highlighting.

## Preview Commands

```bash
task-mind view data.json                      # Default theme
task-mind view config.json --theme monokai    # Specify theme
```
...
| Theme | Command |
|-------|---------|
| github-dark (default) | `task-mind view data.json` |
| monokai | `task-mind view data.json --theme monokai` |
| atom-one-dark | `task-mind view data.json --theme atom-one-dark` |
