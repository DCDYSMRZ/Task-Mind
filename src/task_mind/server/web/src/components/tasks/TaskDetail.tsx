...
import { Send, MessageSquarePlus, ChevronDown, ChevronRight, List, Terminal, Monitor } from 'lucide-react';
...
export default function TaskDetail() {
  ...
  // View mode: 'steps' | 'terminal' | 'interactive'
  const [viewMode, setViewMode] = useState<'steps' | 'terminal' | 'interactive'>('steps');

  // Floating terminal state
  const [showFloatingTerminal, setShowFloatingTerminal] = useState(false);

  // Whether can continue conversation (running, completed, or error status)
  const canContinue = taskDetail?.status === 'running' || taskDetail?.status === 'completed' || taskDetail?.status === 'error';
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden gap-4 p-scaled-4">
      ...
      <div className="shrink-0 flex flex-col gap-4">
        ...
        <div className="card">
          ...
          >
            ...
            {/* Continue/Interrupt button - placed on the right of title */}
            {canContinue && !showContinue && (
              ...
              >
                ...
                {taskDetail.status === 'running' ? t('tasks.interrupt') : t('tasks.continueConversation')}
              </button>
            )}
            {/* Running status hint */}
      ...
      {/* Output area - takes remaining space, scrolls internally */}
      <div className="card flex-1 min-h-0 flex flex-col overflow-hidden">
        {/* View mode tabs */}
        <div className="flex items-center gap-scaled-2 mb-scaled-3 shrink-0 border-b border-[var(--border-subtle)] pb-scaled-2">
          <button
            className={`flex items-center gap-scaled-2 px-scaled-3 py-scaled-1 rounded transition-colors ${
              viewMode === 'steps'
          ...
          >
            <List className="icon-scaled-sm" />
            <span className="text-scaled-sm">Steps</span>
          </button>
          <button
            className={`flex items-center gap-scaled-2 px-scaled-3 py-scaled-1 rounded transition-colors ${
              viewMode === 'terminal'
          ...
          >
            <Terminal className="icon-scaled-sm" />
            <span className="text-scaled-sm">Terminal</span>
          </button>
          <button
            className={`flex items-center gap-scaled-2 px-scaled-3 py-scaled-1 rounded transition-colors ${
          ...
          >
            <Monitor className="icon-scaled-sm" />
            <span className="text-scaled-sm">Live PTY</span>
          </button>
        </div>
        ...
        <div className="flex-1 min-h-0">
          {viewMode === 'steps' ? (
            ...
            <TerminalView steps={taskDetail.steps} sessionId={taskDetail.session_id} />
          )}
        </div>
      </div>
