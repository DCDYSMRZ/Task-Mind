"""FastAPI application factory for Task-Mind Web Service.
...
from task_mind.server.routes import (
    system_router,
    dashboard_router,
    recipes_router,
    tasks_router,
...
from task_mind.server.websocket import manager, MessageType, create_message
...
from task_mind.server.services.community_recipe_service import CommunityRecipeService
...
def create_app(
    title: str = "Task-Mind Web Service",
    ...
    Args:
        ...
        version: Application version (defaults to task-mind version)
    ...
    app = FastAPI(
        title=title,
        version=version,
        description="Local web service for Task-Mind GUI",
        docs_url="/api/docs",
        redoc_url="/api/redoc",
    ...
    async def serve_spa(full_path: str):
        ...
        return HTMLResponse(
            ...
            <!DOCTYPE html>
            <html>
            <head><title>Task-Mind - Frontend Not Built</title></head>
...
cd src/task-mind/server/web
pnpm install
pnpm build
                </pre>
                <p>Then restart <code>task-mind server</code></p>
            </body>
            </html>
            """,
            status_code=503,
