...
from task_mind.server.models import AgentContinueRequest, AgentStartRequest, TaskItemResponse
...
async def start_agent(request: AgentStartRequest) -> TaskItemResponse:
    ...
    result = await AgentService.start_task(
...
async def continue_agent(session_id: str, request: AgentContinueRequest) -> Dict[str, Any]:
    ...
    This is the correct ID that maps to ~/.task-mind/sessions/claude/{session_id}/.
