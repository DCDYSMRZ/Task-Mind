"""Uvicorn server runner for Task-Mind Web Service.
...
from task_mind.server.utils import (
    SERVER_HOST,
    SERVER_PORT,
    find_available_port,
    get_server_url,
...
def run_server(
    ...
    if reload:
        config = uvicorn.Config(
            app="task-mind.server.app:create_app",
            factory=True,
            host=host,
            port=port,
            log_level=log_level,
...
def run_daemon_server() -> None:
    ...
    log_file = Path.home() / ".task-mind" / "server.log"
    ...
    logger.info(f"Starting Task-Mind server daemon on http://{SERVER_HOST}:{SERVER_PORT}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Task-Mind Web Service Runner")
