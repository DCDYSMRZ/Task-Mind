"""Background daemon process management for Task-Mind Web Service.
...
# Fixed port for Task-Mind server
SERVER_PORT = 8093
SERVER_HOST = "127.0.0.1"

# PID and log file paths
TASK_MIND_DIR = Path.home() / ".task-mind"
PID_FILE = TASK_MIND_DIR / "server.pid"
LOG_FILE = TASK_MIND_DIR / "server.log"


def get_pid_file() -> Path:
    """Get the PID file path, ensuring parent directory exists."""
    TASK_MIND_DIR.mkdir(parents=True, exist_ok=True)
    return PID_FILE
...
def is_task_mind_process(proc: psutil.Process) -> bool:
    """Check if a process is a Task-Mind server process.

    Args:
        proc: psutil.Process to check

    Returns:
        True if process appears to be a Task-Mind server
    """
    try:
        cmdline = proc.cmdline()
        cmdline_str = " ".join(cmdline).lower()
        # Check for task-mind server runner in command line
        return "task-mind" in cmdline_str and ("server" in cmdline_str or "runner" in cmdline_str)
    except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
        return False


def is_server_running() -> Tuple[bool, Optional[int]]:
    """Check if the Task-Mind server is currently running.
    ...
    try:
        ...
        if proc.is_running() and is_task_mind_process(proc):
            return True, pid
    except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
        pass
...
def start_daemon() -> Tuple[bool, str]:
    """Start the Task-Mind server as a background daemon.
    ...
    if running:
        return False, f"Task-Mind server is already running (PID: {existing_pid})"
    ...
    try:
        ...
        return True, f"Task-Mind server started on http://{SERVER_HOST}:{SERVER_PORT} (PID: {proc.pid})"
...
def stop_daemon() -> Tuple[bool, str]:
    """Stop the running Task-Mind server daemon.
    ...
    running, pid = is_server_running()
    if not running or pid is None:
        return False, "Task-Mind server is not running"
    ...
    try:
        ...
        return True, f"Task-Mind server stopped (PID: {pid})"
...
def restart_daemon(force: bool = False) -> Tuple[bool, str]:
    """Restart the Task-Mind server daemon.
