...
class CDPSession(CDPClient):
    ...
    def clear_effects(self) -> None:
        ...
        self.evaluate("""
            ...
            document.querySelectorAll('[data-task-mind-highlight], [style*="pointer"], [style*="spotlight"]').forEach(el => {
                ...
                el.removeAttribute('data-task-mind-highlight');
            });
            // Remove task-mind-added DOM elements (annotate, underline, pointer, etc.)
            document.querySelectorAll('.task-mind-underline, .task-mind-annotation, #task-mind-pointer, #task-mind-underline-style').forEach(el => el.remove());
        """)
        ...
        self.evaluate(f"""
            (function() {{
                document.querySelectorAll('{selector}').forEach(el => {{
                    ...
                    el.setAttribute('data-task-mind-highlight', 'true');
                    if ({lifetime} > 0) {{
                        setTimeout(() => {{
                            el.style.removeProperty('border');
                            el.style.removeProperty('outline');
    ...
    def pointer(self, selector: str, lifetime: int = 5000) -> None:
        ...
        self.evaluate(f"""
            document.querySelectorAll('{selector}').forEach(el => {{
                ...
                el.setAttribute('data-task-mind-pointer', 'true');
                if ({lifetime} > 0) {{
                    setTimeout(() => {{
                        el.style.removeProperty('cursor');
                        el.style.removeProperty('boxShadow');
                        el.removeAttribute('data-task-mind-pointer');
                    }}, {lifetime});
                }}
            }});
        """)
    ...
    def spotlight(self, selector: str, lifetime: int = 5000) -> None:
        ...
        self.evaluate(f"""
            (function() {{
                ...
                if (!document.getElementById('task-mind-spotlight-style')) {{
                    const style = document.createElement('style');
                    style.id = 'task-mind-spotlight-style';
                    style.textContent = `
                        @keyframes task-mind-spotlight-fade {{
                ...
                document.querySelectorAll('{selector}').forEach(el => {{
                    ...
                    el.setAttribute('data-task-mind-spotlight', 'true');

                    if ({lifetime} > 0) {{
                        el.style.animation = 'task-mind-spotlight-fade {lifetime_sec}s forwards';
                        el.addEventListener('animationend', function handler() {{
                            el.style.removeProperty('animation');
                            el.style.removeProperty('zIndex');
                            el.style.removeProperty('position');
                            el.removeAttribute('data-task-mind-spotlight');
    ...
    def annotate(self, selector: str, text: str, position: str = "top", lifetime: int = 5000) -> None:
        ...
        self.evaluate(f"""
            document.querySelectorAll('{selector}').forEach(el => {{
                ...
                annotation.className = 'task-mind-annotation';
            ...
            (function() {{
                ...
                elements.forEach(el => {{
                    ...
                    lines.forEach((line, index) => {{
                        ...
                        underline.className = 'task-mind-underline';
