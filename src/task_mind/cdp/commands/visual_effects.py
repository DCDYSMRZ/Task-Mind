...
class VisualEffectsCommands:
    ...
    def highlight(self, selector: str, color: str = "magenta", border_width: int = 3) -> None:
        ...
        (function() {{
            ...
            if (element) {{
                ...
                element.setAttribute('data-task-mind-highlight', 'true');
            }}
        }})();
        """
    ...
    def spotlight(self, selector: str, opacity: float = 0.7) -> None:
        ...
        (function() {{
            ...
            if (element) {{
                ...
                overlay.id = 'task-mind-spotlight';
    ...
    def clear_effects(self) -> None:
        """Clear all visual effects added by Task-Mind"""
        self.logger.info("Clearing all visual effects")
        
        script = """
        (function() {
            document.querySelectorAll('[data-task-mind-highlight]').forEach(el => {
                el.style.border = '';
                el.style.outline = '';
                el.removeAttribute('data-task-mind-highlight');
            });
            
            ['task-mind-spotlight'].forEach(id => {
                const element = document.getElementById(id);
                if (element) element.remove();
            });
            
            document.querySelectorAll('.task-mind-annotation').forEach(el => el.remove());
        })();
        """
