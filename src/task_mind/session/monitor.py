...
from task_mind.session.formatter import JsonFormatter, TerminalFormatter, create_formatter
...
from task_mind.session.parser import (
    IncrementalParser,
    ParsedRecord,
    record_to_step,
...
def get_claude_dir() -> Path:
    ...
    Supports customization via environment variable TASK_MIND_CLAUDE_DIR.

    Returns:
        Claude Code session directory path
    """
    custom_dir = os.environ.get("TASK_MIND_CLAUDE_DIR")
    if custom_dir:
...
def encode_project_path(project_path: str) -> str:
    ...
    # /home/yammi/repos/Task-Mind -> -home-yammi-repos-Task-Mind
...
def watch_session(
    ...
    Args:
        ...
        json_mode: Whether to use JSON format output
    """
    from task_mind.session.storage import get_session_dir, read_metadata
...
def watch_latest_session(
    ...
    Args:
        ...
        json_mode: Whether to use JSON format output
    """
    from task_mind.session.storage import list_sessions
...
class ClaudeCodeAdapter(AgentAdapter):
    ...
    def parse_record(self, data: Dict) -> Optional[ParsedRecord]:
        ...
        from task_mind.session.parser import IncrementalParser
