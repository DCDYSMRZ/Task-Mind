...
Uses the task-mind server for content serving and CDP for Chrome control.
"""
...
from task_mind.server.services.viewer_service import ViewerService
...
class ChromeViewer:
    ...
    2. Ensuring task-mind server is running
    ...
    def __init__(
        ...
        Args:
            ...
            title: Content title (defaults to filename or "task-mind view")
    ...
    def show(self) -> str:
        ...
        # 2. Ensure task-mind server is running
    ...
    def _ensure_server_running(self) -> None:
        """Ensure task-mind server is running, start if not."""
        from task_mind.server.daemon import is_server_running, start_daemon

        running, _ = is_server_running()
        if not running:
            success, message = start_daemon()
            if not success:
                raise RuntimeError(f"Failed to start task-mind server: {message}")
            # Wait a moment for server to be ready
            time.sleep(0.5)

    def _ensure_chrome_running(self) -> None:
        """Ensure Chrome is running with CDP enabled."""
        from task_mind.cdp.commands.chrome import ChromeLauncher
    ...
    def _open_in_chrome(self, url: str) -> None:
        ...
        from task_mind.cdp import CDPSession
