...
[![Platform](https://img.shields.io/badge/platform-macOS%20%7C%20Linux-lightgrey)](https://github.com/DCDYSMRZ/Task-Mind)
...
```bash
uv tool install task-mind-cli   # 安装 Task-Mind
task-mind init                   # 初始化环境
task-mind server start           # 启动 Web 服务
...
Task-Mind 只为*结果*而存在：能运行的 recipe，能执行的脚本，能提取的数据。如果 AI 不能给你一个交付物，它就没完成工作。

### 2. 你的工具，你掌控

我们拒绝「必须等某家公司造出 AGI 才能享受自动化」的叙事。

Task-Mind 是开源的。你的 recipe，你的 skill，你的 Git 仓库。你积累的是能力，不是订阅费。你打造的工具是*你的* —— 可移植、版本化、独立自主。
...
Claude Code 设计了一种文档架构：commands 和 skills，来实践这类哲学。Task-Mind 锦上添花，在这个基础上，深入贯彻作者的设计哲学：每一个方法论知识，都需要与具体可执行的工具结合起来。

在 Task-Mind 的框架下，skills 是方法论的集合，recipe（"配方"）是可执行工具的集合。

作者的期望：通过 Task-Mind 提供的 Claude Code slash commands（/task-mind.run 等核心指令），建立一种 Agent 规范——使其在面对陌生问题时，能探索并将结果标准化为结构化信息；通过自发感知，主动建立 skills 和 recipe 的关联。

最终，你的 Agent 能充分理解你对某一类工作、事务需求的表述，借助已有的 skills 找到并合理运用相关的 recipes，继而实现"仅需花费少量 token 即可驱动事情自动执行"的结果。

Task-Mind 不是 Agent 本身，而是 Agent 的"骨骼"。

Agent 已经足够聪明，但还不够机灵。Task-Mind 让它记住如何做事。

---

## 如何使用

Task-Mind 结合 Claude Code，通过四个 slash command 构建完整的"探索 → 固化 → 执行"闭环。

```
/task-mind.run     探索研究，积累经验
     ↓
/task-mind.recipe  将经验固化为可复用的配方
/task-mind.test    验证配方（趁上下文还在）
     ↓
/task-mind.exec    通过 skill 指导，快速执行
```
...
```
/task-mind.run 研究如何提取 YouTube 视频字幕
```

Agent 会：
- 创建一个 project 存储此次 run 实例
- 使用 Task-Mind 提供的基础工具（navigate、click、exec-js 等）进行探索
...
```
/task-mind.exec video-production 制作一个关于 AI 的短视频
```
...
```
/task-mind.exec 抓取类似网站
```

**你不需要进入任何平台，不需要看任何流程图。**

| | 编排工具（含 AI 辅助） | Task-Mind |
|--|----------------------|-------|
| AI 做什么 | 帮你画流程图 | 直接替你干活 |
| 你要做什么 | 进平台、看图、调试、改配置 | 说需求、等结果 |
| 产出物 | 一张需要维护的流程图 | 可复用的 recipe |

**编排工具的 AI 是你的"绘图助手"；Task-Mind 的 AI 是你的"执行者"。**

当然，如果你需要定时触发、可视化监控、团队协作审批——编排工具更合适。但如果你只是想把事情做完——Task-Mind 让你用嘴解决问题，不需要学任何平台。
...
Task-Mind 是开源项目——任何人都可以通过 PyPI 安装。但**骨骼**是通用的，**大脑**是私人的。
...
Task-Mind 的理念：**跨环境一致**。无论你在哪台机器、全新安装还是新项目，你的资源都应该随时可用。工具来自 PyPI；大脑来自你的私有仓库。

Task-Mind 暂时不提供社区级的云同步服务。取而代之的是，提供一套命令让你用自己的 Git 仓库管理同步。
...
```bash
# 在项目中编辑完 recipes 后
task-mind publish              # 项目 → 系统
task-mind sync                 # 系统 → 远程 Git
```
...
task-mind deploy               # 远程 Git → 系统
task-mind dev-load             # 系统 → 项目（如果你在开发 Task-Mind）
```

**普通用户**（只使用 Task-Mind）：
```bash
task-mind deploy               # 从你的仓库获取最新资源
# 资源现在在 ~/.claude/ 和 ~/.task-mind/，可以直接使用
```

### 同步范围

只同步 Task-Mind 专属资源：
- `task-mind.*.md` 命令（不动你的其他 Claude 命令）
- `task-mind-*` skills（不动你的其他 skills）
- `~/.task-mind/recipes/` 下的所有配方

你的个人非 Task-Mind 的 Claude 命令和 skills 永远不会被触及。
...
- ✅ 媒体查看器 - `task-mind view` 支持视频、图片、音频、3D 模型（glTF/GLB）
- ✅ 社区 Recipe - `recipe install/uninstall/update/search/share` 支持社区贡献
- ✅ WebSocket 实时同步 - 服务端推送更新，减少轮询
- ✅ YouTube Recipe - 下载视频、提取字幕和转录文本
- ✅ 跨平台开机自启 - `task-mind autostart` 管理服务器开机启动（macOS/Linux/Windows）
- ✅ 国际化支持 - UI 国际化，尊重用户语言偏好
- ✅ Web 服务模式 - `task-mind server` 启动浏览器端 GUI（端口 8093）
...
- 项目问题：[提交 Issue](https://github.com/DCDYSMRZ/Task-Mind/issues)
- 技术讨论：[Discussions](https://github.com/DCDYSMRZ/Task-Mind/discussions)

### 贡献者

<a href="https://github.com/DCDYSMRZ/Task-Mind/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=DCDYSMRZ/Task-Mind" />
</a>
