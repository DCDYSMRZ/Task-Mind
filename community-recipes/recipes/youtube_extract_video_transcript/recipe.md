...
**配方执行器说明**：生成的配方本质上是JavaScript代码，通过CDP的Runtime.evaluate接口注入到浏览器中执行。因此，执行配方的标准方式是使用 `uv run task-mind chrome exec-js` 命令。
...
3. 执行配方：
   ...
   uv run task-mind chrome exec-js src/task-mind/recipes/youtube_extract_video_transcript.js --return-value
   ```
...
- Chrome CDP已连接（`task-mind status` 显示连接正常）
...
- 如果YouTube改版导致脚本失效，使用 `/task-mind.recipe update youtube_extract_video_transcript` 更新
